...
/**
 * Function to be used in [ConvertSchemaDsl] ([DataFrame.convertTo]) to help convert a DataFrame to
 * adhere to an OpenApi schema. Is used in generated OpenAPI code.
 */
public fun ConvertSchemaDsl<*>.convertDataRowsWithOpenApi() {
    ...
    convertIf({ fromType, toSchema ->
        ...
        try {
            it.convertRecursiveListOfDataFrame(toSchema.type) { convertDataRowsWithOpenApi() }
        } catch (_: Exception) {
            it
        }
    }
...
/**
 * @return [Pair] of result and the recursive depth. `true` if Receiver is a recursive list of
 *   [DataFrame]s, like [List]<[List]<[DataFrame]<*>>>
...
private fun KType.isRecursiveListOfDataFrame(depth: Int = 0): Pair<Boolean, Int> =
    when (jvmErasure) {
        typeOf<List<*>>().jvmErasure ->
            arguments[0].type?.isRecursiveListOfDataFrame(depth + 1) ?: (false to depth)
 ...
 * @receiver Recursive [List] of [DataFrame]s, like [List]<[List]<[DataFrame]<*>>>, for which to
 *   convert the [DataFrame]s.
 */
private fun Any?.convertRecursiveListOfDataFrame(
    ...
    when (this) {
        is List<*> ->
            map { it?.convertRecursiveListOfDataFrame(type.arguments[0].type!!, convertTo) }
