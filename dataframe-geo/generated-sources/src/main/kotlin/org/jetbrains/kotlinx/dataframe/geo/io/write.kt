 ...
 *
 *
 * Writes this [<code>GeoDataFrame</code>][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame] into a [GeoJSON](https://geojson.org/) file.
 *
 ...
 * row in this [<code>GeoDataFrame</code>][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame]:
 ...
 * > deprecated "crs" field [is written for now](https://github.com/Kotlin/dataframe/issues/1187);
 *
 * @param [path] a [<code>Path</code>][Path] to GeoJSON file
 */
...
/**
 * Writes this [<code>GeoDataFrame</code>][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame] into a
 ...
 *
 * Each feature in the resulting shapefile corresponds to a single row in this [<code>GeoDataFrame</code>][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame]:
 * * `"geometry"` value is taken from the `geometry` column.
 * * other column values are written as feature attributes.
 *
 * The CRS is written to the schema if it is defined in this [<code>GeoDataFrame</code>][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame]. Otherwise,
 * [<code>default CRS</code>][GeoDataFrame.DEFAULT_CRS] is used.
 *
 * If the target directory does not exist, it is created automatically.
 *
 * @param [directoryPath] a [<code>String</code>][String] path to a Shapefile directory
 */
 ...
 * If the target directory does not exist, it is created automatically.
 *
 * @param [directory] a [<code>Path</code>][Path] to a Shapefile directory
 */
