 ...
 * [GeoJSON](https://geojson.org/) file.
 *
 * Each [`Feature`](https://datatracker.ietf.org/doc/html/rfc7946#section-3.2) in the resulting
 * GeoJSON file corresponds to a single row in this
 ...
 * `roads.dbf`, and related files inside that directory.
 *
 * Each feature in the resulting shapefile corresponds to a single row in this
 * [GeoDataFrame][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame]:
 * * `"geometry"` value is taken from the `geometry` column.
 * * other column values are written as feature attributes.
 *
 * The CRS is written to the schema if it is defined in this
 * [GeoDataFrame][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame]. Otherwise,
 * [default CRS][GeoDataFrame.DEFAULT_CRS] is used.
 *
 * If the target directory does not exist, it is created automatically.
 *
