 ...
 * [`Feature`](https://datatracker.ietf.org/doc/html/rfc7946#section-3.2) from the input file :
 ...
 *
 * > Uses the [GeoTools](https://geotools.org/) GeoJSON parser. Nested properties are not supported
 * > and parsed as [`LinkedHashMap<*, *>`][java.util.LinkedHashMap]. According to the specification,
 * > the CRS in GeoJSON should be [WGS 84](https://datatracker.ietf.org/doc/html/rfc7946#section-4);
 * > deprecated "crs" field is used for now; if it is not present, it is read as `null`.
 */
@ExcludeFromSources
internal typealias ReadGeoJsonSnippet = Nothing

/**
 * Reads a [GeoJSON](https://geojson.org/) file into a
 * [GeoDataFrame][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame].
 *
 * Each row in the resulting [GeoDataFrame][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame]
 * corresponds to a single [`Feature`](https://datatracker.ietf.org/doc/html/rfc7946#section-3.2)
 * from the input file :
 * * `"geometry"` value is stored in the `geometry` column.
 * * `"properties"` are flattened into individual columns using their keys as column names.
 * * `"crs"` is read as `null` if not specified (interpreted as WGS 84 by default).
 ...
 */
public fun GeoDataFrame.Companion.readGeoJson(path: String): GeoDataFrame<*> =
    readGeoJson(asUrl(path))
...
public fun GeoDataFrame.Companion.readGeoJson(url: URL): GeoDataFrame<*> =
    url.openStream().use { inputStream ->
        val featureCollection =
            FeatureJSON().readFeatureCollection(inputStream) as SimpleFeatureCollection
 ...
 * @return a new [GeoDataFrame] with the data from the GeoJSON file
 */
public fun DataFrame.Companion.readGeoJson(path: String): GeoDataFrame<*> =
    GeoDataFrame.readGeoJson(path)
 ...
 * @return a new [GeoDataFrame] with the data from the GeoJSON file
 */
public fun DataFrame.Companion.readGeoJson(url: URL): GeoDataFrame<*> =
    GeoDataFrame.readGeoJson(url)

/**
 * Reads a [Shapefile](https://doc.arcgis.com/en/arcgis-online/reference/shapefiles.htm) file into a
 * [GeoDataFrame].
 *
 * Each row in the resulting [GeoDataFrame] corresponds to a single feature from the shapefile:
 ...
 * * Path to a directory containing a shapefile with the same name as the directory (expects
 *   `<dir>/<dir>.shp`).
 *
 * Requires `.shx` and `.dbf` files with the same name to be present in the same directory as the
 * `.shp` file.
 */
@ExcludeFromSources
 ...
 *
 * Each row in the resulting [GeoDataFrame][org.jetbrains.kotlinx.dataframe.geo.GeoDataFrame]
 * corresponds to a single feature from the shapefile:
 * * `Geometry` is stored in the `geometry` column.
 * * Attribute data from the `.dbf` file is stored in separate columns.
 * * CRS is read from the `.prj` file if present, otherwise set to `null`.
 *
 ...
 * ```
 *
 * @param path path to a Shapefile (*.shp or *.shp.gz file), or to a directory containing such a
 *   file
...
public fun GeoDataFrame.Companion.readShapefile(path: String): GeoDataFrame<*> {
    val url =
        if (isUrl(path)) resolveRemoteShapefileUrl(URI(path).toURL()) else resolveShapefileUrl(path)
 ...
 * @return a new [GeoDataFrame] with the data from the Shapefile
 */
public fun DataFrame.Companion.readShapefile(path: String): GeoDataFrame<*> =
    GeoDataFrame.readShapefile(path)
 ...
 * @return a new [GeoDataFrame] with the data from the Shapefile
 */
public fun DataFrame.Companion.readShapefile(url: URL): GeoDataFrame<*> =
