...
/**
 * A data structure representing a geographical DataFrame, combining spatial data with an optional
 * Coordinate Reference System (CRS).
 *
 * @param T The type parameter extending `WithGeometry`, indicating the presence of a geometry
 *   column.
 ...
 */
public class GeoDataFrame<T : WithGeometry>(
     ...
     *
     * This function opens a modification scope where the current [DataFrame] can be transformed
     * using [Kotlin DataFrame operations](https://kotlin.github.io/dataframe/operations.html). The
     * transformation block receives the original [DataFrame] both as a receiver and as an explicit
     * argument, allowing flexible modifications.
     *
     ...
     * @return A new [GeoDataFrame] instance with the modified DataFrame while preserving the
     *   original CRS.
     */
     ...
     *
     * This function reprojects the geometry data from the current CRS to a target CRS. If no target
     * CRS is specified and the `GeoDataFrame` has no CRS, WGS 84 is used by default.
     *
    ...
    public fun applyCrs(targetCrs: CoordinateReferenceSystem): GeoDataFrame<T> {
        ...
        return GeoDataFrame(df.update { geometry }.with { JTS.transform(it, transform) }, targetCrs)
    }
