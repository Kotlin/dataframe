 ...
 * fall through to the [<code>java.sql.Types.BIT</code>][java.sql.Types.BIT] default, [<code>Boolean</code>][Boolean], which does not match the values
 * (see #2087). A single-bit column really is a [<code>Boolean</code>][Boolean] and is left alone.
 *
 * The declared column width is the only thing in the metadata that tells the two apart, and it is
 * [<code>TableColumnMetadata.size</code>][TableColumnMetadata.size] — which is `0` when the driver does not report a display size at all.
 * A `BIT(M)` column of unknown width therefore still maps to [<code>Boolean</code>][Boolean]; both drivers do report the
