...
public object MsSql : DbType("sqlserver") {
     ...
     * mapping and the values [<code>ResultSet.getObject</code>][ResultSet.getObject] returns disagreed (see #2087):
     *
     * - `DATETIMEOFFSET` is reported with the driver-specific JDBC type `-155` and
     *   `microsoft.sql.DateTimeOffset` as its column class, which the default mapping has no entry
     *   for, so it fell back to [<code>String</code>][String] while the driver returned a `DateTimeOffset`. It is read as
     *   [<code>OffsetDateTime</code>][OffsetDateTime]: a JDK type that keeps the UTC offset of the value, and the one the default
     *   mapping already uses for [<code>java.sql.Types.TIMESTAMP_WITH_TIMEZONE</code>][java.sql.Types.TIMESTAMP_WITH_TIMEZONE]. The driver produces it
     *   directly, see [<code>getValueFromResultSet</code>][getValueFromResultSet].
     * - Any column the driver reports `java.lang.Short` for is read as [<code>Short</code>][Short], because that is what
     *   it then returns, while the default mapping declared it [<code>Int</code>][Int]. Under MSSQL those columns are
     *   `SMALLINT` and `TINYINT`; [<code>Short</code>][Short] is also the smallest Kotlin type that fits MSSQL's
     *   `TINYINT`, which — unlike in most databases — is *unsigned* (`0..255`) and so does not fit a
     *   [<code>Byte</code>][Byte]. This branch deliberately keys on the reported column class and not on
     *   [<code>TableColumnMetadata.jdbcType</code>][TableColumnMetadata.jdbcType]: H2 in MSSQL mode delegates its mapping here, and its
     *   `SMALLINT` is reported as [<code>java.sql.Types.SMALLINT</code>][java.sql.Types.SMALLINT] too but comes back as an `Integer`, so
     *   keying on the JDBC type would type that column [<code>Short</code>][Short] and make reading it throw.
     ...
     *   honestly be typed [<code>Any</code>][Any]. The default mapping declared it [<code>String</code>][String].
     */
    ...
    /**
     * Reads `DATETIMEOFFSET` columns as [<code>OffsetDateTime</code>][OffsetDateTime] directly from the driver,
     * see [<code>getExpectedJdbcType</code>][getExpectedJdbcType]. Plain [<code>ResultSet.getObject</code>][ResultSet.getObject] would return a
     * driver-specific `microsoft.sql.DateTimeOffset` instead.
     *
     * This uses the typed [<code>ResultSet.getObject</code>][ResultSet.getObject] overload, which `mssql-jdbc` supports for
     * `DATETIMEOFFSET` → [<code>OffsetDateTime</code>][OffsetDateTime] since 6.0; on an older driver it throws
     * [<code>java.sql.SQLFeatureNotSupportedException</code>][java.sql.SQLFeatureNotSupportedException] rather than falling back, so that the mismatch is
     * reported instead of silently producing a different column type.
     */
