...
/**
 * Validates the provided limit to ensure it is either null or a positive integer. Throws an
 * [IllegalArgumentException] if the limit is negative or zero.
 *
 ...
 *
 * This utility guarantees proper closing of the connection and safe rollback in read-only mode. It
 * should be used when the user does not manually manage JDBC connections.
 *
 * @param [dbConfig] The configuration used to create the connection.
 * @param [dbType] Optional database type (not used here but can be passed through for logging or
 *   future extensions).
 ...
 *
 * It ensures compatibility with most SQL database naming conventions, thus minimizing risks of
 * invalid names or injection vulnerabilities.
 *
...
/**
 * SQL table name pattern matching: __catalog.schema.table__ Allows alphanumeric characters and
 * underscores, must start with letter or underscore
 */
private val SQL_TABLE_NAME_PATTERN =
...
internal val FORBIDDEN_PATTERNS_REGEX =
    listOf(
...
/**
 * Checks if a given string contains forbidden patterns or keywords. Logs a clear and friendly
 * message if any forbidden pattern is found.
 *
 ...
 *
 * ### Key Notes:
 * - Reserved keywords like `DROP`, `DELETE`, `ALTER`, etc., are forbidden **only when they appear
 *   as standalone commands**.
 * - Reserved words as parts of table or column names (e.g., `HELLO_ALTER`, `myDropTable`) **are
 *   allowed**.
 * - Inline or multi-line comments (`--` or `/* */`) are restricted to prevent potential SQL
 *   injection attacks.
 * - Multiple SQL statements separated by semicolons (`;`) are not allowed to prevent the execution
...
/**
 * Validates if the SQL query is safe and starts with SELECT. Ensures a proper syntax structure,
 * checks for balanced quotes, and disallows dangerous commands or patterns.
 */
