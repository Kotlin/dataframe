...
/**
 * Represents all type information that can be retrieved from an SQL column. This can be extended
 * for your specific [DbType] if you need extra information.
 *
 ...
 * @param D the type of the column values after preprocessing, [preprocessedValueType]. Will be
 *   equal to [J] if [valuePreprocessor] is `null`.
 * @param P the type of the column values after postprocessing, comparable to
 *   [DataColumn][DataColumn]`<`[P][P]`>` of [targetSchema]. Will be equal to [D] if [columnBuilder]
 *   is `null`.
 * @property targetSchema the target schema of the column after running the optional
 *   [valuePreprocessor] and [columnBuilder]. Can be `null` if the target schema is dependent on the
 *   runtime input and thus cannot be determined from input types alone.
 ...
 * @property columnBuilder an optional function that converts a [List] with values of type [D] to a
 *   [DataColumn] of with values of type [P].
 */
public class JdbcToDataFrameConverter<J, D, P>(
    ...
    public fun buildDataColumnOrNull(
        name: String,
        values: List<D>,
...
public fun JdbcToDataFrameConverter<*, *, *>.castToAny():
    JdbcToDataFrameConverter<Any?, Any?, Any?> = cast()
...
public inline fun <reified J> jdbcToDfConverterFor(
    ...
    jdbcToDfConverterFor(jdbcSourceType = typeOf<J?>().withNullability(isNullable))
...
public fun <J, D, P> JdbcToDataFrameConverter<J, D, P>.withTargetSchema(
    ...
    withColumnBuilder(targetSchema = targetSchema, canIntroduceNulls = false, columnBuilder = null)
...
public fun <J, D, P> JdbcToDataFrameConverter<J, D, *>.withColumnBuilder(
    ...
    return cast<J, D, P>().copy(targetSchema = targetSchema, columnBuilder = columnBuilder)
}
...
/**
 * This preprocessor can be created for types where you want to convert the values coming from
 * [ResultSet.getObject] to a different type more suitable to be put in a [DataColumn]
 *
...
public fun interface DbValuePreprocessor<in J, out D> {
     ...
     *
     * If you intend to create a [org.jetbrains.kotlinx.dataframe.columns.ColumnGroup], return a
     * [org.jetbrains.kotlinx.dataframe.DataRow] here.
     *
     * If you intend to create a [org.jetbrains.kotlinx.dataframe.columns.FrameColumn], return a
     * [org.jetbrains.kotlinx.dataframe.DataFrame] here.
     */
    public fun preprocess(jdbcValue: J): D
}

@Suppress("UNCHECKED_CAST")
public fun <J, D> DbValuePreprocessor<*, *>.cast(): DbValuePreprocessor<J, D> =
...
public fun interface DbColumnBuilder<in D, out P> {
    ...
    /**
     * Converts the given [values]: [DataColumn] with values of type [D] to a [DataColumn] of with
     * values of type [P].
     */
...
@Suppress("UNCHECKED_CAST")
public fun <D, P> DbColumnBuilder<*, *>.cast(): DbColumnBuilder<D, P> =
    this as DbColumnBuilder<D, P>
