...
public object MsSql : DbType("sqlserver") {
     ...
     *   for, so it fell back to [String] while the driver returned a `DateTimeOffset`. It is read as
     *   [OffsetDateTime]: a JDK type that keeps the UTC offset of the value, and the one the default
     *   mapping already uses for [java.sql.Types.TIMESTAMP_WITH_TIMEZONE]. The driver produces it
     *   directly, see [getValueFromResultSet].
     * - Any column the driver reports `java.lang.Short` for is read as [Short], because that is what
     *   it then returns, while the default mapping declared it [Int]. Under MSSQL those columns are
     *   `SMALLINT` and `TINYINT`; [Short] is also the smallest Kotlin type that fits MSSQL's
     *   `TINYINT`, which — unlike in most databases — is *unsigned* (`0..255`) and so does not fit a
     *   [Byte]. This branch deliberately keys on the reported column class and not on
     *   [TableColumnMetadata.jdbcType]: H2 in MSSQL mode delegates its mapping here, and its
     *   `SMALLINT` is reported as [java.sql.Types.SMALLINT] too but comes back as an `Integer`, so
     *   keying on the JDBC type would type that column [Short] and make reading it throw.
     ...
     *
     * This uses the typed [ResultSet.getObject] overload, which `mssql-jdbc` supports for
     * `DATETIMEOFFSET` → [OffsetDateTime] since 6.0; on an older driver it throws
     * [java.sql.SQLFeatureNotSupportedException] rather than falling back, so that the mismatch is
