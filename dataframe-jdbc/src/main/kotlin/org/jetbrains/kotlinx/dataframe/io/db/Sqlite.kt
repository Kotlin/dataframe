 ...
 * [KType] must correspond to JDBC actual type. Even for default Sqlite types, you can override the
 * actual [KType] after reading the column by providing a custom type in [customTypesMap].
 */
public class Sqlite(public val customTypesMap: Map<String, KType> = mapOf()) : DbType("sqlite") {
    ...
    override fun getExpectedJdbcType(tableColumnMetadata: TableColumnMetadata): KType =
        customTypesMap[tableColumnMetadata.sqlTypeName]?.withNullability(
            tableColumnMetadata.isNullable
        ) ?: super.getExpectedJdbcType(tableColumnMetadata)

    override fun isSystemTable(tableMetadata: TableMetadata): Boolean =
        tableMetadata.name.startsWith("sqlite_")
    ...
    public companion object {
        ...
        public fun withCustomTypes(customTypesMap: Map<String, KType>): Sqlite =
            Sqlite(customTypesMap)
    }
}
