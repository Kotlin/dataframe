...
internal const val MSSQL_TEST_DATABASE_NAME = "testKDFdatabase"
...
internal fun setUpMsSqlTestData(connection: Connection) {
    connection.createStatement().use { st ->
        ...
        val dropDatabaseQuery = "IF DB_ID('$MSSQL_TEST_DATABASE_NAME') IS NOT NULL\n" +
            "DROP DATABASE $MSSQL_TEST_DATABASE_NAME"
        st.executeUpdate(dropDatabaseQuery)

        // Create the test database
        val createDatabaseQuery = "CREATE DATABASE $MSSQL_TEST_DATABASE_NAME"
        st.executeUpdate(createDatabaseQuery)

        // Use the newly created database
        val useDatabaseQuery = "USE $MSSQL_TEST_DATABASE_NAME"
...
internal fun tearDownMsSqlTestData(connection: Connection) {
    connection.createStatement().use { st ->
        st.execute("USE master")
        st.execute("DROP DATABASE IF EXISTS $MSSQL_TEST_DATABASE_NAME")
    }
    connection.close()
}

abstract class MsSqlTestBase {
    ...
    fun `readAllSqlTables without catalogue should only return tables from URL database`() {
        ...
        try {
            ...
            connect(MSSQL_TEST_DATABASE_NAME).use { scopedConn ->
