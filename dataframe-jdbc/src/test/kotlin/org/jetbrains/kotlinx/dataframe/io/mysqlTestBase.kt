...
internal const val MYSQL_TEST_DATABASE_NAME = "testKDFdatabase"
...
internal fun setUpMySqlTestData(connection: Connection) {
    connection.createStatement().use { st ->
        ...
        val dropDatabaseQuery = "DROP DATABASE IF EXISTS $MYSQL_TEST_DATABASE_NAME"
        st.executeUpdate(dropDatabaseQuery)

        // Create the test database
        val createDatabaseQuery = "CREATE DATABASE $MYSQL_TEST_DATABASE_NAME"
        st.executeUpdate(createDatabaseQuery)

        // Use the newly created database
        val useDatabaseQuery = "USE $MYSQL_TEST_DATABASE_NAME"
...
internal fun tearDownMySqlTestData(connection: Connection) {
    try {
        connection.createStatement().use { st -> st.execute("DROP TABLE IF EXISTS table1") }
        connection.createStatement().use { st -> st.execute("DROP TABLE IF EXISTS table2") }
        connection.createStatement().use { st ->
            st.execute("DROP DATABASE IF EXISTS $MYSQL_TEST_DATABASE_NAME")
        }
        connection.close()
...
abstract class MySqlTestBase {
    ...
    fun `readAllSqlTables without catalogue should only return tables from URL database`() {
        ...
        try {
            ...
            connect(MYSQL_TEST_DATABASE_NAME).use { scopedConn ->
