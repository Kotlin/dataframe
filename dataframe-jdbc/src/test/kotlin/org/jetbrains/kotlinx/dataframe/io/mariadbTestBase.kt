...
internal const val MARIADB_TEST_DATABASE_NAME = "testKDFdatabase"
...
internal fun setUpMariadbTestData(connection: Connection) {
    connection.createStatement().use { st ->
        // Drop the test database if it exists
        val dropDatabaseQuery = "DROP DATABASE IF EXISTS $MARIADB_TEST_DATABASE_NAME"
        st.executeUpdate(dropDatabaseQuery)

        // Create the test database
        val createDatabaseQuery = "CREATE DATABASE $MARIADB_TEST_DATABASE_NAME"
        st.executeUpdate(createDatabaseQuery)

        // Use the newly created database
        val useDatabaseQuery = "USE $MARIADB_TEST_DATABASE_NAME"
...
internal fun tearDownMariadbTestData(connection: Connection) {
    try {
        connection.createStatement().use { st -> st.execute("DROP TABLE IF EXISTS table1") }
        connection.createStatement().use { st -> st.execute("DROP TABLE IF EXISTS table2") }
        connection.createStatement().use { st ->
            st.execute("DROP DATABASE IF EXISTS $MARIADB_TEST_DATABASE_NAME")
        }
        connection.close()
...
abstract class MariadbTestBase {
    ...
    protected abstract fun connect(database: String? = null): Connection
