...
class DuckDbTest {
    ...
    fun `read each general-purpose DuckDB type`() {
        ...
        val ts1 = df["timestamptz_col"].single()
        val expectedTs1 = GeneralPurposeTypes.expected["timestamptz_col"].single()
        ts1 shouldBe expectedTs1

        val ts2 = df["timestampwtz_col"].single()
        val expectedTs2 = GeneralPurposeTypes.expected["timestampwtz_col"].single()
        ts2 shouldBe expectedTs2

        df.cast<GeneralPurposeTypes>(verify = true) shouldBe GeneralPurposeTypes.expected
        df.assertInferredTypesMatchSchema()
    }
