...
public fun readOpenApiAsString(
    ...
    require(isOpenApiStr(openApiAsString)) {
        "Not an OpenApi specification with type schemas: $openApiAsString"
    }
 ...
 *
 * @param swaggerParseResult the result of parsing an OpenAPI specification, created using
 *   [readOpenApi] or [readOpenApiAsString].
 * @param extensionProperties whether to add extension properties to the generated interfaces. This
 *   is usually not necessary, since both the KSP- and the Gradle plugin, will add extension
 *   properties to the generated code.
...
private fun readOpenApi(
    ...
    val openApi =
        swaggerParseResult.openAPI
            ?: error("Failed to parse OpenAPI, ${swaggerParseResult.messages.toList()}")

    val topInterfaceName = ValidFieldName.of(name)

    // take the components.schemas from the openApi spec and convert them to a list of Markers,
    // representing the
    // interfaces, enums, and typeAliases that need to be generated.
    val result =
        openApi.components?.schemas?.toMap()?.toMarkers(topInterfaceName)?.toList() ?: emptyList()
            ...
            .generate(
                ...
                interfaceMode =
                    when (marker) {
                        ...
                        is OpenApiMarker.MarkerAlias -> InterfaceGenerationMode.TypeAlias
                    },
                extensionProperties = false,
                readDfMethod =
                    if (marker is OpenApiMarker.Interface) DefaultReadOpenApiMethod else null,
            )
            .declarations
    ...
    val (typeAliases, markers) =
        result.partition { it is OpenApiMarker.TypeAlias || it is OpenApiMarker.MarkerAlias }

    val generatedMarkers = markers.map(::toCode).reduceOrNull(Code::merge) ?: ""

    val generatedTypeAliases = typeAliases.map(::toCode).reduceOrNull(Code::merge) ?: ""

    val generatedExtensionProperties =
        if (!extensionProperties) {
            ""
        } else {
            result.map(::toExtensionProperties).reduceOrNull(Code::merge) ?: ""
        }
    ...
    val helperCompanionObject =
        if (!generateHelperCompanionObject) {
            ...
            val accessors =
                markers.filterIsInstance<OpenApiMarker.Interface>().joinToString("\n|        ") {
...
/**
 * Converts named OpenApi schemas to a list of [OpenApiMarker]s. Will cause an exception for
 * circular references, however they shouldn't occur in OpenApi specs.
 *
 * Some explanation: OpenApi provides schemas for all the types used. For each type, we want to
 * generate a [Marker] (Which can be an interface, enum or typealias). However, the OpenApi schema
 * is not ordered per se, so when we are reading the schema it might be that we have a reference to
 * a (super)type (which are queried using `getRefMarker`) for which we have not yet created a
 * [Marker]. In that case, we "pause" that one (by returning `CannotFindRefMarker`) and try to read
 * another type schema first. Circular references cannot exist since it's encoded in JSON, so we
 * never get stuck in an infinite loop. When all markers are "retrieved" (so turned from a
 * [RetrievableMarker] to a [MarkerResult.OpenApiMarker]), we're done and have converted everything!
 * As for `produceAdditionalMarker`: In OpenAPI not all enums/objects have to be defined as a
 * separate schema. Although recommended, you can still define an object anonymously directly as a
 * type. For this, we have `produceAdditionalMarker` since during the conversion of a schema ->
 * [Marker] we get an additional new [Marker].
 */
private fun Map<String, Schema<*>>.toMarkers(
    topInterfaceName: ValidFieldName
): List<OpenApiMarker> {
    // Convert the schemas to toMarker calls that can be repeated to resolve references.
    val retrievableMarkers =
        mapValues { (typeName, value) ->
    ...
    // convert all the retrievable markers to actual markers, resolving references as we go and if
    // possible
    while (retrievableMarkers.isNotEmpty()) {
        try {
            retrievableMarkers.entries.first { (name, retrieveMarker) ->
                // To avoid producing additional markers twice due to a CannotFindRefMarker, save
                // them here first
                ...
                val produceAdditionalMarker = ProduceAdditionalMarker { validName, marker, _ ->
                    ...
                    var attempt = 1
                    while (
                        result.quotedIfNeeded in markers ||
                            result.quotedIfNeeded in additionalMarkers
                    ) {
                        result =
                            ValidFieldName.of(
                                baseName.unquoted +
                                    (if (result.needsQuote) " ($attempt)" else "$attempt")
                            )
                        attempt++
                    }

                    additionalMarkers[result.quotedIfNeeded] =
                        marker.withName(result.quotedIfNeeded)
                ...
                when (res) {
                    ...
                    is MarkerResult.CannotFindRefMarker ->
                        false // Cannot find a referenced Marker for this one, so we'll try again
                              // later
                }
            }
 ...
 * - `type: object`
 *     - `properties:` (`additionalProperties` are ignored) creating an [OpenApiMarker.Interface]
 *       using the fields in the properties.
 *     - `additionalProperties:` (if `properties` is not present) creating an
 *       [OpenApiMarker.AdditionalPropertiesInterface] using the additionalProperties schema as type
 *       of `value`.
 * - `type:` if type is something else, generating a type alias for it. This can be a
 *   [OpenApiMarker.TypeAlias] or a [OpenApiMarker.MarkerAlias].
 *
 ...
 * @return A [MarkerResult.OpenApiMarker] if successful, otherwise
 *   [MarkerResult.CannotFindRefMarker].
 */
private fun Schema<*>.toMarker(
    ...
    @Suppress("NAME_SHADOWING") val required = (this.required ?: emptyList()) + required
    val nullable = nullable ?: false
    return when {
        // If allOf is defined, multiple objects are to be composed together. This is done using
        // inheritance.
        // https://swagger.io/docs/specification/data-models/oneof-anyof-allof-not/#allof
        allOf != null -> {
            val allOfSchemas =
                allOf!!.associateWith { it.toOpenApiType(getRefMarker = getRefMarker) }
            ...
            for ((schema, openApiTypeResult) in allOfSchemas) {
                when (openApiTypeResult) {
                    ...
                    is OpenApiTypeResult.OpenApiType -> {
                        ...
                        val fieldTypeResult =
                            openApiType.toFieldType(
                                schema = schema,
                                schemaName = typeName,
                                nullable = nullable,
                                getRefMarker = getRefMarker,
                                produceAdditionalMarker = { name, marker, isTopLevelObject ->
                                    // the top-level object must not be produced as additional
                                    // marker.
                                    // instead, we just need it to be the tempMarker for which we
                                    // gather just the fields.
                                    if (isTopLevelObject) {
                                        tempMarker = marker
                                        name.quotedIfNeeded
                                    } else {
            ...
            val enumMarker =
                produceNewEnum(
                    ...
                    produceAdditionalMarker =
                        ProduceAdditionalMarker.NOOP, // we need it here, not as additional marker
                )
        ...
        type == "object" ->
            when {
                ...
                properties != null -> {
                    ...
                    val fields = buildList {
                        for ((name, property) in (properties ?: emptyMap())) {
                            ...
                            val openApiTypeResult =
                                property.toOpenApiType(getRefMarker = getRefMarker)
                            ...
                            when (openApiTypeResult) {
                                ...
                                is OpenApiTypeResult.UsingRef -> {
                                    keyValuePaths +=
                                        openApiTypeResult.marker.additionalPropertyPaths.map {
                                            it.prepend(name)
                                        }
                                    ...
                                    val fieldType =
                                        openApiTypeResult.marker.toFieldType().let {
                                ...
                                is OpenApiTypeResult.Enum -> {
                                    ...
                                    this +=
                                        generatedFieldOf(
                                            overrides = false,
                                            fieldName =
                                                ValidFieldName.of(name.snakeToLowerCamelCase()),
                                            columnName = name,
                                            fieldType =
                                                FieldType.ValueFieldType(
                                                    typeFqName =
                                                        enumMarker.name +
                                                            if (enumMarker.nullable || !isRequired)
                                                                "?"
                                                            else ""
                                                ),
                                        )
                                ...
                                is OpenApiTypeResult.OpenApiType -> {
                                    ...
                                    when (fieldTypeResult) {
                                        ...
                                        is FieldTypeResult.FieldType -> {
                                            ...
                                            keyValuePaths +=
                                                fieldTypeResult.additionalPropertyPaths.map {
                                                    it.prepend(name)
                                                }
                ...
                properties == null &&
                    ...
                    val openApiTypeResult =
                        (additionalProperties as? Schema<*>)?.toOpenApiType(
                            getRefMarker = getRefMarker
                        )
                        ...
                        when (openApiTypeResult) {
                            ...
                            is OpenApiTypeResult.UsingRef -> {
                                ...
                                additionalPropertyPaths +=
                                    marker.additionalPropertyPaths.map { it.prependWildcard() }
                                marker.toFieldType()
                            }

                            is OpenApiTypeResult.OpenApiType -> {
                                val fieldTypeResult =
                                    openApiTypeResult.openApiType.toFieldType(
                                        schema = this,
                                        schemaName = typeName,
                                        nullable = openApiTypeResult.nullable,
                                        getRefMarker = getRefMarker,
                            ...
                            null -> FieldType.ValueFieldType(typeFqName = typeOf<Any?>().toString())
                        }
        ...
        // If type is something else, produce it as type alias. Can be a reference to another
        // OpenApi type or something else.
        else -> {
            val openApiTypeResult = toOpenApiType(getRefMarker = getRefMarker)
            ...
            val typeAliasMarker =
                when (openApiTypeResult) {
                            ...
                            when (typeResult) {
                                ...
                                is FieldTypeResult.FieldType ->
                                    when (typeResult.fieldType) {
                                        ...
                                        is FieldType.GroupFieldType -> typeResult.fieldType.name
                    ...
                    is OpenApiTypeResult.Enum ->
                        error("cannot happen, since enum != null is checked earlier")
                }
...
/**
 * Converts a single property of an OpenApi type schema to [OpenApiTypeResult] representing a single
 * type for DataFrame. It must either have `$ref`, `type`, `enum`, `oneOf`, `anyOf`, or `not`
 * defined. It can become an [OpenApiType], [OpenApiMarker] reference or unresolved reference (if
 * `$ref:` is set), enum (if `enum:` is set). `anyOf` and `oneOf` types are merged.
 *
 * These results still have to be converted to [FieldType]s to be able to generate [OpenApiMarker]s
 * from it (unless it's a [OpenApiTypeResult.UsingRef] of course).
 *
...
private fun Schema<*>.toOpenApiType(getRefMarker: GetRefMarker): OpenApiTypeResult {
    ...
    if (`$ref` != null) {
        ...
        return when (val it = getRefMarker(typeName)) {
            is MarkerResult.CannotFindRefMarker -> OpenApiTypeResult.CannotFindRefMarker

            is MarkerResult.OpenApiMarker -> OpenApiTypeResult.UsingRef(it.marker)
        }
    }
    ...
    if (enum != null) {
        ...
        @Suppress("NAME_SHADOWING") val nullable = enum.any { it == null }
    ...
    if (openApiType == null || openApiType is OpenApiType.Any) {
        ...
        openApiType =
            when {
                ...
                anyOfTypes.size == 2 &&
                    anyOfRefs.isEmpty() &&
                    anyOfTypes.containsAll(listOf(OpenApiType.Number, OpenApiType.Integer)) ->
                    OpenApiType.Number
 ...
 *
 * @param schema Schema of the property that the [OpenApiType] belongs to. Used to get extra
 *   information if needed (for arrays / objects / format etc.).
 * @param schemaName Name of the schema that the property belongs to. Used in the name generation of
 *   the additionally produced [Marker]s.
...
private fun OpenApiType.toFieldType(
    ...
    when (this) {
        ...
        // Becomes a DataRow<Any> or DataRow<Any?> since we don't know the type, but we do know it's
        // an object
        is OpenApiType.AnyObject -> FieldTypeResult.FieldType(getType(nullable = nullable))
        ...
        is OpenApiType.Array -> {
            ...
            if (schema.items == null) {
                ...
                val arrayTypeResult = schema.items!!.toOpenApiType(getRefMarker = getRefMarker)

                // convert the type to a FieldType
                when (arrayTypeResult) {
                    is OpenApiTypeResult.CannotFindRefMarker -> FieldTypeResult.CannotFindRefMarker
                        ...
                        when {
                            ...
                            arrayTypeResult.marker is OpenApiMarker.AdditionalPropertyInterface ->
                                FieldTypeResult.FieldType(
                                    ...
                                    additionalPropertyPaths =
                                        arrayTypeResult.marker.additionalPropertyPaths.map {
                    ...
                    is OpenApiTypeResult.OpenApiType -> {
                        ...
                        val arrayTypeSchemaResult =
                            arrayTypeResult.openApiType.toFieldType(
                                schema = schema.items!!,
                                schemaName =
                                    schemaName +
                                        "Content", // type name objects in the array will get
                        ...
                        when (arrayTypeSchemaResult) {
                            ...
                            is FieldTypeResult.FieldType -> {
                                ...
                                val additionalPropertyPaths =
                                    arrayTypeSchemaResult.additionalPropertyPaths.map {
                                        it.prependArrayWithWildcard()
                                    }
                                        ...
                                        when {
                                            ...
                                            fieldType is FieldType.FrameFieldType ->
                                                getTypeAsList(
                                                    nullableArray = nullable,
                                                    typeFqName =
                                                        "${DataFrame::class.qualifiedName}<${fieldType.name}>",
                                                )
        ...
        is OpenApiType.Object -> {
            ...
            when (dataFrameSchemaResult) {
                is MarkerResult.CannotFindRefMarker -> FieldTypeResult.CannotFindRefMarker
                ...
                is MarkerResult.OpenApiMarker -> {
                    ...
                    when (val marker = dataFrameSchemaResult.marker.withName(newName)) {
                        ...
                        // accessed like Marker (or DataRow<Marker>)
                            ...
                            FieldTypeResult.FieldType(
                                fieldType = getType(nullable = nullable, marker = marker),
                                additionalPropertyPaths = marker.additionalPropertyPaths,
                            )
                    }
                }
