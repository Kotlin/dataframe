...
internal sealed class OpenApiType(val name: kotlin.String?) : IsObject {
                ...
                typeFqName =
                    when (format) {
                        OpenApiStringFormat.DATE ->
                            if (nullable) typeOf<LocalDate?>() else typeOf<LocalDate>()
                        OpenApiStringFormat.DATE_TIME ->
                            if (nullable) typeOf<LocalDateTime?>() else typeOf<LocalDateTime>()
                        OpenApiStringFormat.PASSWORD ->
                            if (nullable) typeOf<kotlin.String?>() else typeOf<kotlin.String>()
                        OpenApiStringFormat.BYTE ->
                            if (nullable) typeOf<Byte?>() else typeOf<Byte>()
                        OpenApiStringFormat.BINARY ->
                            if (nullable) typeOf<ByteArray?>() else typeOf<ByteArray>()
        ...
        fun getType(
            ...
            FieldType.ValueFieldType(
                typeFqName =
                    when (format) {
                        null,
                        OpenApiIntegerFormat.INT32 ->
                            if (nullable) typeOf<Int?>() else typeOf<Int>()
                        OpenApiIntegerFormat.INT64 ->
                            if (nullable) typeOf<Long?>() else typeOf<Long>()
                    }.toString()
            )
    }
        ...
        fun getType(
            ...
            FieldType.ValueFieldType(
                typeFqName =
                    when (format) {
                        null,
                        OpenApiNumberFormat.FLOAT ->
                            if (nullable) typeOf<Float?>() else typeOf<Float>()
                        OpenApiNumberFormat.DOUBLE ->
                            if (nullable) typeOf<Double?>() else typeOf<Double>()
                    }.toString()
            )
    }
    ...
    object Object : OpenApiType("object") {
        ...
        fun getType(nullable: kotlin.Boolean, marker: OpenApiMarker): FieldType =
            FieldType.GroupFieldType(
                markerName = marker.name.let { if (nullable) it.toNullable() else it },
                renderAsObject = true,
            )
    }
    ...
    object Array : OpenApiType("array") {
        ...
        fun getTypeAsList(
            ...
            FieldType.ValueFieldType(
                typeFqName =
                    "${List::class.qualifiedName!!}<$typeFqName>${if (nullableArray) "?" else ""}"
            )
        ...
        fun getTypeAsFrameList(
            ...
            FieldType.ValueFieldType(
                typeFqName =
                    "${List::class.qualifiedName!!}<${DataFrame::class.qualifiedName!!}<${
...
internal enum class OpenApiIntegerFormat(val value: String) {
    ...
    companion object {
        fun fromStringOrNull(value: String?): OpenApiIntegerFormat? =
...
internal enum class OpenApiNumberFormat(val value: String) {
    ...
    companion object {
        fun fromStringOrNull(value: String?): OpenApiNumberFormat? =
            values().firstOrNull { it.value == value }
    }
}
...
internal enum class OpenApiStringFormat(val value: String) {
    ...
    companion object {
        fun fromStringOrNull(value: String?): OpenApiStringFormat? =
            values().firstOrNull { it.value == value }
    }
}
