...
/**
 * Used to add `readJson` and `convertToMyMarker` functions to the generated interfaces. Makes sure
 * [convertDataRowsWithOpenApi] is always used in conversions.
 */
internal object DefaultReadOpenApiMethod :
    AbstractDefaultReadMethod(path = null, arguments = MethodArguments.EMPTY, methodName = "") {

    override val additionalImports: List<String> =
        listOf(
    ...
    override fun toDeclaration(marker: Marker, visibility: String): String {
        val returnType =
            DataFrame::class.asClassName().parameterizedBy(ClassName("", listOf(marker.shortName)))

        // convertTo: ConvertSchemaDsl<MyMarker>.() -> Unit = {}
        val convertToParameter =
            ParameterSpec.builder(
                    name = "convertTo",
                    type =
                        LambdaTypeName.get(
                            receiver =
                                ConvertSchemaDsl::class.asClassName()
            ...
            TypeSpec.companionObjectBuilder()
                ...
                .addProperty(
                    PropertySpec.Companion.builder(
                            name = "keyValuePaths",
                            type = typeOf<List<JsonPath>>().asTypeName(),
                        )
                        .getter(
