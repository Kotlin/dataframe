...
internal fun interface GetRefMarker {
    ...
    /**
     * Produces a [MarkerResult] (either [MarkerResult.CannotFindRefMarker] or
     * [MarkerResult.OpenApiMarker]) for the given [refName] representing a query to find a marker
...
/**
 * Either [MarkerResult.CannotFindRefMarker] or [MarkerResult.OpenApiMarker] containing an
 * [org.jetbrains.kotlinx.dataframe.io.OpenApiMarker].
 */
internal sealed interface MarkerResult {
    ...
    data class OpenApiMarker(val marker: org.jetbrains.kotlinx.dataframe.io.OpenApiMarker) :
        MarkerResult
...
/**
 * Represents a call to produce an additional [Marker] from inside a schema component. Not all
 * objects or enums are named, so this is used to create and produce a name for them.
 */
internal fun interface ProduceAdditionalMarker {
     ...
     *
     * @param isTopLevelObject only used in `allOf` cases. If true, the additionally produced marker
     *   is a top-level object that is to be merged with another object.
     * @param marker the marker to produce.
     * @param validName the name of the marker.
     * @return the name of the produced marker. This name is guaranteed to be unique and might not
     *   be the same as the provided [validName].
     */
    operator fun invoke(
...
/**
 * Represents a call to [toMarker] that can be repeated until it returns a
...
internal fun interface RetrievableMarker {
     ...
     * [MarkerResult.OpenApiMarker].
     *
     * @param getRefMarker A function that returns a [Marker] for a given reference name if
     *   successful.
     * @param produceAdditionalMarker A function that produces an additional [Marker] for a given
     *   name. This is used for `object` types not present in the root of `components/schemas`.
...
/**
 * Either a [OpenApiTypeResult.UsingRef], [OpenApiTypeResult.CannotFindRefMarker],
 * [OpenApiTypeResult.OpenApiType], or [OpenApiTypeResult.Enum].
 */
internal sealed interface OpenApiTypeResult {

    /**
     * Property is a reference with name [name] and Marker [marker]. Ref cannot be nullable by
     * OpenAPI spec.
     */
