...
public fun <T, C> FormatClause<T, C>.where(filter: RowValueFilter<T, C>): FormatClause<T, C> = FormatClause(filter = filter, df = df, columns = columns, oldFormatter = oldFormatter)
...
public class FormatClause<T, C>(
    ...
    override fun toString(): String {
        return "FormatClause(df=$df, columns=$columns, oldFormatter=$oldFormatter, filter=$filter)"
    }
}
