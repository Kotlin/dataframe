...
/**
 * Which type of visibility the marker (generated class-like code declaration) will have:
...
public interface IsolatedMarker {
    ...
    /**
     * Type parameters for in front of the function or getter/setter. Like `<T : Int>` in `val <T :
     * Int> MyType<T>.something get() = ...`.
     */
    public val typeParameters: String

    /** Type arguments belonging to this marker. Like `<MyType>` in `MyMarker<MyType>`. */
    public val typeArguments: String
}

public abstract class AbstractMarker(typeParameters: List<String>, typeArguments: List<String>) :
    IsolatedMarker {
...
public open class Marker(
    ...
    public val allSuperMarkers: Map<String, Marker> by lazy {
        ...
        this.superMarkers.forEach { result.putAll(it.value.allSuperMarkers) }
        result
    }
    ...
    public val allFields: List<GeneratedField> by lazy {
        ...
        this.superMarkers.values.forEach {
            it.allFields.forEach { fieldsMap[it.fieldName.quotedIfNeeded] = it }
        }
        fields.forEach { fieldsMap[it.fieldName.quotedIfNeeded] = it }
    ...
    public fun containsColumn(columnName: String): Boolean =
        allFieldsByColumn.containsKey(columnName)

    public val columnNames: List<String>
        get() = allFields.map { it.columnName }
    ...
    public fun implements(schema: Marker): Boolean =
        if (schema.name == name) true
        else allSuperMarkers[schema.name]?.let { it === schema } ?: false
