...
internal fun KType.getFieldKind(): FieldKind =
    FieldKind.of(
        ...
        isListToFrame = {
            jvmErasure == List::class &&
                (arguments[0].type?.jvmErasure?.hasAnnotation<DataSchema>() == true)
        },
...
public sealed interface FieldKind {
    public val shouldBeConvertedToColumnGroup: Boolean
        get() = false

    public val shouldBeConvertedToFrameColumn: Boolean
        get() = false
...
internal object MarkersExtractor {
    ...
    fun get(markerClass: KClass<*>, nullableProperties: Boolean = false): Marker =
        cache.getOrPut(Pair(markerClass, nullableProperties)) {
            ...
            val baseSchemas =
                markerClass.superclasses
                    .filter { it != Any::class }
                    .map { get(it, nullableProperties) }
            Marker(
                name =
                    markerClass.quotedQualifiedNameOrNull()
                        ?: markerClass.simpleName!!.quoteIfNeeded(),
                isOpen = isOpen,
                fields = fields,
    ...
    private fun getFields(
        ...
        val structuralProperties =
            markerClass.memberProperties.filter { !it.hasAnnotation<ScopeProperty>() }
                ...
                val columnSchema =
                    when {
                        fieldKind.shouldBeConvertedToColumnGroup -> {
                            val nestedType =
                                if (clazz == DataRow::class)
                                    type.arguments[0].type ?: typeOf<Any?>()
                                else type
                            val marker =
                                get(
                                    nestedType.jvmErasure,
                                    nullableProperties || type.isMarkedNullable,
                                )
                            fieldType =
                        ...
                        fieldKind.shouldBeConvertedToFrameColumn -> {
                            ...
                            fieldType =
                                FieldType.ValueFieldType(
                                    if (nullableProperties) type.toString().toNullable()
                                    else type.toString()
                                )
                            ColumnSchema.Value(
