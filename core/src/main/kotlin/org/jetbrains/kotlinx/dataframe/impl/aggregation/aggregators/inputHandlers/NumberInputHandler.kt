 ...
 *
 * When calculating the value type, it will try to find the common type in terms of
 * [number unification][UnifyingNumbers]. Preprocessing will handle the conversion of the values in
 * the input to this unified number type.
 */
internal class NumberInputHandler<out Return : Any?> : AggregatorInputHandler<Number, Return> {
     ...
     *
     * - If [ValueType.kType] is a primitive number type and [ValueType.needsFullConversion], then
     *   the values are converted to [ValueType.kType].
     *
     * - If [ValueType.kType] is given as [Number]`(?)`, then we first calculate the unified type
     *   and convert to that. This is heavy and should be avoided if possible.
     *
     * @throws IllegalArgumentException if the input type is not [Number]`(?)` or a primitive number
     *   type.
     */
    override fun preprocessAggregation(
        ...
        return when (valueType.kType.withNullability(false)) {
            // If the type is not a specific number, but rather a mixed Number, we unify the types
            // first.
            ...
            !in primitiveNumberTypes ->
                ...
                val unifiedValues =
                    if (valueType.needsFullConversion) {
                        values.convertToUnifiedNumberType(
                            UnifiedNumberTypeOptions.PRIMITIVES_ONLY,
                            valueType.kType,
                        )
    ...
    /**
     * If the specific [ValueType] of the input is not known, but you still want to call
     * [aggregate], this function can be called to calculate it in terms of
     * [number unification][UnifyingNumbers]
     *
     * @return The (primitive) unified number type of the input values. If no valid unification can
     *   be found or the input is solely [Number]`(?)`, the type [Number]`(?)` is returned.
     * @throws IllegalArgumentException if the input type is not [Number]`(?)` or a primitive number
     *   type.
     */
    override fun calculateValueType(valueTypes: Set<KType>): ValueType {
        val unifiedType =
            valueTypes.unifiedNumberTypeOrNull(UnifiedNumberTypeOptions.PRIMITIVES_ONLY)
        ...
        return unifiedType.toValueType(
            needsFullConversion = valueTypes.singleOrNull() != unifiedType
        )
    }
     ...
     * If the specific [ValueType] of the input is not known, but you still want to call
     * [aggregate], this function can be called to calculate it in terms of
     * [number unification][UnifyingNumbers] by getting the types of [values] at runtime. This is
     * heavy because it uses reflection on each value.
     *
     * @return The (primitive) unified number type of the input values. If no valid unification can
     *   be found or the input is solely [Number]`(?)`, the type [Number]`(?)` is returned.
     * @throws IllegalArgumentException if the input type is not [Number]`(?)` or a primitive number
