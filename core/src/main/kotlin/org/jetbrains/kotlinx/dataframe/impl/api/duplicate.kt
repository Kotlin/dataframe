...
internal fun <T> DataFrame<T>.duplicateRowsImpl(n: Int): DataFrame<T> = columns().map {
    it.duplicateValuesImpl(n)
}.toDataFrame().cast()
...
internal fun <T> DataColumn<T>.duplicateValuesImpl(n: Int): DataColumn<T> {
    ...
    return when (this) {
        is ColumnGroup<*> -> DataColumn.createColumnGroup(
            ...
            if (isFrameColumn()) {
                DataColumn.createFrameColumn(name, list as List<AnyFrame>)
            } else {
                DataColumn.createValueColumn(name, list, type, defaultValue = defaultValue())
            }
        }
    }.cast()
}
...
internal fun <T> DataFrame<T>.duplicateRowsImpl(n: Int, indicesSorted: Iterable<Int>): DataFrame<T> = columns().map {
