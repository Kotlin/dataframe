...
internal fun <T, C> DataFrame<T>.implodeImpl(
            ...
            replace(columns)
                .with { column ->
                    val (value, type) =
                        when (column.kind()) {
                            ColumnKind.Value ->
                                (if (dropNA) column.dropNA() else column).toList() to
                                    getListType(column.type())
                            ColumnKind.Group ->
                                column.asColumnGroup().extractDataFrame() to typeOf<AnyFrame>()
                            ColumnKind.Frame ->
                                column.asAnyFrameColumn().concat() to typeOf<List<AnyFrame>>()
                        }
                    var first = true
                    column.map(type) {
                        if (first) {
