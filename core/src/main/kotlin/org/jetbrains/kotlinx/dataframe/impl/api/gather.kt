...
internal fun <T, C, K, R> Gather<T, C, K, R>.gatherImpl(
    ...
    if (filter == null) {
                        ...
                        if (valueTransform != null) {
                            when {
                                explode && value is List<*> ->
                                    (value as List<C>).map(valueTransform)
        ...
        df =
            when {
                keysColumn != null && valuesColumn != null ->
                    df.explode { keysColumn and valuesColumn }
        ...
        when {
            ...
            keysColumn != null -> {
                df =
                    df.replace { nameAndValuePairs }
                        .with { it.map { it.first } named keysColumn.name() }
            }

            valuesColumn != null -> {
                df =
                    df.replace { nameAndValuePairs }
                        .with { it.map { it.second } named valuesColumn.name() }
            }
        }
    }
