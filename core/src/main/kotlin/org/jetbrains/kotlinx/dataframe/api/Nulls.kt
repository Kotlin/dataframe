 ...
 *   select columns} {@set [Update.Columns.SELECTING_COLUMNS] [Selecting
 *   Columns][FillNulls.FillNullsSelectingOptions]}
 *
 ...
 * @include [Update.Columns] {@comment Description of what this function expects the user to do:
 *   select columns} {@set [Update.Columns.SELECTING_COLUMNS] [Selecting
 *   Columns][FillNaNs.FillNaNsSelectingOptions]}
 *
 ...
 * @include [Update.Columns] {@comment Description of what this function expects the user to do:
 *   select columns} {@set [Update.Columns.SELECTING_COLUMNS] [Selecting
 *   Columns][FillNA.FillNASelectingOptions]}
 *
 ...
 *   [DataFrame] to drop rows in.
 */
@ExcludeFromSources
private typealias DropDslParam = Nothing

/**
 * @param columns The [KProperties][KProperty] used to select the columns of this [DataFrame] to
 *   drop rows in.
 */
@ExcludeFromSources
...
/**
 * @param columns The [Strings][String] corresponding to the names of columns in this [DataFrame] to
 *   drop rows in.
 */
@ExcludeFromSources
...
/**
 * @param columns The [Column References][ColumnReference] used to select the columns of this
 *   [DataFrame] to drop rows in.
 */
@ExcludeFromSources
 ...
 * [Selecting Columns][DropNullsSelectingOptions]).
 *
 ...
 * @include [SelectingColumns.ColumnsSelectionDsl.ColumnsSelectionDslWithExample] {@include
 *   [SetDropNullsOperationArg]} `df.`[dropNulls][dropNulls]`(whereAllNull = true) {
 *   `[colsOf][colsOf]`<`[Double][Double]`>() }`
...
/**
 * @include [CommonDropNullsFunctionDoc] This overload operates on all columns in the [DataFrame].
 * @include [DropNulls.WhereAllNullParam]
 */
@Refine
 ...
 *
 * Optionally, you can select which columns to operate on (see
 * [Selecting Columns][DropNASelectingOptions]). Also, you can supply `whereAllNA = true` to only
 * drop rows where all selected cells are [`NA`][NA]. By default, rows are dropped if any of the
 * selected cells are [`NA`][NA].
 *
...
internal interface DropNA {
     ...
     *   [`NA`][NA]. If `false`, rows are dropped if any of the selected cells is [`NA`][NA].
     */
 ...
 * @include [SelectingColumns.ColumnsSelectionDsl.ColumnsSelectionDslWithExample] {@include
 *   [SetDropNAOperationArg]} `df.`[dropNA][dropNA]`(whereAllNA = true) {
 *   `[colsOf][colsOf]`<`[Double][Double]`>() }`
 ...
 * @include [CommonDropNAFunctionDoc]
 * @include [SelectingColumns.ColumnNamesApi.ColumnNamesApiWithExample] {@include
 *   [SetDropNAOperationArg]} `df.`[dropNA][dropNA]`("length", whereAllNA = true)`
 * @include [DropNA.WhereAllNAParam]
 * @include [DropColumnNamesParam]
 */
public fun <T> DataFrame<T>.dropNA(
...
/**
 * @include [CommonDropNAFunctionDoc] This overload operates on all columns in the [DataFrame].
 * @include [DropNA.WhereAllNAParam]
 */
@Refine
 ...
 *
 * Optionally, you can select which columns to operate on (see
 * [Selecting Columns][DropNaNsSelectingOptions]). Also, you can supply `whereAllNaN = true` to only
 * drop rows where all selected cells are [`NaN`][Double.isNaN]. By default, rows are dropped if any
 * of the selected cells are [`NaN`][Double.isNaN].
 *
 ...
 * @include [SelectingColumns.ColumnsSelectionDsl.ColumnsSelectionDslWithExample] {@include
 *   [SetDropNaNsOperationArg]} `df.`[dropNaNs][dropNaNs]`(whereAllNaN = true) {
 *   `[colsOf][colsOf]`<`[Double][Double]`>() }`
...
/**
 * @include [CommonDropNaNsFunctionDoc] This overload operates on all columns in the [DataFrame].
 * @include [DropNaNs.WhereAllNaNParam]
 */
