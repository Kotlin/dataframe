...
internal data class PivotImpl<T>(val df: DataFrame<T>, val columns: PivotColumnsSelector<T, *>, val inward: Boolean?) :
    Pivot<T>,
    AggregatableInternal<T> {
    fun toGroupedPivot(
        moveToTop: Boolean,
        columns: ColumnsSelector<T, *>,
    ): PivotGroupBy<T> = PivotGroupByImpl(df.groupBy(moveToTop, columns), this.columns, inward)
