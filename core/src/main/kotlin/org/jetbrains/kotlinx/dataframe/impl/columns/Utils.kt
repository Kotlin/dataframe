...
internal fun <T> ColumnWithPath<T>.changePath(path: ColumnPath): ColumnWithPath<T> =
    data.addPath(path)
...
@PublishedApi internal fun AnyCol.asAnyFrameColumn(): FrameColumn<*> = this as FrameColumn<*>

internal fun <T> AnyCol.grouped() = this as ColumnGroup<T>

internal fun <T> ColumnGroup<*>.withDf(newDf: DataFrame<T>) =
    DataColumn.createColumnGroup(name, newDf)
...
/**
 * Applies a transformation on [this] [SingleColumn] by converting its single [ColumnWithPath]<[A]>
 * to [List]<[ColumnWithPath]<[B]>] using [converter]. Since [converter] allows you to return
 * multiple columns, the result is turned into a [ColumnSet]<[B]>.
 */
internal fun <A, B> SingleColumn<A>.transformSingle(
    ...
    object : ColumnSet<B> {
        override fun resolve(context: ColumnResolutionContext): List<ColumnWithPath<B>> =
            this@transformSingle.resolveSingle(context)?.let(converter) ?: emptyList()
    }

/**
 * Applies a transformation on [this] by converting its [List]<[ColumnWithPath]<[A]>] to
 * [List]<[ColumnWithPath]<[B]>] using [converter].
 *
 * The result can either be used as a normal [ColumnSet]<[B]>, which resolves [this] and then
 * applies [converter] on the result,
 *
 * or it can be used as a [TransformableColumnSet]<[B]>, where a [ColumnsResolverTransformer] can be
 * injected before the [converter] is applied.
 */
@PublishedApi
internal inline fun <A, B> ColumnsResolver<A>.transform(
    ...
    object : TransformableColumnSet<B> {
        override fun resolve(context: ColumnResolutionContext) =
            this@transform.resolve(context).let(converter)
        ...
        override fun transformResolve(
            ...
            transformer.transform(this@transform).resolve(context).let {
                converter(it as List<ColumnWithPath<A>>)
            }
    }

/**
 * Applies a transformation on [this] [SingleColumn] by converting its single [ColumnWithPath]<[A]>
 * to [List]<[ColumnWithPath]<[B]>] using [converter] but also providing the
 * [ColumnResolutionContext] to the converter.
 *
 * Since [converter] allows you to return multiple columns, the result is turned into a
 * [ColumnSet]<[B]>.
 *
 * The result can either be used as a normal [ColumnSet]<[B]>, which resolves [this] and then
 * applies [converter] on the result,
...
internal fun <A, B> SingleColumn<A>.transformSingleWithContext(
    ...
    object : ColumnSet<B> {
        ...
        override fun resolve(context: ColumnResolutionContext): List<ColumnWithPath<B>> =
            this@transformSingleWithContext.resolveSingle(context)?.let { converter(context, it) }
                ?: emptyList()
    }

/**
 * Applies a transformation on [this] by converting its [List]<[ColumnWithPath]<[A]>] to
 * [List]<[ColumnWithPath]<[B]>] using [converter], but also providing the [ColumnResolutionContext]
 * to the converter.
 *
 * The result can either be used as a normal [ColumnSet]<[B]>, which resolves [this] and then
 * applies [converter] on the result,
...
internal fun <A, B> ColumnsResolver<A>.transformWithContext(
    ...
    object : TransformableColumnSet<B> {
        override fun resolve(context: ColumnResolutionContext) =
            this@transformWithContext.resolve(context).let { converter(context, it) }

        override fun transformResolve(
            context: ColumnResolutionContext,
            transformer: ColumnsResolverTransformer,
        ): List<ColumnWithPath<B>> =
            transformer.transform(this@transformWithContext).resolve(context).let {
                converter(context, it as List<ColumnWithPath<A>>)
            }
    }
...
/**
 * Converts [this] [ColumnsResolver] to a [SingleColumn]. [resolveSingle] will return the single
 * column of [this] if there is only one, else it will return throw an exception: In case of an
 * empty [ColumnsResolver], a [NoSuchElementException] will be thrown. In case of more than one
 * column, a [IllegalArgumentException] will be thrown. If the result is used as a [ColumnSet],
 * `null` will be converted to an empty list.
 */
...
/**
 * Same as [singleImpl], however, it passes any [ColumnsResolverTransformer] back to [this] if it is
 * supplied.
 */
internal fun <T> TransformableColumnSet<T>.singleWithTransformerImpl():
    TransformableSingleColumn<T> =
...
/**
 * Converts [this] [ColumnsResolver] to a [SingleColumn]. [resolveSingle] will return the single
 * column of [this] if there is only one, else it will return `null`. If the result used as a
 * [ColumnSet], `null` will be converted to an empty list.
 */
...
/**
 * Same as [singleOrNullImpl], however, it passes any [ColumnsResolverTransformer] back to [this] if
 * it is supplied.
 */
internal fun <T> TransformableColumnSet<T>.singleOrNullWithTransformerImpl():
    TransformableSingleColumn<T> =
    object : TransformableSingleColumn<T> {
        override fun resolveSingle(context: ColumnResolutionContext): ColumnWithPath<T>? =
...
internal fun <T> ColumnSet<T>.getAtOrNull(index: Int): SingleColumn<T> =
    object : SingleColumn<T> {
        override fun resolveSingle(context: ColumnResolutionContext): ColumnWithPath<T>? =
            this@getAtOrNull.resolve(context).getOrNull(index)
    }

internal fun ColumnSet<*>.getChildrenAt(index: Int): ColumnSet<*> = transform {
    it.mapNotNull { it.getCol(index) }
}

internal fun <C> ColumnsContainer<*>.getColumn(name: String, policy: UnresolvedColumnsPolicy) =
    getColumnOrNull(name)?.cast()
        ?: when (policy) {
            UnresolvedColumnsPolicy.Fail -> error("Column not found: $name")
...
internal fun <C> ColumnsContainer<*>.getColumn(path: ColumnPath, policy: UnresolvedColumnsPolicy) =
    getColumnOrNull(path)?.cast()
        ?: when (policy) {
            UnresolvedColumnsPolicy.Fail -> error("Column not found: '${path.joinToString()}'")
 ...
 *
 * Note: this doesn't merge similarly called groups into one, nor does it insert columns into groups
 * if they weren't so already.
 */
...
/**
 * Returns a new list of column paths, except the ones inside [columns]. NOTE: The structure is not
 * kept the same; if a column is removed, its parent will be removed as well, and all its siblings
 * will be lifted out of the group. This also happens if a column is "removed" that does not exist
 * in [this].
 */
...
/**
 * Returns a new list of column paths, except the ones inside [columns]. NOTE: ColumnGroups are
 * adapted to keep their structure. If a column inside a column group is excepted, it will be
 * removed from the group. Empty groups will be removed if [removeEmptyGroups]` == true`
 */
    ...
    return flatMap {
        ...
        for (columnToExcept in columns.sortedByDescending { it.path.size }) {
            ...
            if (nodeToExcept != null) {
                // remove the children from the node (if it's a column group) and remove its data
                // (the column itself)
                nodeToExcept.allChildren().forEach { it.data = null }
                nodeToExcept.data = null

                // we need to update the data of the parent node(s) to reflect the removal of the
                // column
                if (nodeToExcept.parent != null) {
                    ...
                    // now we update the parent's parents recursively with new column group
                    // instances
 ...
 *
 * NOTE: nullable DataFrames cannot become a [FrameColumns][FrameColumn], so they become
 * [ValueColumns][ValueColumn] instead.
 */
...
internal fun <T> BaseColumn<T>.addParent(parent: ColumnGroup<*>) =
    (this as DataColumnInternal<T>).addParent(parent)
