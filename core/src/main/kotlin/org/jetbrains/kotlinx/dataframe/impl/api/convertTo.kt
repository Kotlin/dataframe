                        ...
                        when (targetSchema.kind) {
                            ...
                            ColumnKind.Group -> {
                                val column = when {
                                    ...
                                    originalColumn.kind == ColumnKind.Value && originalColumn.all {
                                        it is DataRow<*>
                                    } ->
                                ...
                                require(column.kind == ColumnKind.Group) {
                                    "Column `${column.name}` is ${column.kind} and can not be converted to " +
                                        "`ColumnGroup`"
                                }
                                ...
                                val frames = when (column.kind) {
                                    ...
                                    ColumnKind.Value -> {
                                        require(
                                            ...
                                            "Column `${column.name}` is ValueColumn and contains objects " +
                                                "that can not be converted into `DataFrame`"
                                        }
                                        column.values().map {
                                            when (it) {
                                                null -> emptyDataFrame()
