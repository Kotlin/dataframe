...
internal object Parsers : GlobalParserOptions {
    ...
    private val parsersOrder = listOf(
        ...
        stringParser<AnyFrame>(catch = true) { if (it.startsWith("[")) DataFrame.readJsonStr(it) else null },
        // JSON object as DataRow<*>
        stringParser<AnyRow>(catch = true) { if (it.startsWith("{")) DataFrame.readJsonStr(it).single() else null },
