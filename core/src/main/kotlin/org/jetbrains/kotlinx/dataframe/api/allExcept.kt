...
public interface AllExceptColumnsSelectionDsl {
     ...
     *   [DslGrammarTemplate.ColumnsSelectorDef]} {@include [LineBreak]} {@include
     *   [DslGrammarTemplate.ColumnDef]} {@include [LineBreak]} {@include
     *   [DslGrammarTemplate.ColumnNoAccessorDef]} {@include [LineBreak]} {@include
     *   [DslGrammarTemplate.ColumnsResolverDef]} }
     *
     * {@set [DslGrammarTemplate.PLAIN_DSL_FUNCTIONS] {@include [PlainDslName]}**` { `**{@include
     * [DslGrammarTemplate.ColumnsSelectorRef]}**` \}`**
     *
     ...
     * [ColumnSetName]}` [`**` { `**`] `{@include [DslGrammarTemplate.ColumnsResolverRef]}` [`**` \}
     * `**`]`
     *
     * {@include [Indent]}`| `{@include [ColumnSetName]}` `{@include [DslGrammarTemplate.ColumnRef]}
     *
     * {@include [Indent]}`| `**`.`**{@include [ColumnSetName]}**`(`**{@include
     * [DslGrammarTemplate.ColumnRef]}**`,`**` ..`**`)`** } {@set
     * [DslGrammarTemplate.COLUMN_GROUP_FUNCTIONS] {@include [Indent]}{@include
     * [ColumnGroupName]}**` { `**{@include [DslGrammarTemplate.ColumnsSelectorRef]}**` \} `**
     *
     * {@include [Indent]}`| `{@include [ColumnGroupName]}**`(`**{@include
     * [DslGrammarTemplate.ColumnNoAccessorRef]}**`,`**` ..`**`)`**
     *
     * {@include [Indent]}`| `{@include [ColumnGroupExceptName]}**` { `**{@include
     * [DslGrammarTemplate.ColumnsSelectorRef]}**` \} `**
     *
     * {@include [Indent]}`| `{@include [ColumnGroupExceptName]}**`(`**{@include
     * [DslGrammarTemplate.ColumnNoAccessorRef]}**`,`**` ..`**`)`** }
     ...
     * ## (All) (Cols) Except
     *
     * Exclude a selection of columns from the current selection using the {@include
     * [ColumnsSelectionDslLink]}.
     *
     * ### Check out: [Grammar]
     *
     * ### On [ColumnSets][ColumnSet]
     * This function can be explained the easiest with [ColumnSets][ColumnSet]. Let's say we want
     * all [Int] columns apart from `age` and `height`.
     *
     * We can do:
     *
     * `df.`[select][DataFrame.select]` { `[colsOf][colsOf]`<`[Int][Int]`>()
     * `[except][ColumnSet.except]` (age `[and][ColumnsSelectionDsl.and]` height) }`
     *
     * which will 'subtract' the [ColumnSet] created by `age `[and][ColumnsSelectionDsl.and]`
     * height` from the [ColumnSet] created by [colsOf][colsOf]`<`[Int][Int]`>()`.
     *
     * {@include [LineBreak]} This operation can also be used to exclude columns from
     * [Column Groups][ColumnGroup].
     *
     * For instance:
     *
     * `df.`[select][DataFrame.select]` { `[colsAtAnyDepth][ColumnsSelectionDsl.colsAtAnyDepth]` {
     * "a" `[in][String.contains]` it.`[name][DataColumn.name]`() } `[except][ColumnSet.except]`
     * userData.age }` {@include [LineBreak]}
     *
     * Note that the selection of columns to exclude from [column sets][ColumnSet] is always done
     * relative to the outer scope. Use the {@include [ExtensionPropertiesApiLink]} to prevent
     * scoping issues if possible. {@include [LineBreak]} Special case: If a column that needs to be
     * removed appears multiple times in the [ColumnSet], it is excepted each time it is encountered
     * (including inside [ColumnGroups][ColumnGroup]). You could say the receiver [ColumnSet] is
     * [simplified][ColumnsSelectionDsl.simplify] before the operation is performed:
     *
     ...
     * Instead of having to write [all][ColumnsSelectionDsl.all]`()
     * `[except][ColumnsSelectionDsl.except]` { ... }` in the DSL, you can use
     * [allExcept][ColumnsSelectionDsl.allExcept]` { ... }` to achieve the same result.
     *
     * For example:
     *
     * `df.`[select][DataFrame.select]` { `[allExcept][ColumnsSelectionDsl.allExcept]` {
     * userData.age `[and][ColumnsSelectionDsl.and]` height } }`
     *
     * ### On [ColumnGroups][ColumnGroup]: All Cols Except
     * The variant of this function on [ColumnGroups][ColumnGroup] is a bit different as it changes
     * the scope relative to the column group. {@include [LineBreak]} In other words:
     *
     * `df.`[select][DataFrame.select]` { myColGroup.`[allColsExcept][SingleColumn.allColsExcept]` {
     * colA `[and][ColumnsSelectionDsl.and]` colB } }`
     *
     * is shorthand for
     *
     * `df.`[select][DataFrame.select]` { myColGroup.`[select][ColumnsSelectionDsl.select]` {
     * `[all][ColumnsSelectionDsl.all]`() `[except][ColumnSet.except]` { colA
     * `[and][ColumnsSelectionDsl.and]` colB } } }`
     *
     * or
     *
     * `df.`[select][DataFrame.select]` { myColGroup.`[allCols][ColumnsSelectionDsl.allCols]`()
     * `[except][ColumnSet.except]` { myColGroup.colA `[and][ColumnsSelectionDsl.and]`
     * myColGroup.colB } }` {@include [LineBreak]} Also note the name change, similar to
     * [allCols][ColumnsSelectionDsl.allCols], this makes it clearer that you're selecting columns
     * inside the group, 'lifting' them out.
     *
     * ### On [ColumnGroups][ColumnGroup]: Except
     * This variant can be used to exclude some nested columns from a [ColumnGroup] in the
     * selection. In contrast to [allColsExcept][ColumnsSelectionDsl.allColsExcept], this function
     * does not 'lift' the columns out of the group, preserving the structure.
     *
     * So:
     *
     ...
     * is shorthand for:
     *
     * `df.`[select][DataFrame.select]` { `[cols][ColumnsSelectionDsl.cols]`(colGroup)
     * `[except][ColumnSet.except]` colGroup.col }`
     *
    ...
    /**
     * @include [CommonExceptDocs] {@set [CommonExceptDocs.EXAMPLE]
     *   `df.`[select][ColumnsSelectionDsl.select]` {
     *   `[colsOf][ColumnsSelectionDsl.colsOf]`<`[Number][Number]`>() `[except][ColumnSet.except]`
     *   {@get [ARGUMENT_1]} \}`
     *
     * `df.`[select][ColumnsSelectionDsl.select]` { `[cols][ColumnsSelectionDsl.cols]`(name, age)
     * `[except][ColumnSet.except]` {@get [ARGUMENT_2]} \}` }
     */
    @ExcludeFromSources
     ...
     * @include [CommonExceptDocs] {@set [CommonExceptDocs.EXAMPLE]
     *   `df.`[select][ColumnsSelectionDsl.select]` {
     *   `[colsOf][ColumnsSelectionDsl.colsOf]`<`[Number][Number]`>().`[except][ColumnSet.except]`{@get
     *   [ARGUMENT_1]} \}`
     *
     * `df.`[select][ColumnsSelectionDsl.select]` { `[cols][ColumnsSelectionDsl.cols]`(name,
     * age).`[except][ColumnSet.except]`{@get [ARGUMENT_2]} \}` }
     */
    @ExcludeFromSources
     ...
     * @set [CommonExceptDocs.PARAM] @param [selector\] A lambda in which you specify the columns
     *   that need to be excluded from the [ColumnSet]. The scope of the selector is the same as the
     *   outer scope.
    ...
    /**
     * @include [ColumnSetInfixDocs]
     * @set [CommonExceptDocs.PARAM] @param [other\] A [ColumnsResolver] containing the columns that
     ...
     * @set [CommonExceptDocs.PARAM] @param [others\] Any number of
     *   [ColumnsResolvers][ColumnsResolver] containing the columns that need to be excluded from
     *   the [ColumnSet].
    ...
    /**
     * @include [ColumnSetInfixDocs]
     * @set [CommonExceptDocs.PARAM] @param [other\] A [String] referring to the column (relative to
     *   the current scope) that needs to be excluded from the [ColumnSet].
    ...
    /**
     * @include [ColumnSetInfixDocs]
     * @set [CommonExceptDocs.PARAM] @param [other\] A [KProperty] referring to the column (relative
     *   to the current scope) that needs to be excluded from the [ColumnSet].
    ...
    /**
     * @include [ColumnSetVarargDocs]
     * @set [CommonExceptDocs.PARAM] @param [others\] Any number of [KProperties][KProperty]
     *   referring to the columns (relative to the current scope) that need to be excluded from the
     *   [ColumnSet].
    ...
    /**
     * @include [ColumnSetInfixDocs]
     * @set [CommonExceptDocs.PARAM] @param [other\] A [ColumnPath] referring to the column
     *   (relative to the current scope) that needs to be excluded from the [ColumnSet].
    ...
    /**
     * @include [ColumnSetVarargDocs]
     * @set [CommonExceptDocs.PARAM] @param [others\] Any number of [ColumnPaths][ColumnPath]
     *   referring to the columns (relative to the current scope) that need to be excluded from the
     *   [ColumnSet].
     ...
     * @set [CommonExceptDocs.EXAMPLE] `df.`[select][ColumnsSelectionDsl.select]` {
     *   `[allExcept][ColumnsSelectionDsl.allExcept]{@get [ARGUMENT_1]}` \}`
     *
     *   `df.`[select][ColumnsSelectionDsl.select]` {
     *   `[allExcept][ColumnsSelectionDsl.allExcept]{@get [ARGUMENT_2]}` \}`
     */
    @ExcludeFromSources
     ...
     * @set [CommonExceptDocs.PARAM] @param [selector\] A lambda in which you specify the columns
     *   that need to be excluded from the current selection. The scope of the selector is the same
     *   as the outer scope.
     ...
     *
     * @include [ColumnsSelectionDslDocs]
     * @set [CommonExceptDocs.PARAM] @param [others\] A [ColumnsResolver] containing the columns
     *   that need to be excluded from the current selection.
     ...
     * @include [ColumnsSelectionDslDocs]
     * @set [CommonExceptDocs.PARAM] @param [others\] Any number of [Strings][String] referring to
     *   the columns (relative to the current scope) that need to be excluded from the current
     *   selection.
     * @set [ColumnsSelectionDslDocs.ARGUMENT_1] `("age", "height")`
     ...
     *   `df.`[select][ColumnsSelectionDsl.select]` { `<code>{@get
     *   [RECEIVER_1]}</code>[allColsExcept][{@get [RECEIVER_TYPE]}.allColsExcept]<code>{@get
     *   [ARGUMENT_1]}</code>` \}`
     *
     *   `df.`[select][ColumnsSelectionDsl.select]` { city `[and][ColumnsSelectionDsl.and]`
     *   `<code>{@get [RECEIVER_2]}</code>[allColsExcept][{@get
     *   [RECEIVER_TYPE]}.allColsExcept]<code>{@get [ARGUMENT_2]}</code>` \}`
     */
    @Suppress("ClassName")
    @ExcludeFromSources
    private interface ColumnGroupDocs {
        ...
        /**
         * @set [CommonExceptDocs.PARAM] @param [selector\] A lambda in which you specify the
         *   columns that need to be excluded from the current selection in [this\] column group.
         *   The other columns will be included in the selection by default. The scope of the
         *   selector is relative to the column group.
        ...
        /**
         * @set [CommonExceptDocs.PARAM] @param [others\] Any number of [Strings][String] referring
         *   to the columns (relative to the column group) that need to be excluded from the current
         *   selection in [this\] column group. The other columns will be included in the selection
         *   by default.
        ...
        /**
         * @set [CommonExceptDocs.PARAM] @param [others\] Any number of [KProperties][KProperty]
         *   referring to the columns (relative to the column group) that need to be excluded from
         *   the current selection in [this\] column group. The other columns will be included in
     ...
     * @set [CommonExceptDocs.EXAMPLE] {@comment <code> blocks are there to prevent double ``}
     *   `df.`[select][ColumnsSelectionDsl.select]` { `<code>{@get
     *   [RECEIVER_1]}</code>[except][{@get [RECEIVER_TYPE]}.except]<code>{@get
     *   [ARGUMENT_1]}</code>` \}`
     *
     *   `df.`[select][ColumnsSelectionDsl.select]` { city `[and][ColumnsSelectionDsl.and]`
     *   `<code>{@get [RECEIVER_2]}</code>[except][{@get [RECEIVER_TYPE]}.except]<code>{@get
     *   [ARGUMENT_2]}</code>` \}`
     */
    @ExcludeFromSources private interface ColumnGroupExceptDocs : ColumnGroupDocs
