...
@PublishedApi
internal fun compileTimeSchemaImpl(
    runtimeSchema: DataFrameSchema?,
    klass: KClass<*>,
): DataFrameSchema {
    val compileSchema = getSchema(klass)
    if (runtimeSchema == null) return compileSchema
    val root = ColumnPath(emptyList())
    val order = buildMap { putColumnsOrder(runtimeSchema, path = root) }
    return compileSchema.sortedBy(order, path = root)
}

internal fun MutableMap<ColumnPath, Int>.putColumnsOrder(
...
internal fun DataFrameSchema.sortedBy(
    order: Map<ColumnPath, Int>,
    path: ColumnPath,
): DataFrameSchema {
    val sorted =
        columns
            ...
            .sortedBy { (name, _) -> order[path + name] }
            .toMap()
