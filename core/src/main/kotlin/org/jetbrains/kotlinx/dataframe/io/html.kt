...
internal val formatter =
    DataFrameFormatter("formatted", "null", "structural", "numbers", "dataFrameCaption")

internal fun getResources(vararg resource: String) =
    resource.joinToString(separator = "\n") { getResourceText(it) }

internal fun getResourceText(resource: String, vararg replacement: Pair<String, Any>): String {
    /**
     * The choice of loader is crucial here: it should always be a class loaded by the same class
     * loader as the resource we load. I.e. [DataFrame] isn't a good fit because it might be loaded
     * by Kotlin IDEA plugin (because Kotlin plugin loads DataFrame compiler plugin), and plugin's
     * classloader knows nothing about the resources.
     */
    val loader = HtmlContent::class.java
    val res =
        loader.getResourceAsStream(resource)
            ?: error(
                "Resource '$resource' not found. Load was attempted by $loader, loaded by ${loader.classLoader}"
            )
    var template = InputStreamReader(res).readText()
    replacement.forEach { template = template.replace(it.first, it.second.toString()) }
    return template
}
...
internal fun tableJs(columns: List<ColumnDataForJs>, id: Int, rootId: Int, nrow: Int): String {
    ...
    val data = buildString {
        ...
        fun appendColWithChildren(col: ColumnDataForJs): Int {
            ...
            val colIndex = index++
            val values =
                col.values.joinToString(",", prefix = "[", postfix = "]") {
                    when (it) {
                        is HtmlContent -> {
            ...
            append(
                "{ name: \"$colName\", children: $children, rightAlign: ${col.rightAlign}, values: $values }, \n"
            )
...
internal fun AnyFrame.toHtmlData(
    ...
    fun AnyFrame.columnToJs(
        ...
        val contents = values.map { row ->
            ...
            if (dfLikeContent != null) {
                ...
                if (df.isEmpty()) {
                    ...
                    queue +=
                        RenderingQueueItem(
                            df,
                            id,
                            dfLikeContent.configuration(defaultConfiguration),
                        )
                ...
                val formatter = renderConfig.cellFormatter ?: return@map HtmlContent(html, null)

                // ask formatter for all attributes defined for this cell or any of its parents
                // (outer column groups)
                val parentCols =
                    col.path.indices
                ...
                val style =
                    (parentAttributes and cellAttributes)
                        ?.attributes()
                        ?.ifEmpty { null }
                        ?.flatMap {
                            if (it.first == "color") {
                        ...
                        ?.toMap() // removing duplicate keys, allowing only the final one to be
                                  // applied
                        ?.entries
            ...
            if (hf == null) {
                ...
                val parentCols =
                    col.path.indices
                        ...
                        .map { ColumnWithPath(renderRootDf[it], it) }
                val parentAttributes =
                    parentCols.map { hf(FormattingDsl, it) }.reduceOrNull(CellAttributes?::and)
    ...
    while (!queue.isEmpty()) {
        ...
        val preparedColumns =
            nextDf.columns().map { nextDf.columnToJs(it.addPath(), rowsLimit, configuration) }
    ...
    when (this) {
        is AnyFrame -> {
            object : DataFrameLike {
                override fun configuration(default: DisplayConfiguration) =
                    default.copy(cellFormatter = null)
...
private data class RenderingQueueItem(
    val df: DataFrame<*>,
    val id: Int,
 ...
 * It is essential to prevent potential memory problems when serializing HTML data for display in
 ...
 * @return The downsized BufferedImage if value is a BufferedImage and downsizing is enabled in the
 *   DisplayConfiguration, otherwise returns the input value unchanged.
 */
...
/**
 * Renders [this] [DataFrame] as static HTML (meaning no JS is used). CSS rendering is enabled by
 * default but can be turned off using [includeCss]
 *
 * __IMPORTANT:__ If JavaScript is enabled, the table inside the returned [DataFrameHtmlData] will
 * be HIDDEN when rendered. This is done so this function can be used as a fallback mechanism for
 * [AnyFrame.toHtmlData], which requires JavaScript to be displayed. Call `.copy(script = "")` on
 * the returned [DataFrameHtmlData] to remove this hiding/fallback mechanism.
 *
 * @param configuration optional configuration for rendering
 * @param cellRenderer optional cell renderer for rendering
 * @param includeCss whether to include CSS in the output. This is `true` by default but it can be
 *   set to `false` to emulate what happens in environments where custom CSS is not allowed.
 * @param openNestedDfs whether to open nested dataframes. This is `false` by default byt ut can be
 *   set to `true` to emulate what happens in environments where `<details>` tags are not supported.
 */
public fun AnyFrame.toStaticHtml(
    ...
    fun StringBuilder.emitTag(
        tag: String,
        attributes: String = "",
        tagContents: StringBuilder.() -> Unit,
    ) {
        append("<$tag")
            ...
            for (row in colGrid) {
                emitTag("tr") {
                    for ((j, col) in row.withIndex()) {
                        ...
                        // check if the next cell has a left border, and if so, add a right border
                        // to this cell
    ...
    // Adds a single cell to the html. DataRows from column groups already need to be split up into
    // separate cells.
            ...
            when (col) {
                ...
                is FrameColumn<*> -> {
                    ...
                    emitTag("details", if (openNestedDfs) "open" else "") {
                        emitTag("summary") { append("DataFrame [${cellValue.size}]") }
                        ...
                        // CSS will not be included here, as it is already included in the main
                        // table
                        cellValue
                            .toStaticHtml(
                                configuration =
                ...
                else -> append(cellRenderer.content(cellValue, configuration).truncatedContent)
            }
        }

    // Adds a single row to the html. This row uses the flattened columns to get them displayed
    // non-collapsed.
    fun StringBuilder.emitRow(row: AnyRow) =
        emitTag("tr") {
            for ((i, col) in flattenedCols.withIndex()) {
                val border = borders[i].toMutableSet()
                // check if the next cell has a left border, and if so, add a right border to this
                // cell
...
/**
 * Wrapper class which contains a nullable [ColumnWithPath] and a set of [Border]s. (Empty cells can
 * have borders too)
 */
...
/**
 * Given a [DataFrame], this function returns a depth-first "matrix" containing all columns laid out
 * in such a way that they can be used to render the header of a table. The
        ...
        for ((i, child) in cols().withIndex()) {
            ...
            if (borders.isNotEmpty()) {
                ...
                for (j in (depth - 1).coerceAtLeast(0)..maxDepth) {
                    matrix[j][breadth] =
                        matrix[j][breadth].let { it.copy(borders = it.borders + borders) }
                }
            }
 ...
 *
 * To change the formatting of certain cells or columns in the dataframe, use [DataFrame.format].
 *
 * Use [toHtml] if you don't need the [DataFrameHtmlData] to include CSS- and script definitions.
 *
 * The [DataFrameHtmlData] can be saved as an *.html file and displayed in the browser. If you save
 * it as a file and find it in the project tree, the
 ...
 *
 * By default, cell content is formatted as text Use [RenderedContent.media] or [IMG], [IFRAME] if
 * you need custom HTML inside a cell.
 *
 * __NOTE:__ In Kotlin Notebook, output [DataFrame] directly, or use [toHtml], as that environment
 * already has CSS- and script definitions for DataFrame. Using [toStandaloneHtml] might produce
 * unexpected results.
 *
 * @param [configuration] The [DisplayConfiguration] to use. Default:
...
/**
 * Returns a [DataFrameHtmlData] without additional definitions. Can be rendered in Jupyter kernel
 * (Kotlin Notebook) environments or other environments that already have CSS- and script
 ...
 *
 * It can be used to compose rendered dataframe tables with additional HTML elements, or to simply
 * print the HTML or write it to file.
 */
public class DataFrameHtmlData(
    ...
    /**
     * Opens a new tab in your default browser. Consider [writeHtml] with the
     * [HTML file auto-reload](https://www.jetbrains.com/help/idea/editing-html-files.html#ws_html_preview_output_procedure)
     * feature of IntelliJ IDEA if you want to experiment with the output and run program multiple
     * times
     */
    ...
    public companion object {
        /**
         * @return CSS and JS required to render DataFrame tables Can be used as a starting point to
         *   create page with multiple tables
...
/**
 * A collection of settings for rendering dataframes as HTML tables or native Kotlin Notebook table
 * output.
 *
 * @param rowsLimit null to disable rows limit
 * @param cellContentLimit -1 to disable content trimming
 * @param enableFallbackStaticTables true to add additional pure HTML table that will be visible
 *   only if JS is disabled; For example hosting *.ipynb files with outputs on GitHub
 * @param cellFormatter Optional cell formatter applied to data cells during HTML rendering.
 * @param headerFormatter Optional header formatter applied to column headers; supports inheritance
 *   for nested column groups.
 */
public data class DisplayConfiguration(
    ...
    @Deprecated(message = DISPLAY_CONFIGURATION, level = DeprecationLevel.HIDDEN)
    public constructor(
        rowsLimit: Int? = 20,
    ...
    /** For binary compatibility. */
    @Deprecated(message = DISPLAY_CONFIGURATION_COPY, level = DeprecationLevel.HIDDEN)
    public fun copy(
    ...
    /** DSL accessor. */
    public operator fun invoke(block: DisplayConfiguration.() -> Unit): DisplayConfiguration =
        apply(block)
}
...
public value class RendererDecimalFormat private constructor(internal val format: String) {
    public companion object {
        public fun fromPrecision(precision: Int): RendererDecimalFormat {
            require(precision >= 0) {
                "precision must be >= 0. for custom format use RendererDecimalFormat.of"
            }
...
private fun flagFromEnv(envName: String): Boolean =
    System.getenv(envName)?.toBooleanStrictOrNull() ?: false

internal fun String.escapeNewLines() = replace("\n", "\\n").replace("\r", "\\r")

internal fun String.escapeForHtmlInJs() = replace("\"", "\\\"").escapeNewLines()

internal fun renderValueForHtml(
    value: Any?,
    truncate: Int,
    format: RendererDecimalFormat,
): RenderedContent = formatter.truncate(renderValueToString(value, format), truncate)
...
internal class DataFrameFormatter(
    ...
    private fun String.addCss(css: String? = null): RenderedContent =
        RenderedContent.text(this).addCss(css)

    private fun String.ellipsis(fullText: String) =
        addCss(structuralClass).copy(fullContent = fullText)
    ...
    private fun RenderedContent.addCss(css: String? = null): RenderedContent =
        if (css != null) {
            copy(
                truncatedContent = "<span class=\"$css\">$truncatedContent</span>",
                isFormatted = true,
            )
        } else {
            this
    ...
    private class Builder {
        ...
        fun result() =
            RenderedContent(sb.toString(), len, if (isTruncated) "" else null, isFormatted)
    }
    ...
    private fun render(
            ...
            for (index in values.indices) {
                ...
                val rendered =
                    render(
                        values[index],
                        renderer,
                        configuration.copy(cellContentLimit = valueLimit),
                    )
        ...
        val result: RenderedContent? =
            when (value) {
                ...
                is AnyFrame ->
                    renderer
                        .content("DataFrame [${value.size}]", configuration)
                        .addCss(dataFrameClass)
                    ...
                    if (key.length > keyLimit) {
                        ...
                        val renderedValue =
                            render(
                                value.second,
                                renderer,
                                configuration.copy(cellContentLimit = limit - key.length),
                            ) ?: "...".addCss(structuralClass)
