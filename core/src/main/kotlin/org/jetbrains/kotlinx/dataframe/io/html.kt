...
internal fun getResourceText(
    resource: String,
    vararg replacement: Pair<String, Any>,
): String {
 ...
 */
private fun downsizeBufferedImageIfNeeded(
    value: Any?,
    renderConfig: DisplayConfiguration,
): Any? =
    when {
...
private fun AnyFrame.getColumnsHeaderGrid(): List<List<ColumnWithPathWithBorder<*>>> {
    ...
    fun ColumnWithPath<*>.addChildren(
        depth: Int = 0,
        breadth: Int = 0,
    ) {
        var breadth = breadth
...
internal class DataFrameFormatter(
    ...
    private fun wrap(
        ...
        postfix: String,
    ): RenderedContent = content.copy(truncatedContent = prefix + content.truncatedContent + postfix)
