...
internal fun <T> TreeNode<T>.getOrPutEmpty(
    path: ColumnPath,
    emptyData: T,
): TreeNode<T> = getOrPut(path) { emptyData }

internal fun <T> TreeNode<T?>.put(
    path: ColumnPath,
    data: T,
): TreeNode<T?> = getOrPut(path).also { it.data = data }
