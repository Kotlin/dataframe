...
public data class RenderedContent(
    ...
    public companion object {
        ...
        public fun textWithLength(
            str: String,
            len: Int,
        ): RenderedContent = RenderedContent(str, len, null, false)

        public fun text(str: String): RenderedContent = RenderedContent(str, str.length, null, false)

        public fun truncatedText(
            str: String,
            fullText: String,
        ): RenderedContent = RenderedContent(str, str.length, fullText, false)
    }
...
public abstract class ChainedCellRenderer(private val parent: CellRenderer) : CellRenderer {
    ...
    public abstract fun maybeTooltip(
        value: Any?,
        configuration: DisplayConfiguration,
    ): String?
    ...
    public override fun content(
        value: Any?,
        configuration: DisplayConfiguration,
    ): RenderedContent = maybeContent(value, configuration) ?: parent.content(value, configuration)

    public override fun tooltip(
        value: Any?,
        configuration: DisplayConfiguration,
    ): String = maybeTooltip(value, configuration) ?: parent.tooltip(value, configuration)
}
...
public object DefaultCellRenderer : CellRenderer {
    public override fun content(
        value: Any?,
        configuration: DisplayConfiguration,
    ): RenderedContent = renderValueForHtml(value, configuration.cellContentLimit, configuration.decimalFormat)

    public override fun tooltip(
        value: Any?,
        configuration: DisplayConfiguration,
    ): String = renderValueForHtml(value, tooltipLimit, configuration.decimalFormat).truncatedContent
}
