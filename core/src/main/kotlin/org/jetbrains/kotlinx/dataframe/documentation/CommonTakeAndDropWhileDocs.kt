 ...
 *
 * NOTE: To avoid ambiguity, `{@get [CommonTakeAndDropWhileDocs.OPERATION]}While` is called `{@get
 * [CommonTakeAndDropWhileDocs.OPERATION]}ColsWhile` when called on a [String] or [ColumnPath]
 * resembling a [ColumnGroup].
 *
 * ### Check out: [Usage\]
 *
 * #### Examples:
 * `df.`[select][DataFrame.select]` { `[`cols`][ColumnsSelectionDsl.cols]` { "my"
 * `[`in`][String.contains]` it.`[`name`][DataColumn.name]` }.`[\`{@get
 * [OPERATION]}While\`][ColumnSet.{@get [OPERATION]}While]` { "my" `[`in`][String.contains]`
 * it.`[`name`][DataColumn.name]` } }`
 *
 * `df.`[select][DataFrame.select]` { myColumnGroup.`[\`{@get
 * [OPERATION]}While\`][SingleColumn.{@get [OPERATION]}ColsWhile]` {
 * it.`[`any`][ColumnWithPath.any]` { it == "Alice" } } }`
 *
 * `df.`[select][DataFrame.select]` { "myColumnGroup".`[\`{@get
 * [OPERATION]}ColsWhile\`][String.{@get [OPERATION]}ColsWhile]` {
 * it.`[`kind`][ColumnWithPath.kind]`() == `[`ColumnKind.Value`][ColumnKind.Value]` } }`
 *
