...
class FrameColTests : ColumnsSelectionDslTests() {
    ...
    fun `frameCol exceptions`() {
        shouldThrow<IllegalArgumentException> { dfWithFrames.select { frameCol("name") } }
        shouldThrow<IllegalStateException> { dfWithFrames.select { frameCol("nonExisting") } }
        shouldThrow<IllegalStateException> { dfWithFrames.select { name.frameCol("nonExisting") } }
        shouldThrow<IllegalStateException> { dfWithFrames.select { "age".frameCol("test") } }
        shouldThrow<IndexOutOfBoundsException> { dfWithFrames.select { frameCol(100) } }
    }
    ...
    fun `frameCol at lower level`() {
        ...
        listOf(
                ...
                dfWithFrames.select {
                    NonDataSchemaPerson::name.frameCol<Person>(pathOf("frameCol"))
                },
                ...
                dfWithFrames.select { "name".frameCol(PersonWithFrame::frameCol) },
                dfWithFrames.select {
                    NonDataSchemaPerson::name.frameCol(PersonWithFrame::frameCol)
                },
