...
class DataRowTests : BaseTest() {
    ...
    @Test
    fun diff() {
        typed.update { age }.with { diffOrNull { age } }.age.drop(1).values() shouldBe
            typed.age.values().zipWithNext { curr, next -> next - curr }
    }

    @Test
    fun mean() {
        typed.mapToColumn("mean") { rowMean() }.values() shouldBe
            typed.age.values().zip(typed.weight.values()) { a, b ->
                if (b != null) (a + b) / 2.0 else a
            }
    }

    @Test
    fun std() {
        typed.mapToColumn("std") { rowStd(skipNaN = true, ddof = 0) }.values() shouldBe
            typed.age.values().zip(typed.weight.values()) { a, b ->
                if (b == null) {
                    .0
                } else {
    ...
    @Test
    fun sum() {
        typed.convert { weight }.toDouble().mapToColumn("sum") { rowSum() }.values() shouldBe
            typed.age.values().zip(typed.weight.values()) { a, b -> a + (b ?: 0).toDouble() }
    }

    @Test
    fun namedValuesOf() {
        typed.mapToColumn("vals") { namedValuesOf<Int>().map { it.value } }.values() shouldBe
            typed.merge { age and weight }.by { it.filterNotNull() }.intoList()
    }

    @Test
    fun valuesOf() {
        typed.mapToColumn("vals") { valuesOf<String>() }.values() shouldBe
            typed.merge { name and city }.by { it.filterNotNull() }.intoList()
    }

    @Test
    fun namedValuesFilter() {
        typed.mapToColumn("vals") { namedValues().firstOrNull { it.value == null }?.name } shouldBe
