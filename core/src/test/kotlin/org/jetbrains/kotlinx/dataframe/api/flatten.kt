...
class FlattenTests {
    ...
    fun `flatten nested`() {
        ...
        val grouped = df.group("a", "b").into("e").group("e", "c").into("f")
    ...
    fun `flatten with parent name conflict`() {
        val df = dataFrameOf("a", "b", "c", "d")(1, 2, 3, 4)
        val grouped =
            df.group("a", "b").into("e").group("e", "c").into("f").rename { "f"["e"] }.to("a")
    ...
    fun `flatten the aggregation and check column names`() {
        ...
        aggregate.flatten(keepParentNameForColumns = true).columnNames() shouldBe
            listOf("city", "mean_age", "mean_weight", "std_age", "std_weight")
