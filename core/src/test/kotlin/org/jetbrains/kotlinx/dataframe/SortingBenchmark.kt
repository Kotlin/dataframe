...
open class SortingBenchmark {
    ...
    @Param("10000", "100000", "1000000") var size: Int = 0

    @Param("int", "string", "double", "category", "list", "frame") lateinit var columnType: String
        ...
        df =
            (0 until size).toDataFrame {
                ...
                "frame" from
                    {
                        dataFrameOf(
                            "x" to List(random.nextInt(1, 50)) { random.nextInt() }.toColumn()
                        )
                    }
            }
