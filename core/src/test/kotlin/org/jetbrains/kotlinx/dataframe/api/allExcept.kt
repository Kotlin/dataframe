...
class AllExceptTests : ColumnsSelectionDslTests() {
    ...
    fun `issue 761`() {
        val renamed =
            df.rename { colsAtAnyDepth() except name.firstName }.to { it.name.uppercase() }
    ...
    fun exceptions() {
        shouldThrow<IllegalStateException> {
            dfGroup.select { name.firstName.allColsExcept("firstName"["secondName"]) }
        }
    ...
    fun `top-level`() {
        ...
        listOf(df.select { all() }, df.select { allExcept { none() } }).shouldAllBeEqual()
        ...
        df.getColumns { (name and name and name.firstName).except(name.firstName) }
            .forEach {
    ...
    fun `on columnSet`() {
        ...
        listOf(
                ...
                df.select {
                    name.allCols() except { colsAtAnyDepth().filter { "last" in it.name } }
                },
    ...
    fun `on lower level`() {
        ...
        listOf(
                ...
                //            df.select { name.allColsExcept(pathOf("name", "lastName")) }, //
                // breaks
                ...
                //            df.select { Person::name.allColsExcept(pathOf("name", "lastName")) },
                // // breaks
                ...
                df.select { NonDataSchemaPerson::name.allColsExcept(Name::lastName) },
                df.select {
                    NonDataSchemaPerson::name.allColsExcept(Name::lastName, Name::lastName)
                },
                df.select { NonDataSchemaPerson::name.allColsExcept(pathOf("lastName")) },
                df.select {
                    NonDataSchemaPerson::name.allColsExcept(pathOf("lastName"), pathOf("lastName"))
                },
                //            df.select { NonDataSchemaPerson::name.allColsExcept(pathOf("name",
                // "lastName")) }, // breaks
                df.select {
                    NonDataSchemaPerson::name.allColsExcept { cols { "last" in it.name } }
                },
                ...
                //            df.select { pathOf("name").allColsExcept(pathOf("name", "lastName"))
                // }, // breaks
    ...
    fun `2 levels deep`() {
        listOf(
                ...
                dfGroup.select { name.allColsExcept("firstName"["secondName"]) },
                dfGroup.select {
                    name.allColsExcept { firstName.secondName and firstName.secondName }
                },
                dfGroup.select { name.allColsExcept { colGroup("firstName").col("secondName") } },
            )
            .shouldAllBeEqual()

        listOf(
                dfGroup
                    .remove { name.firstName.secondName }
                    .select { name.firstName.allCols() }
                    .alsoDebug(),
                dfGroup.select { name.firstName.allColsExcept("secondName") },
                dfGroup.select { name.firstName.allColsExcept { secondName } },
                dfGroup.select { name.firstName.allColsExcept(pathOf("secondName")) },
            )
            .shouldAllBeEqual()
        ...
        listOf(
                ...
                dfGroup
                    .select { name { firstName.allColsExcept(pathOf("firstName")) } }
                    .alsoDebug(),
                dfGroup.select {
                    (name.allColsExcept("firstName"["firstName"])).first().asColumnGroup().allCols()
                },
        ...
        listOf(
                ...
                dfGroup.select {
                    name.firstName.allColsExcept { secondNameAccessor and thirdNameAccessor }
                },
                dfGroup.select { name.firstName.allColsExcept("secondName", "thirdName") },
                dfGroup.select {
                    name.firstName.allColsExcept(FirstNames::secondName, FirstNames::thirdName)
                },
                dfGroup.select {
                    name.firstName.allColsExcept(pathOf("secondName"), pathOf("thirdName"))
                },
                dfGroup.select {
                    name.firstName.allColsExcept {
                        cols { it.name in listOf("secondName", "thirdName") }
                    }
                },
                dfGroup.select { name.firstName { allExcept { secondName and thirdName } } },
                dfGroup.select {
                    name.firstName { allExcept { secondNameAccessor and thirdNameAccessor } }
                },
                dfGroup.select { name.firstName { allExcept("secondName", "thirdName") } },
                dfGroup.select {
                    name.firstName { allExcept(FirstNames::secondName, FirstNames::thirdName) }
                },
                dfGroup.select {
                    name.firstName { allExcept(pathOf("secondName"), pathOf("thirdName")) }
                },
                dfGroup.select {
                    name.firstName {
                        allExcept { cols { it.name in listOf("secondName", "thirdName") } }
                    }
                },
                dfGroup.select { name { firstName.allColsExcept { secondName and thirdName } } },
                dfGroup.select {
    ...
    fun `except on column group`() {
        ...
        listOf(
                ...
                df.select {
                    name.except { cols { "first" in it.name } and cols { "first" in it.name } }
                },
                ...
                //            df.select { name.except(name.firstName and name.firstName) }, // not
                // allowed
                //            df.select { name.except(firstNameAccessor and firstNameAccessor) }, //
                // not allowed
                df.select { name.except("firstName") },
                ...
                df.select { "name".except(pathOf("firstName")) },
                df.select { "name".except(pathOf("firstName"), pathOf("firstName")) },
                //            df.select { Person::name.except(name.firstName and name.firstName) },
                // // not allowed
                //            df.select { Person::name.except(firstNameAccessor and
                // firstNameAccessor) }, // not allowed
                ...
                df.select {
                    NonDataSchemaPerson::name.except {
                        cols { "first" in it.name } and cols { "first" in it.name }
                    }
                },
                ...
                df.select {
                    NonDataSchemaPerson::name.except { firstNameAccessor and firstNameAccessor }
                },
                //            df.select { NonDataSchemaPerson::name.except(name.firstName and
                // name.firstName) }, // not allowed
                //            df.select { NonDataSchemaPerson::name.except(firstNameAccessor and
                // firstNameAccessor) }, // not allowed
                df.select { NonDataSchemaPerson::name.except("firstName") },
                ...
                df.select {
                    NonDataSchemaPerson::name.except(pathOf("firstName"), pathOf("firstName"))
                },
                ...
                //            df.select { pathOf("name").except(name.firstName and name.firstName)
                // }, // not allowed
                //            df.select { pathOf("name").except(firstNameAccessor and
                // firstNameAccessor) }, // not allowed
