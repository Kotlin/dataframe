...
class SeriesTests {
    ...
    fun `diff test`() {
        val withDiff =
            typed.sortBy { city and day }.groupBy { city }.add("diff") { diff(0) { temp } }.concat()
        ...
        val expected =
            typed
                .sortBy { city and day }
                .rows()
                .map { row -> srcData[row.city to (row.day - 1)]?.let { row.temp - it } ?: 0 }
    ...
    fun movingAverage() {
        ...
        val expected =
            typed
                ...
                .map { row ->
                    (0 until k)
                        .mapNotNull { srcData[row.city to row.day - it] }
