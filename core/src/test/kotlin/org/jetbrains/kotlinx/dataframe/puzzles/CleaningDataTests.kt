...
class CleaningDataTests {
    ...
    private val fromTo =
        listOf(
            ...
            .toColumn("From_To")
    private val flightNumber =
        listOf(10045.0, Double.NaN, 10065.0, Double.NaN, 10085.0).toColumn("FlightNumber")
    ...
    fun `airline test`() {
        val expected =
            columnOf("KLM", "Air France", "British Airways", "Air France", "Swiss Air")
                .named("Airline")

        df.update { airline }
            .with { "([a-zA-Z\\s]+)".toRegex().find(it)?.value?.trim() ?: "" }[airline] shouldBe
            expected

        df.update { "Airline"<String>() }
            .with { "([a-zA-Z\\s]+)".toRegex().find(it)?.value?.trim() ?: "" }["Airline"] shouldBe
            expected
    }
    ...
    fun `split delays`() {
        ...
        val expected =
            dataFrameOf("delay_1", "delay_2", "delay_3")(
                23.0,
                47.0,
                Double.NaN,
                Double.NaN,
        ...
        df.convert { recentDelays }
            .with { it.map { d -> d.toDouble() } }
            .split { recentDelays }
            .default<_, _, Number>(Double.NaN)
            .into { "delay_$it" }[delay1, delay2, delay3] shouldBe expected

        df.convert { "RecentDelays"<List<Int>>() }
            .with { it.map { d -> d.toDouble() } }
