...
class ColsTests : ColumnsSelectionDslTests() {
    ...
    @Test
    fun `cols exceptions`() {
        shouldThrow<IllegalArgumentException> { df.select { "age".allCols() } }
        shouldThrow<IllegalArgumentException> { df.select { cols("non-existent") } }
        shouldThrow<IllegalArgumentException> { df.select { name.cols("non-existent") } }

        // calls are always relative to the receiver
        shouldThrow<IllegalArgumentException> { df.select { name.cols(name.firstName) } }

        // "breaking spec" and using a column accessor where it doesn't belong does not work.
        shouldThrow<IllegalArgumentException> {
            df.select select1@{ name.select { cols(this@select1.name.firstName) } }
        }
    }
    ...
    fun `cols and get with column references`() {
        ...
        listOf(
                ...
                df.select { name.select { cols(firstName, lastName) } },
                df.select { it["name"].asColumnGroup().select { cols("firstName", "lastName") } },
                df.select { "name".cols(firstName, lastName) },
                df.select { "name"[firstName, lastName] },
    ...
    fun `cols and get with column paths`() {
        ...
        listOf(
                ...
                df.select {
                    NonDataSchemaPerson::name.cols(pathOf("firstName"), pathOf("lastName"))
                },
                df.select {
                    NonDataSchemaPerson::name.cols<String>(pathOf("firstName"), pathOf("lastName"))
                },
                ...
                df.select {
                    it["name"].asColumnGroup().cols(pathOf("firstName"), pathOf("lastName"))
                },
                df.select {
                    it["name"].asColumnGroup().cols<String>(pathOf("firstName"), pathOf("lastName"))
                },
    ...
    fun `cols and get with KProperties`() {
        ...
        listOf(
                ...
                df.select {
                    columnGroup(NonDataSchemaPerson::name).cols(Name::firstName, Name::lastName)
                },
                df.select {
                    columnGroup(NonDataSchemaPerson::name)[Name::firstName, Name::lastName]
                },
    ...
    fun `triple nested accessor edge case`() {
        listOf(
                ...
                dfGroup.select {
                    name.cols(colGroup("firstName").col("firstName"), col("lastName"))
                },
                // so not absolute
