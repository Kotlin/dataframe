...
class FastDoubleParserTests {
    ...
    fun `can fast parse doubles`() {
        val parser =
            FastDoubleParser(ParserOptions(locale = Locale.ROOT, useFastDoubleParser = true))
    ...
    fun `can fast parse german locale`() {
        val parser =
            FastDoubleParser(ParserOptions(locale = Locale.GERMANY, useFastDoubleParser = true))

        val numbers = listOf("12,45", "-13,35", "100.123,35", "-204.235,23", "1,234e3")

        val expectedDoubles = listOf(12.45, -13.35, 100_123.35, -204_235.23, 1.234e3)
    ...
    fun `can fast parse french locale`() {
        val parser =
            FastDoubleParser(ParserOptions(locale = Locale.FRANCE, useFastDoubleParser = true))
    ...
    fun `can fast parse estonian locale`() {
        ...
        val expectedDoubles = listOf(12.45, -13.35, 100_123.35, -204_235.23, 1.234e3, -345.122)
    ...
    fun `fast parse any locale`() {
        ...
        for (locale in locales) {
            val parser =
                FastDoubleParser(ParserOptions(locale = locale, useFastDoubleParser = true))
            ...
            for (double in doubles) {
                ...
                assert(
                    double == parsedString || (double.isNaN() && parsedString?.isNaN() == true)
                ) {
                ...
                assert(
                    double == parsedCharArray ||
                        (double.isNaN() && parsedCharArray?.isNaN() == true)
                ) {
                ...
                assert(
                    double == parsedByteArray ||
                        (double.isNaN() && parsedByteArray?.isNaN() == true)
                ) {
