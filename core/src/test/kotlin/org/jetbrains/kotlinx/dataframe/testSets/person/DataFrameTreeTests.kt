...
class DataFrameTreeTests : BaseTest() {
    ...
    @Test
    fun selectDfs() {
        val cols = typed2.select { cols { it.hasNulls }.recursively() }
        cols shouldBe typed2.select { weight and nameAndCity.city }
    }
    ...
    @Test
    fun `group cols`() {
        val joined = typed2.move { all().recursively(false) }.into { pathOf(it.path.joinToString(".")) }
    ...
    fun splitFrameColumnsIntoRows() {
        ...
        val res =
            plain.split(grouped.groups).intoRows().remove { it[groupCol]["city"] }.ungroup(groupCol)
                .sortBy { name and age }
    ...
    fun `distinct at column group`() {
        typed2.nameAndCity.distinct().filter { name.startsWith("A") }.columns() shouldBe typed.select { name and city }
            .distinct()
