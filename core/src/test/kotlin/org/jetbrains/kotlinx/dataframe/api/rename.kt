...
class RenameTests : ColumnsSelectionDslTests() {
    ...
    companion object {
        val simpleDf = dataFrameOf("a", "b", "c")(1, 2, 3, 4, 5, 6)
    ...
    @Test
    fun `simple rename`() {
        val renamedDf = dataFrameOf("a_renamed", "b_renamed", "c_renamed")(1, 2, 3, 4, 5, 6)
    ...
    fun `test rename with String to String pairs`() {
        ...
        simpleDf.rename("c" to "c_renamed", "a" to "a_renamed", "b" to "b_renamed") shouldBe
            renamedDf
    }
    ...
    fun `partial grouped rename`() {
        val renamedDf =
            dataFrameOf("a_renamed", "b", "c")(1, 2, 3, 4, 5, 6)
                .group { "a_renamed" and "b" }
                .into("group_renamed")

        groupedDf.rename { "group" and "group"["a"] }.to { it.name + "_renamed" } shouldBe renamedDf
    }

    @Test
    fun `grouped rename`() {
        val renamedDf =
            dataFrameOf("a_renamed", "b_renamed", "c_renamed")(1, 2, 3, 4, 5, 6)
                .group { "a_renamed" and "b_renamed" }
                .into("group_renamed")

        groupedDf.rename { colsAtAnyDepth() }.to { it.name + "_renamed" } shouldBe renamedDf
    }
    ...
    fun `double grouped rename in 3 steps`() {
        ...
        doubleGroupedDf.rename { colsAtAnyDepth() }.to { it.name + "_renamed" } shouldBe renamedDf
    }
    ...
    fun `col by index named with convert`() {
        val df = dataFrameOf("a" to columnOf(1, 4), "b" to columnOf(2, 5), "c" to columnOf(3, 6))
...
class RenameToCamelCaseTests {
    companion object {
        ...
        val nestedColumnGroup = dataFrameOf("test_name")(dataFrameOf("another_name")(1).first())
        val doublyNestedColumnGroup =
            dataFrameOf("test_name")(
                dataFrameOf("another_name")(dataFrameOf("third_name")(1).first()).first()
            )
    ...
    fun `deeply nested df`() {
        ...
        df.schema().asClue {
            ...
            df["testName"]["groupName"].asAnyFrameColumn()[0].columnNames() shouldBe
                listOf("anotherName")
        }
    }
    ...
    fun `rename to camelCase`() {
        val dfWithUpperCaseColumnNames =
            dataFrameOf("First_Column", "second_column", "ThirdColumn")(1, 2, 3)
