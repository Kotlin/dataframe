...
class ColGroupTests : ColumnsSelectionDslTests() {
    ...
    fun `colGroup exceptions`() {
        shouldThrow<IllegalArgumentException> { dfGroup.select { colGroup("age") } }
        shouldThrow<IllegalStateException> { dfGroup.select { colGroup("nonExisting") } }
        shouldThrow<IllegalStateException> { dfGroup.select { name.colGroup("nonExisting") } }
        shouldThrow<IllegalStateException> { dfGroup.select { "age".colGroup("test") } }
        shouldThrow<IndexOutOfBoundsException> { dfGroup.select { colGroup(100) } }
    }
    ...
    fun `frameCol at lower level`() {
        ...
        listOf(
                ...
                dfGroup.select {
                    NonDataSchemaPerson::name.colGroup<FirstNames>(pathOf("firstName"))
                },
