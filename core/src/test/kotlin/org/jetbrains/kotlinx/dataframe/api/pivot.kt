...
class PivotTests {
    ...
    fun `pivot aggregate with default`() {
        val pivoted =
            df.pivot(b, inward = false).groupBy(a).aggregate {
                get(c).first() default '-' into "first"
                get(c).last() into "last" default '?'
            }
        pivoted.columnsCount() shouldBe 3
        pivoted.rowsCount() shouldBe 2
        val cols = pivoted.getColumns {
            allExcept(a).colsAtAnyDepth().filter { !it.isColumnGroup() }
        }
        cols.size shouldBe 4
        cols.forEach { it.type() shouldBe typeOf<Char>() }
    ...
    fun `pivot groupBy inward by default`() {
        val df = dataFrameOf("a", "b")(2, "a", 4, "b", 3, "c")
    ...
    fun `pivot values with nulls`() {
        ...
        df.pivot("a").values("b") shouldBe dataFrameOf("1", "2", "3")(1, listOf(1, null), null)[0]
    ...
    fun `pivot in aggregate`() {
        ...
        val expected = dataFrameOf("a", "q", "w")(1, 1, 0, 2, 1, 1).group("q", "w").into("b")

        df.groupBy("a").aggregate { pivot("b").count() } shouldBe expected

        df.groupBy("a").aggregate { pivot("b").count() into "c" } shouldBe
            expected.rename("b" to "c")
    }
    ...
    fun `pivot two in aggregate`() {
        ...
        val expected =
            dataFrameOf(
                ...
                columnOf(
                    columnOf(columnOf(1, 1) named "q", columnOf(0, 1) named "w") named "b",
                    columnOf(columnOf(1, 1) named "w", columnOf(0, 1) named "q") named "c",
                ) named "d",
            )

        df.groupBy("a").aggregate { pivot("b", "c").count() into "d" } shouldBe expected
    }

    @Test
    fun `pivot minBy values`() {
        val df = dataFrameOf("a", "b", "c", "d")(1, 2, 3, 5, 1, 0, 2, 4, 2, 1, 3, 2, 2, 5, 5, 3)
        df.pivot("a").minBy("b").values("c", "d", separate = true) shouldBe
            dataFrameOf("c1", "c2", "d1", "d2")(2, 3, 4, 2)
                .move { all() }
                .into { pathOf(it.name()[0].toString(), it.name()[1].toString()) }[0]
    }

    @Test
    fun `pivot groupBy last with`() {
        val df = dataFrameOf("a", "b", "c")(1, 2, 3, 1, 0, 2, 2, 1, 3, 2, 1, 5)
        df.pivot("a", inward = false).groupBy("b").default(-1).last().with { "c"<Int>() } shouldBe
            dataFrameOf("b", "1", "2")(2, 3, -1, 0, 2, -1, 1, -1, 5)
    }
    ...
    fun `pivot then in aggregate`() {
        ...
        val df1 =
            df.groupBy("category1").aggregate { pivot { "category2" then "category3" }.count() }
    ...
    fun `pivot with default of other type`() {
        ...
        pivoted["city"]["London"]["isHappy"].type() shouldBe
            listOf(typeOf<Int>(), typeOf<Boolean>()).commonType()
    }
}
