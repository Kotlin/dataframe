...
class MaxTests {
    ...
    fun `max with different numeric types`() {
        ...
        columnOf(5.toShort(), 2.toShort(), 8.toShort(), 1.toShort(), 9.toShort()).max() shouldBe
            9.toShort()
        columnOf(5.toByte(), 2.toByte(), 8.toByte(), 1.toByte(), 9.toByte()).max() shouldBe
            9.toByte()
    ...
    fun `max with just nulls`() {
        DataColumn.createValueColumn("", listOf(null, null), nothingType(true))
            .maxOrNull()
            .shouldBeNull()
    }
    ...
    fun `maxBy with selector function`() {
        ...
        val people = columnOf(Person("Alice", 30), Person("Bob", 25), Person("Charlie", 35))
    ...
    fun `rowMaxOf with dataframe and nulls`() {
        val df = dataFrameOf("a", "b", "c")(1f, 2, 3, 4f, null, 6, 7f, 8, 9)
    ...
    @[Test Suppress("ktlint:standard:argument-list-wrapping")]
    fun `dataframe max`() {
        val df = dataFrameOf("a", "b", "c")(1, 2f, 3.0, 4, 5f, 6.0, 7, 8f, 9.0)
    ...
    fun `max with NaN values for floating point numbers`() {
        ...
        floatWithNaN.max(skipNaN = true) shouldBe
            9.0f // With skipNaN=true, NaN values should be ignored
        ...
        doubleWithNaN.max(skipNaN = true) shouldBe
            9.0 // With skipNaN=true, NaN values should be ignored
        ...
        multipleNaN.max(skipNaN = true) shouldBe
            2.0f // With skipNaN=true, NaN values should be ignored
        ...
        "c" shouldNotBeIn
            maxsWithNaN
                .columnNames() // Column 'c' should be excluded due to mixed number types todo
        ...
        val maxForWithNaN =
            dfWithNaN.maxFor("a", "b") // Max functions should return NaN if any value is NaN
        ...
        // Test max of all columns as a single value
        (dfWithNaN.max("a", "b") as Double).isNaN() shouldBe
            true // Max functions should return NaN if any value is NaN
        dfWithNaN.max("a", "b", skipNaN = true) shouldBe
            8.0 // With skipNaN=true, NaN values should be ignored

        // Test maxOf with transformation that might produce NaN values
        val dfForTransform = dataFrameOf("a", "b")(4.0, 0.0, 1.0, 2.0, 0.0, 0.0)
