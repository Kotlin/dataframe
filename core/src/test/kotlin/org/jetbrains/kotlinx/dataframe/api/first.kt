...
/**
 * Tests the behavior of the [first] ([FirstColumnsSelectionDsl.firstCol]) and [firstOrNull]
 * functions, including:
 *
 * - [ColumnsSelectionDsl]: selecting the first column or first column matching a condition, with
 *   invocations on illegal types, and in case when no column matches the condition.
 *
 * - [DataColumn]: getting the first value or the first value matching a predicate, verifying
 *   behavior on empty columns, columns with `null` values, and columns without values matching the
 *   predicate.
 *
 * - [DataFrame]: getting the first [row][DataRow] or first matching [row][DataRow], verifying
 *   behavior on empty DataFrames, DataFrames with `null` values, and DataFrames without rows
 *   matching the predicate.
 *
 * - [GroupBy]: reducing each group to its first [row][DataRow] or the first [row][DataRow] matching
 *   a predicate, with handling groups that contain no matching rows.
 *
 * - [Pivot]: reducing each group in the [Pivot] to its first [row][DataRow] or the first
 *   [row][DataRow] matching a predicate, with handling groups that contain no matching rows.
 *
 * - [PivotGroupBy]: reducing each combined [pivot] + [groupBy] group to its first [row][DataRow] or
 *   the first [row][DataRow] matching a predicate, with handling [pivot] + [groupBy] combinations
...
class FirstTests : ColumnsSelectionDslTests() {
    ...
    fun `ColumnsSelectionDsl first`() {
        shouldThrow<IllegalArgumentException> { df.select { "age".firstCol() } }
        shouldThrow<IllegalArgumentException> { df.select { columnGroup(Person::age).firstCol() } }
        shouldThrow<IllegalArgumentException> { df.select { Person::age.firstCol() } }
        shouldThrow<NoSuchElementException> { df.select { first { false } } }
        ...
        listOf(
                ...
                df.select {
                    NonDataSchemaPerson::name.firstCol { col -> col.any { it == "Alice" } }
                },
                df.select { pathOf("name").firstCol() },
                df.select { pathOf("name").firstCol { col -> col.any { it == "Alice" } } },
                df.select {
                    it["name"].asColumnGroup().firstCol { col -> col.any { it == "Alice" } }
                },
            )
            .shouldAllBeEqual()
    }
    ...
    fun `first on DataColumn`() {
        ...
        shouldThrow<IndexOutOfBoundsException> { df.drop(df.nrow).isHappy.first() }
        shouldThrow<NoSuchElementException> { df.age.first { it > 50 } }
    }
    ...
    fun `first on DataFrame`() {
        ...
        shouldThrow<NoSuchElementException> { df.drop(df.nrow).first() }
        shouldThrow<NoSuchElementException> { df.first { age > 50 } }
        shouldThrow<NoSuchElementException> { df.drop(df.nrow).first { isHappy } }
    }
    ...
    fun `first on GroupBy`() {
        ...
        firstUnhappy shouldBe
            dataFrameOf(
                "isHappy" to columnOf(false),
                "name" to
                    columnOf("firstName" to columnOf("Charlie"), "lastName" to columnOf("Daniels")),
                "age" to columnOf(20),
    ...
    fun `first on GroupBy with predicate`() {
        ...
        firstHappy shouldBe
            dataFrameOf(
                "isHappy" to columnOf(true),
                "name" to columnOf("firstName" to columnOf("Bob"), "lastName" to columnOf("Dylan")),
                "age" to columnOf(45),
