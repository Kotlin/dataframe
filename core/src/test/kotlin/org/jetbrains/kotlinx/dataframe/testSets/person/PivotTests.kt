...
class PivotTests {
    ...
    fun `simple pivot`() {
        ...
        data["weight"].type() shouldBe
            typeOf<Comparable<*>>() // Comparable<String + Int> -> Comparable<Nothing | *>

        res.renderToString(columnTypes = true, title = true) shouldBe
            defaultExpected
                .group { drop(1) }
                .into("key")
                .renderToString(columnTypes = true, title = true)
    ...
    fun `pivot with transform`() {
        ...
        pivoted.getColumns { "key".allCols() }.map { it.name() }.toSet() shouldBe
            typed.key.distinct().map { "_$it" }.toSet()
    }
    ...
    fun `pivot with value map`() {
        val pivoted =
            typed.pivot(inward = false) { key }.groupBy { name }.values { value.map { "_$it" } }
    ...
    fun `pivot two values`() {
        ...
        val expected =
            defaultExpected.replace("age", "city", "weight").with {
                columnOf(
                    it named "value",
                    it.map(Infer.Type) {
                        if (it is List<*>) {
        ...
        pivoted.renderToString(title = true, columnTypes = true) shouldBe
            expected.renderToString(title = true, columnTypes = true)
    }
    ...
    fun `pivot two columns with then`() {
        ...
        val leafColumns = pivoted.getColumnsWithPaths {
            all().drop(1).colsAtAnyDepth().filter { !it.isColumnGroup() }
        }
    ...
    fun `pivot two values without groupBy`() {
        ...
        //        nullGroup.columnTypes() shouldBe listOf(typeOf<Comparable<*>?>(),
        // typeOf<KClass<Any>?>())
    ...
    fun `pivot in group aggregator`() {
        val pivoted = typed.groupBy { name }.aggregate { pivot { key }.with { value } into "key" }
        pivoted.columnsCount() shouldBe 2
        pivoted.print()
        pivoted.ungroup("key") shouldBe
            typed.pivot(inward = false) { key }.groupBy { name }.with { value }
    }
    ...
    fun `equal pivots`() {
        ...
        val pivoted = typed.groupBy { name }.aggregate { pivot { key }.with { value } }
        pivoted.print()
        pivoted shouldBe expected
    }
    ...
    fun `grouped pivot with key and value conversions`() {
        ...
        val pivoted =
            grouped
                .pivot(inward = false) { key.map(transform = keyConverter) }
        ...
        val gathered =
            pivoted
                ...
                .with {
                    it as? Comparable<*>
                } // cast to make the equality test succeed (values are already the same)
        val expected =
            expectedFiltered
                .update { key }
    ...
    fun `gather with name conversion`() {
        val pivoted =
            typed.pivot { key.map(transform = keyConverter) }.groupBy { name }.with { value }
        val gathered =
            pivoted
                .gather { "key".allCols() }
                .notNull()
    ...
    fun `pivot matches yes no`() {
        val pivoted =
            typed.drop(1).pivot(inward = false) { key }.groupBy { name }.matches("yes", "no")
    ...
    fun `pivot aggregate into`() {
        val pivoted =
            typed
                .pivot(inward = false) { key }
                .groupBy { name }
                .aggregate { value.first() into "value" }
        pivoted.columns().drop(1).forEach {
    ...
    fun `pivot columns inward`() {
        ...
        typed.pivot(inward = false) { name and key }.count().columnsCount() shouldBe
            typed.name.countDistinct() + typed.key.countDistinct()
        typed.pivot(inward = true) { name and key }.count() shouldBe
            typed.pivot { name and key }.count()
    }
    ...
    fun `pivot from group`() {
        val pivoted =
            typed
                ...
                .groupByOther()
                .count()
        pivoted.getColumnGroup("info").getColumnGroup("value").columnsCount() shouldBe
            typed.value.countDistinct()
    }
}
