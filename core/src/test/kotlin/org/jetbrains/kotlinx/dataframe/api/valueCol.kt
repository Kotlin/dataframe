...
class ValueColTests : ColumnsSelectionDslTests() {
    ...
    fun `valueCol exceptions`() {
        shouldThrow<IllegalArgumentException> { df.select { valueCol("name") } }
        shouldThrow<IllegalStateException> { df.select { valueCol("nonExisting") } }
        shouldThrow<IllegalStateException> { df.select { name.valueCol("nonExisting") } }
        shouldThrow<IllegalStateException> { df.select { "age".valueCol("test") } }
        shouldThrow<IndexOutOfBoundsException> { df.select { valueCol(100) } }
    }
