...
class ColTests : ColumnsSelectionDslTests() {
    ...
    fun `col on renamed column`() {
        df.select { colGroup("name").named("name1").col("firstName") }.alsoDebug()
    }

    @Test
    fun `col exceptions`() {
        shouldThrow<IllegalStateException> { df.select { col("nonExisting") } }
        shouldThrow<IllegalStateException> { df.select { name.col("nonExisting") } }
        shouldThrow<IllegalStateException> { df.select { "age".col("test") } }
        shouldThrow<IndexOutOfBoundsException> { df.select { col(100) } }
    }
    ...
    fun `col at top-level`() {
        ...
        listOf(
                df.select { age },
                @Suppress("DEPRECATION") df.select { col(ageAccessor) },
                df.select { col("age") },
