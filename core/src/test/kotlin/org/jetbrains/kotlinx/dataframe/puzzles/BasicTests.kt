...
class BasicTests {
    ...
    fun `select rows (3, 4, 8) and columns (animal, age)`() {
        val expected = dataFrameOf("animal", "age")("dog", Double.NaN, "dog", 5.0, "dog", 7.0)
    ...
    fun `calculate mean age for each animal`() {
        val expected =
            dataFrameOf("animal", "age")("cat", Double.NaN, "snake", 2.5, "dog", Double.NaN)
    ...
    fun `count number of each type of animal`() {
        val expected = dataFrameOf("animal", "count")("cat", 4, "snake", 2, "dog", 4)
    ...
    fun `find mean age for each animal type and number of visits`() {
        ...
        val actualDfAcc =
            df.pivot(inward = false) { visits }.groupBy { animal }.mean(skipNaN = true) { age }
        val actualDfStr =
            df.pivot("visits", inward = false).groupBy("animal").mean("age", skipNaN = true)
