...
class DataFrameTests : BaseTest() {
    ...
    fun `create from map`() {
        ...
        df["age"].type() shouldBe NULLABLE_INT
    }
    ...
    fun `union dataframes with different type of the same column`() {
        ...
        df2["age"].type() shouldBe NULLABLE_DOUBLE
        val merged = df.concat(df2)
        merged["age"].type() shouldBe NULLABLE_NUMBER
        val updated = merged.convert("age") { "age"<Number?>()?.toDouble() }
        updated["age"].type() shouldBe NULLABLE_DOUBLE
    }
    ...
    fun `addRow`() {
        ...
        res.name.type() shouldBe STRING
        res.age.type() shouldBe NULLABLE_INT
        res.city.type() shouldBe NULLABLE_STRING
        res.weight.type() shouldBe NULLABLE_INT
    ...
    fun `mean for all columns`() {
        ...
        d["age"].type() shouldBe DOUBLE
        d["weight"].type() shouldBe DOUBLE
    }
    ...
    fun `create with random`() {
        ...
        df.columns().forEach { it.type() shouldBe INT }
    }
    ...
    fun `create with list of names and fill nulls`() {
        ...
        df.nrow shouldBe 10
        df.ncol shouldBe 2
        df.columns().forEach { col -> (col.type() == NULLABLE_DOUBLE && col.allNulls()) shouldBe true }
    }
    ...
    fun `create with list of names and fill true`() {
        ...
        df.nrow shouldBe 5
        df.ncol shouldBe 2
        df.columns().forEach { col -> (col.type() == BOOLEAN && col.all { it == true }) shouldBe true }
    }
    ...
    fun `pivot mean values`() {
        ...
        pivoted.getColumnGroup(1).columns().forEach {
            ...
            group.columns().forEach {
                it.type() shouldBe NULLABLE_DOUBLE
            }
        }
    }
    ...
    fun updateWithZero() {
        ...
        updated.age.type shouldBe INT
        updated["weight"].type shouldBe DOUBLE
