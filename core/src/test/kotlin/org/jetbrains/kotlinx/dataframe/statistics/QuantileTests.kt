...
class QuantileTests {
    ...
    fun `linear estimation`() {
        ...
        sequenceOf(1.0, 4.0, 3.0, 2.0)
            ...
            Quantile.withDefaults()
                .with(Quantile.EstimationMethod.HF8)
        ...
        val nanResult =
            sequenceOf(1.0, Double.NaN, 3.0, 2.0)
                .quantileOrNull(
                    p = 0.5,
                    type = typeOf<Double>(),
                    skipNaN = false,
    ...
    fun `constant estimation`() {
        ...
        sequenceOf("apple", "cherry", "banana")
            .quantileOrNull(
                p = 0.5,
                type = typeOf<String>(),
                skipNaN = true,
        ...
        sequenceOf(1, 4, 3, 2)
            ...
            Quantile.withDefaults()
                .with(Quantile.EstimationMethod.HF3)
                .evaluate(doubleArrayOf(1.0, 4.0, 3.0, 2.0), 0.5)
                .toInt()
        ...
        sequenceOf(1, 4, 3, 2)
            ...
            Quantile.withDefaults()
                .with(Quantile.EstimationMethod.HF1)
                .evaluate(doubleArrayOf(1.0, 4.0, 3.0, 2.0), 0.5)
                .toInt()
    ...
    fun `edge cases`() {
        ...
        // We don't test extreme low quantile values (p close to 0.0) as they can cause index
        // calculation issues

        // We don't test extreme high quantile values (p close to 1.0) as they can cause index
        // calculation issues

        // All NaN values with skipNaN = true - p = 0.1 (10th percentile)
        sequenceOf(Double.NaN, Double.NaN)
            .quantileOrNull(
                p = 0.1,
                type = typeOf<Double>(),
                skipNaN = true,
