...
class GetTests {
    ...
    fun `exceptions from empty dataframe`() {
        ...
        shouldThrow<NoSuchElementException> { empty.first() }
        shouldThrow<NoSuchElementException> { empty.last() }
    ...
    @DataSchema data class Schema(val a: Int)

    @Test
    fun `create typed frame column accessor`() {
        val df =
            dataFrameOf(columnOf(dataFrameOf("a")(1), dataFrameOf("a", "b")(2, 3, 4, 5)).named("x"))
    ...
    fun `create typed column group accessor`() {
        val df = dataFrameOf(dataFrameOf("a", "b")(1, 2, 3, 4).asColumnGroup("x"))
    ...
    fun `throw meaningful exception when traverse columns in DataRow`() {
        ...
        val throwable = shouldThrowAny { df[0].getColumnGroup("a") }
