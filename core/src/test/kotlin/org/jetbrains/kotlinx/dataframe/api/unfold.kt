...
class UnfoldTests {
    @Test
    fun unfold() {
        val df = dataFrameOf("col" to listOf(A("123", 321)))
    ...
    fun `unfold deep`() {
        ...
        df1.unfold { col("col") }[pathOf("col", "participants")].type() shouldBe
            typeOf<List<Person>>()

        df1.unfold(maxDepth = 2) { col("col") }[pathOf("col", "participants")][0]
            .shouldBeInstanceOf<AnyFrame> { it["firstName"][0] shouldBe "Alice" }
    }
    ...
    fun `unfold pair of dataframe structures`() {
        ...
        val df = dataFrameOf("pairs" to columnOf(schema to schema.first())).unfold("pairs")

        df.schema().asClue {
            val pairsGroup = df.shouldHaveColumnGroup("pairs")
            pairsGroup.shouldHaveFrameColumn("first") { it[0].shouldHaveColumn<Int>("b") }
            pairsGroup.shouldHaveColumnGroup("second") { it.shouldHaveColumn<Int>("b") }
        }
    }

    @DataSchema data class SimpleDataSchema(val b: Int)

    @Test
    fun `unfold pair of dataschema object structures`() {
        val element = SimpleDataSchema(42)
        val df = dataFrameOf("pairs" to columnOf(listOf(element) to element)).unfold("pairs")
