...
kotlin.sourceSets {
    main { kotlin.srcDir("src/generated-dataschema-accessors/main/kotlin/") }
    test { kotlin.srcDir("src/generated-dataschema-accessors/test/kotlin/") }
}

sourceSets {
    // Gradle creates configurations and compilation task for each source set
    create("samples") { kotlin.srcDir("src/test/kotlin") }
}
...
dependencies {
    ...
    // kotest shouldBe forcefully escapes newlines when content is identical except additional
    // newlines
    // which makes diff very hard to perceive. CodeGenerationTests.kt suffers from it a lot. using
    // assertEquals there for working diff.
...
configurations {
    val testJava16Implementation by getting { extendsFrom(configurations.testImplementation.get()) }
    val testJava16RuntimeOnly by getting { extendsFrom(configurations.testRuntimeOnly.get()) }
}
...
benchmark {
    ...
    configurations { register("sort") { include("SortingBenchmark") } }
}
...
val compileSamplesKotlin =
    tasks.named<KotlinCompile>("compileSamplesKotlin") {
...
val samplesTest =
    tasks.register<Test>("samplesTest") {
        group = "Verification"
        description =
            "Runs all samples that are used in the documentation, but modified to save their outputs to a file."
        ...
        filter { includeTestsMatching("org.jetbrains.kotlinx.dataframe.samples.api.*") }

        ignoreFailures = true

        testClassesDirs =
            fileTree("${layout.buildDirectory.get().asFile.path}/classes/testWithOutputs/kotlin")
        classpath =
...
val clearSamplesOutputs by tasks.registering {
    ...
    doFirst {
        delete {
            val generatedSnippets =
                fileTree(file("../docs/StardustDocs/resources/snippets"))
...
tasks.withType<KorroTask> { dependsOn(copySamplesOutputs) }
...
// generateLibrariesJson makes sure a META-INF/kotlin-jupyter-libraries/libraries.json file is
// generated
// This file allows loading dataframe-jupyter when dataframe-core is present on its own in a Kotlin
// Notebook.
...
val generateLibrariesJson by tasks.registering {
    val outDir =
        generatedJupyterResourcesDir.get().asFile.resolve("META-INF/kotlin-jupyter-libraries")
...
tasks.processResources {
    if (project.hasProperty("includeCoreLibrariesJson")) {
        ...
        doLast {
            logger.lifecycle(
                "$this includes generated META-INF/kotlin-jupyter-libraries/libraries.json"
            )
        }
    }
}

tasks.withType<Jar> { mustRunAfter(tasks.generateKeywordsSrc) }

tasks.processKDocsMain { dependsOn(tasks.generateKeywordsSrc, tasks.generateBuildConfigClasses) }

tasks.runKtlintFormatOverMainSourceSet { dependsOn(tasks.generateKeywordsSrc) }

tasks.runKtlintFormatOverTestSourceSet { dependsOn(tasks.generateKeywordsSrc) }

tasks.runKtlintFormatOverGeneratedMainSourcesSourceSet { dependsOn(tasks.generateKeywordsSrc) }

tasks.runKtlintCheckOverMainSourceSet { dependsOn(tasks.generateKeywordsSrc) }

tasks.runKtlintCheckOverTestSourceSet { dependsOn(tasks.generateKeywordsSrc) }

tasks.runKtlintCheckOverGeneratedMainSourcesSourceSet { dependsOn(tasks.generateKeywordsSrc) }
...
val testJava16Task =
    tasks.register<Test>("testJava16") {
...
tasks.check { dependsOn(testJava16Task) }
