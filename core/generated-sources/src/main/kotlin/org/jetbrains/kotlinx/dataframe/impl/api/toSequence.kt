...
internal fun AnyFrame.toSequenceImpl(type: KType): Sequence<Any> {
    ...
    require(clazz.isData) {
        "`$clazz` is not a data class. `toList` is supported only for data classes."
    }
                    ...
                    when (column.kind) {
                            ...
                            val col: AnyCol =
                                if (it.type.jvmErasure == List::class) {
                                    ...
                                    require(elementType != null) {
                                        "FrameColumn can not be converted to type `List<*>`"
                                    }
                                    column.asAnyFrameColumn().map {
                                        it.toSequenceImpl(elementType).toList()
                                    }
                                } else {
                        ...
                        ColumnKind.Group -> {
                            DataColumn.createValueColumn(
                                column.name(),
                                column.asColumnGroup().toSequenceImpl(it.type).toList(),
                            )
                        }
    ...
    return rows().asSequence().map { row ->
        val parameters = convertedColumns.map { row[it] }.toTypedArray()
