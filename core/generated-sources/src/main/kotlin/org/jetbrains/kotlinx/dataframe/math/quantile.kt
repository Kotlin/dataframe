 ...
 *
 * When [method] is a [QuantileEstimationMethod.Selecting] method, [this] can be a sequence with any
 * self-comparable type. The returned value will be selected from the sequence.
 *
 * Otherwise, when [method] is a [QuantileEstimationMethod.Interpolating] method, [this] can only be
 * a sequence with primitive number types. The returned value will be [Double].
 *
...
internal fun <T : Comparable<T>> Sequence<Any>.quantileOrNull(
    ...
    if (type.canBeNaN && !skipNaN && any { it.isNaN }) {
        // ensure that using a selecting quantile estimation method always returns the same type as
        // the input
        if (type == typeOf<Float>() && method is QuantileEstimationMethod.Selecting)
            return Float.NaN
    ...
    return when (method) {
        is QuantileEstimationMethod.Selecting -> method.quantile<T>(p, list as List<T>)
 ...
 *
 * The returned index `i` is either exactly or approaching the index of the quantile in the sequence
 * [this] (when it's sorted and NaN's removed). Returns -1.0 if the sequence [this] is empty.
...
/**
 * Inspired by Hyndman and Fan (1996) Sample Quantiles in Statistical Packages. The American
 * Statistician, 50, 361-365. DOI:10.1080/00031305.1996.10473566
 *
 * and
 * https://commons.apache.org/proper/commons-statistics/commons-statistics-descriptive/javadocs/api-1.1/org/apache/commons/statistics/descriptive/Quantile.EstimationMethod.html
 *
 * They are split in [Selecting] (where [oneBasedIndexOfQuantile] gives an exact index of the
 * quantile in a sorted list of type [Int]) and [Interpolating] (where [oneBasedIndexOfQuantile]
 * gives an approximation for that index of type [Double]). For the [Selecting], the [Value] type
 * can thus be any self-comparable type, but for [Interpolating], [Value] can only be of type
 * [Double], because it needs to perform calculations on the values.
 *
...
internal sealed interface QuantileEstimationMethod<Value : Comparable<Value>, Index : Number> {
    ...
    /**
     * Gives the (1-based) index of the [p]-quantile for a distribution of size [count]. If the
     * result `h` is a whole number, the `h`'th smallest of the [count] values is the quantile
     * estimate. If not, `h` is an estimation of the index of the p-quantile. Rounding or
     * interpolation needs to occur to get the actual quantile estimate.
     */
    ...
    sealed interface Selecting : QuantileEstimationMethod<Comparable<Any>, Int> {
        ...
        data object R1 : Selecting {
            override fun oneBasedIndexOfQuantile(p: Double, count: Int): Int =
                ceil(p * count).toInt().coerceIn(1..count)
        ...
        data object R3 : Selecting {
            ...
            override fun oneBasedIndexOfQuantile(p: Double, count: Int): Int =
                round(count * p).toInt().coerceIn(1..count)
    ...
    sealed interface Interpolating : QuantileEstimationMethod<Double, Double> {
        ...
        /**
         * Linear interpolation of the modes for the order statistics for the uniform distribution
         * on [0, 1].
         */
        data object R7 : Interpolating, PieceWiseLinear {
            override fun oneBasedIndexOfQuantile(p: Double, count: Int): Double =
                ((count - 1.0) * p + 1.0).coerceIn(1.0..count.toDouble())
        }

        /**
         * Linear interpolation of the approximate medians for order statistics. Recommended by H &
         * F.
         */
        data object R8 : Interpolating, PieceWiseLinear {
            override fun oneBasedIndexOfQuantile(p: Double, count: Int): Double =
                ((count + 1.0 / 3.0) * p + 1.0 / 3.0).coerceIn(1.0..count.toDouble())
        }
...
@Suppress("UNCHECKED_CAST")
internal fun <T : Comparable<T>> QuantileEstimationMethod.Selecting.quantile(
    p: Double,
    values: List<T>,
): T = quantile(p, values as List<Comparable<Any>>) as T

@Suppress("UNCHECKED_CAST")
internal fun <T : Comparable<T>> QuantileEstimationMethod.Selecting.cast():
    QuantileEstimationMethod<T, Int> = this as QuantileEstimationMethod<T, Int>
