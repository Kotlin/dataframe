...
/**
 * Implementation of [AggregatorAggregationHandler] which functions like a selector: it takes a
 * sequence of values and returns a single value, which must exist in the input.
 *
 * This is reflected in the type constraints.
 *
 * @param selector This function actually does the selection. Before it is called, nulls are
 *   filtered out. The type of the values is passed as [KType] to the selector.
 * @param indexOfResult This function must be supplied to give the index of the result in the input
 *   values.
 * @param getReturnType This function must be supplied to give the return type of [selector] given
 *   some input type and whether the input is empty. In practice the return type is always either
...
internal class SelectingAggregationHandler<in Value : Return & Any, out Return : Any?>(
    ...
    override fun calculateReturnType(valueType: KType, emptyInput: Boolean): KType =
        getReturnType(valueType.withNullability(false), emptyInput).also {
            require(
                it == valueType.withNullability(false) || it == valueType.withNullability(true)
            ) {
