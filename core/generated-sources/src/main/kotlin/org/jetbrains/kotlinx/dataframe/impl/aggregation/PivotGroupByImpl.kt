...
internal data class PivotGroupByImpl<T>(
    ...
    override fun <R> aggregate(separate: Boolean, body: AggregateBody<T, R>): DataFrame<T> = df.aggregate {
    ...
    override fun remainingColumnsSelector(): ColumnsSelector<*, *> =
        df.groups.firstOrNull()?.getPivotColumnPaths(columns).orEmpty()
            .let { pivotPaths ->
                {
                    all().except(pivotPaths.toColumnSet() and (df as GroupByImpl).keyColumnsInGroups.toColumnSet())
                }
            }
}
