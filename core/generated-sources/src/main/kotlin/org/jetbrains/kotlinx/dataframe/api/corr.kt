 ...
 * [See `convert` on the documentation website.](https://kotlin.github.io/dataframe/corr.html)
 */
internal interface CorrDocs {
     ...
     * #### For example:
     *
     * <code>`df`</code>`.`[corr][org.jetbrains.kotlinx.dataframe.api.corr]` { length
     * `[and][ColumnsSelectionDsl.and]` age }`
     *
     * <code>`df`</code>`.`[corr][org.jetbrains.kotlinx.dataframe.api.corr]` {
     ...
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; __`.`__[**`with`**][Corr.with]` { columnsSelector:
 ...
 *
 * This function does not compute the correlation immediately. Instead, it defines the primary set
 * of columns and returns a [Corr][org.jetbrains.kotlinx.dataframe.api.Corr] instance that allows
 * configuring how the correlation should be computed.
 *
 * The function is available for numeric- and [Boolean] columns. [Boolean] values are converted into
 * 1 for true and 0 for false. All other columns are ignored. If a
 * [ColumnGroup][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup] instance is passed as the
 * target column for correlation, it will be unpacked into suitable nested columns.
 *
 * The [Corr][org.jetbrains.kotlinx.dataframe.api.Corr] object provides two methods to perform
 * correlation calculations:
 * - [with][org.jetbrains.kotlinx.dataframe.api.Corr.with] — computes correlations between the
 *   initially selected columns and a second set of columns.
 * - [withItself][org.jetbrains.kotlinx.dataframe.api.Corr.withItself] — computes pairwise
 *   correlations within the initially selected columns.
 *
 * Each method returns a square or rectangular correlation matrix represented by a
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame], where rows and columns correspond to the
 * selected column sets, and each cell contains the Pearson correlation coefficient between the
 * corresponding pair of columns.
 *
 * To compute correlations between all suitable columns in the
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame], use
 * [DataFrame.corr()][org.jetbrains.kotlinx.dataframe.DataFrame.corr].
 *
...
/**
 * Calculates the correlation of specified [columns][otherColumns] with values in the columns
 * previously selected with [corr][org.jetbrains.kotlinx.dataframe.api.corr].
 *
 * Returns a correlation matrix represented by a
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame], where rows and columns correspond to the
 * selected column sets, and each cell contains the Pearson correlation coefficient between the
 * corresponding pair of columns.
 *
