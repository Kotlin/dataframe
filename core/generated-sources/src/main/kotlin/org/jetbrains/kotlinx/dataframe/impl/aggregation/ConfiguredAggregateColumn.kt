...
internal class ConfiguredAggregateColumn<C>
private constructor(
    ...
    private fun resolve(
        ...
        return if (resolved.size == 1) {
            ...
            resolved.map { it.toDescriptor(true) }
        }
    }
    ...
    companion object {
        ...
        fun <C> withPath(src: SingleColumn<C>, newPath: ColumnPath): SingleColumn<C> =
            when (src) {
                is ConfiguredAggregateColumn<C> ->
                    ConfiguredAggregateColumn(src.columns, src.default, newPath)
        ...
        fun <C> withDefault(src: SingleColumn<C>, default: C?): SingleColumn<C> =
            when (src) {
                is ConfiguredAggregateColumn<C> ->
                    ConfiguredAggregateColumn(src.columns, default, src.newPath)
