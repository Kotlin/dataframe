...
public inline fun <T, reified V> Pivot<T>.with(
    noinline expression: RowExpression<T, V>
): DataRow<T> = delegate { with(expression) }

// endregion

// region ReducedPivot

public inline fun <T, reified V> ReducedPivot<T>.with(
    noinline expression: RowExpression<T, V>
): DataRow<T> = pivot.delegate { reduce(reducer).with(expression) }

// endregion

// region PivotGroupBy

public inline fun <T, reified V> PivotGroupBy<T>.with(
    noinline expression: RowExpression<T, V>
): DataFrame<T> {
    val type = typeOf<V>()
...
public inline fun <T, reified V> ReducedPivotGroupBy<T>.with(
    noinline expression: RowExpression<T, V>
): DataFrame<T> {
    val type = typeOf<V>()
