 ...
 *
 * Base interface for [ValueColumn] and [FrameColumn], but not for [ColumnGroup]. However,
 * implementations for all three [column kinds][ColumnKind] derive from DataColumn and can cast to
 * it safely. Column operations that have signature clash with [DataFrame] API ([filter], [take],
 * [map] etc.) are defined for [DataColumn] and not for [BaseColumn].
 *
...
public interface DataColumn<out T> : BaseColumn<T> {
    ...
    public companion object {
         ...
         *
         * Be careful; values are NOT checked to adhere to [type] for efficiency, unless you specify
         * [infer].
         *
         ...
         *
         * The column [type] will be defined at compile-time using [T] argument. Be careful with
         * casting; values are NOT checked to adhere to `reified` type [T] for efficiency, unless
         * you specify [infer].
         *
        ...
        public inline fun <reified T> createValueColumn(
            ...
            createValueColumn(name = name, values = values, type = typeOf<T>(), infer = infer)
         ...
         */
        public fun <T> createColumnGroup(name: String, df: DataFrame<T>): ColumnGroup<T> =
            ColumnGroupImpl(name, df)

        /**
         * Creates [FrameColumn] using the given [name] and list of dataframes [groups].
         *
         * [groups] must be a non-null list of [DataFrames][DataFrame], as [FrameColumn] does not
         * allow `null` values. This is NOT checked at runtime for efficiency, nor is the validity
         * of given [schema].
         *
         * @param name name of the frame column
         * @param groups the dataframes to be put in the column
         * @param schema an optional (lazily calculated) [DataFrameSchema] representing the
         *   intersecting schema of [groups]
         */
        ...
        /**
         * Creates either a [FrameColumn], [ColumnGroup], or [ValueColumn] by analyzing each value
         * in [values].
         *
         ...
         *
         * For instance, when there are other [DataFrames][DataFrame] present in [values], we'll
         * convert:
         * - `null` -> [DataFrame.empty]`()`
         * - [DataRow] -> single-row [DataFrame]
         * - [List][List]`<`[DataRow][DataRow]`<*>>` -> multi-row [DataFrame]
         *
         * to be able to create a [FrameColumn]. There are more conversions for other types as well.
         *
         * @param name name of the column
         * @param values the values to represent each row in the column
         * @param suggestedType optional suggested type for values. Default is
         *   [TypeSuggestion.Infer]. See [TypeSuggestion] for more information.
         * @param nullable optionally you can specify whether [values] contains nulls, if `null` it
         *   is inferred.
         */
        ...
        /**
         * Calls [createColumnGroup], [createFrameColumn], or [createValueColumn] based on [type].
         *
         * This may be unsafe but is more efficient than [createByInference].
         *
         * Be careful; Values in [values] are NOT checked to adhere to the given [type], nor do we
         * check whether there are unexpected nulls among the values.
         *
         * It's recommended to use [createValueColumn], [createColumnGroup], and [createFrameColumn]
         * instead.
         *
         * @param name the name of the column
         * @param values the values to represent each row in the column
         * @param type the (unchecked) common type of [values]
         * @param infer in case a [ValueColumn] is created, this controls how/whether types need to
         *   be inferred
         */
        public fun <T> createByType(
            ...
            when (type.toColumnKind()) { // AnyFrame -> Frame, AnyRow? -> Group, else -> Value
                ...
                ColumnKind.Group ->
                    createColumnGroup(name, (values as List<AnyRow?>).concat())
                        .asDataColumn()
                        .cast()

                ColumnKind.Frame ->
                    createFrameColumn(name, values as List<AnyFrame>).asDataColumn().cast()
            }

        /**
         * Calls [createColumnGroup], [createFrameColumn], or [createValueColumn] based on type [T].
         *
         * This is generally safe, as [T] can be inferred by the compiler, and more efficient than
         * [createByInference].
         *
         * Be careful when casting occurs; Values in [values] are NOT checked to adhere to the
         * given/inferred type [T], nor do we check whether there are unexpected nulls among the
         * values.
         *
         * It's recommended to use [createValueColumn], [createColumnGroup], and [createFrameColumn]
         * instead.
        ...
        /**
         * Creates an empty [DataColumn] with given [name] of type [Nothing]. If you want to specify
         * another type, use [`emptyOf<T>()`][emptyOf].
         *
         * @see emptyOf
         */
        ...
        ): FrameColumn<T> = df.chunkedImpl(startIndices = startIndices, name = name)
        ...
        public fun <T> create(
            ...
            type: KType,
            infer: Infer = Infer.None,
        ): DataColumn<T> = createByType(name = name, values = values, type = type, infer = infer)
...
public val AnyCol.name: String
    get() = name()
public val AnyCol.path: ColumnPath
    get() = path()

public val <T> DataColumn<T>.values: Iterable<T>
    get() = values()
public val AnyCol.hasNulls: Boolean
    get() = hasNulls()
public val AnyCol.size: Int
    get() = size()
public val AnyCol.indices: IntRange
    get() = indices()

public val AnyCol.type: KType
    get() = type()
public val AnyCol.kind: ColumnKind
    get() = kind()
public val AnyCol.typeClass: KClass<*>
    get() =
        type.classifier as? KClass<*>
            ?: error(
                "Cannot cast ${type.classifier?.javaClass} to a ${KClass::class}. Column $name: $type"
            )
