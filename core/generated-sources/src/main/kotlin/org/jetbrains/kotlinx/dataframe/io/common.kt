...
/**
 * Opens a stream to [url] to create a [DataFrame] from it. If the URL is a file URL, the file is
 * read directly. If the URL is an HTTP URL, it's also read directly, but if the server returns an
 * error code, the error response is read and parsed as [DataFrame] too.
 *
        ...
        if (code != 200) {
            ...
            try {
                ...
                throw RuntimeException(
                    "Server returned HTTP response code: $code. Response: $response"
                )
            }
        }
        return connection.inputStream.use(body)
...
@Deprecated(
    ...
    ReplaceWith(
        "this.toDataFrame(header = null, containsColumns)",
        "org.jetbrains.kotlinx.dataframe.api.toDataFrame",
    ),
...
public fun isUrl(path: String): Boolean =
    listOf("http:", "https:", "ftp:").any { path.startsWith(it) }
...
/**
 * Converts a file path or URL [String] to a [URL]. If the path is a file path, the file is checked
 * for existence and not being a directory.
 */
