...
public class DataFrameSchemaImpl(override val columns: Map<String, ColumnSchema>) : DataFrameSchema {
    ...
    override fun compare(other: DataFrameSchema, strictlyEqualLeafs: Boolean): CompareResult {
        ...
        columns.forEach {
            ...
            if (otherColumn == null) {
                result = result.combine(if (strictlyEqualLeafs) CompareResult.None else CompareResult.IsDerived)
            } else {
                result = result.combine(it.value.compareStrictlyEqualLeafs(otherColumn))
            }
        ...
        other.columns.forEach {
            ...
            if (thisField == null) {
                result = result.combine(if (strictlyEqualLeafs) CompareResult.None else CompareResult.IsSuper)
                if (result == CompareResult.None) return CompareResult.None
            }
        }
        return result
