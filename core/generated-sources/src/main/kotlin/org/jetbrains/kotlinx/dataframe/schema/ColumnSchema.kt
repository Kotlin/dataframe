...
public sealed class ColumnSchema {
     ...
     *
     * For [<code>Value</code>][Value] the [<code>types</code>][type] are compared, and for [<code>Group</code>][Group] the [<code>DataFrameSchema</code>][DataFrameSchema] of the
     * nested columns. How strict that comparison is, is decided by the [<code>comparisonMode</code>][comparisonMode];
     * see [<code>ComparisonMode</code>][ComparisonMode] for what each mode means.
     *
     * For [<code>Frame</code>][Frame] the [<code>DataFrameSchema</code>][DataFrameSchema] of the dataframes it holds is compared as well, but
     * [<code>nullability</code>][nullable] counts too: two frame column schemas over the same nested schema
     * that differ in nullability never match, whatever the [<code>comparisonMode</code>][comparisonMode] is. The nullable one
     * comes out [<code>IsSuper</code>][CompareResult.IsSuper] of the other, and the other
     * [<code>IsDerived</code>][CompareResult.IsDerived] of it.
     *
     * This comparison ignores the order of the columns inside a nested schema, while [<code>equals</code>][equals]
     * requires the same columns in the same order; see
     * [<code>equals</code>][org.jetbrains.kotlinx.dataframe.impl.schema.DataFrameSchemaImpl.equals]
     * for that difference.
     *
