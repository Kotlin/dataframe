 ...
 *
 * For the second step, a [new][stepTwo] [Aggregator] will be constructed using the supplied
 * handlers: [stepTwoAggregationHandler] and [stepTwoInputHandler]. For both arguments, it holds
 * that if they are not supplied or `null`, the handlers of this [aggregator] will be cast and
 * reused. In all cases [NoMultipleColumnsHandler] will be used as
 * [AggregatorMultipleColumnsHandler].
 *
 * This is useful for aggregators that do not depend on the distribution of values across multiple
 * columns. It may be more memory efficient than [FlatteningMultipleColumnsHandler] and could be
 * parallelized in the future.
 *
 * @param stepTwoAggregationHandler The [aggregation handler][AggregatorAggregationHandler] for the
 *   second step. If not supplied, the handler of the first step is reused.
 * @param stepTwoInputHandler The [input handler][AggregatorInputHandler] for the second step. If
 *   not supplied, the handler of the first step is reused.
...
internal class TwoStepMultipleColumnsHandler<in Value : Any, Return : Any?>(
    ...
    /**
     * The second step [Aggregator] which can take multiple outputs of this [aggregator] and
     * aggregate it to a single value.
     */
    ...
    val stepTwo by lazy {
        Aggregator(
                ...
                inputHandler =
                    stepTwoInputHandler
                        ?: aggregator as AggregatorInputHandler<Return & Any, Return>,
     ...
     *
     * This function calls [aggregator][aggregator]
     * [.aggregateSingleColumn()][Aggregator.aggregateSingleColumn] on each column and then
     * [stepTwo] [.aggregateSequence()][Aggregator.aggregateSequence] on the results.
     */
    ...
    /**
     * Function that can give the return type of [aggregateMultipleColumns], given types of the
     * columns. This allows aggregators to avoid runtime type calculations.
     *
     * @param colTypes The types of the input columns.
     * @param colsEmpty If `true`, all the input columns are considered empty. This often affects
     *   the return type.
     */
    @Suppress("UNCHECKED_CAST")
    override fun calculateReturnTypeMultipleColumns(
        colTypes: Set<KType>,
        colsEmpty: Boolean,
    ): KType {
