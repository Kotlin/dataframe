...
internal class ColumnGroupWithParent<T>(
    ...
    override fun getValue(thisRef: Any?, property: KProperty<*>): DataColumnGroup<T> =
        source.getValue(thisRef, property) as DataColumnGroup<T>
    ...
    override operator fun <R> get(column: KProperty<DataFrame<R>>): FrameColumn<R> =
        get(column.columnName).asAnyFrameColumn().castFrameColumn()

    override operator fun <C> get(columns: ColumnsSelector<T, C>): List<DataColumn<C>> = super.get(columns).map {
        it.addParent(this)
    }
    ...
    override fun get(firstIndex: Int, vararg otherIndices: Int) =
        ColumnGroupWithParent(parent, super.get(firstIndex, *otherIndices))
}
