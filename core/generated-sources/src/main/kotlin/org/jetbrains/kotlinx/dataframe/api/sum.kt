...
/**
 * Returns the sum of the values of each selected column of this [<code>DataFrame</code>][DataFrame] separately.
 *
 *
 *
 * All primitive number types are supported: [<code>Byte</code>][Byte], [<code>Short</code>][Short], [<code>Int</code>][Int], [<code>Long</code>][Long], [<code>Float</code>][Float], and [<code>Double</code>][Double].
 ...
 *
 * See [<code>Selecting Columns</code>][SumDocs.SumForSelectingOptions].
 *
 * See also:
 * - [<code>`sum`</code>][DataFrame.sum]`()` — the same, but for all suitable columns at once.
 * - [<code>`sum`</code>][DataFrame.sum]` { columns }` — a single sum of all values in the selected columns.
 * - [<code>`meanFor`</code>][DataFrame.meanFor] — the sum of each selected column divided by its number of values.
 * - [<code>The Sum Operation</code>][org.jetbrains.kotlinx.dataframe.api.SumDocs] — an overview of all `sum` modes.
 *
 * For more information: [See `sum` on the documentation website.](https://kotlin.github.io/dataframe/sum.html)
 *
 ...
 * ```
 *
 * @param [columns] The names of the columns of this [<code>DataFrame</code>][DataFrame] to compute the sum of.
 *
 *
 * @param [skipNaN] If `true`, [<code>`NaN`</code>][org.jetbrains.kotlinx.dataframe.documentation.NaN] values are ignored, just like `null` values.
 *   If `false` (the default), a [<code>`NaN`</code>][org.jetbrains.kotlinx.dataframe.documentation.NaN] in the input is propagated to the result.
 ...
 *
 * See also:
 * - [<code>`sumFor`</code>][DataFrame.sumFor] — the sum of each selected column separately.
 * - [<code>`sumOf`</code>][DataFrame.sumOf] — the sum of the values a row expression returns for each row.
 * - [<code>The Sum Operation</code>][org.jetbrains.kotlinx.dataframe.api.SumDocs] — an overview of all `sum` modes.
 *
 * For more information: [See `sum` on the documentation website.](https://kotlin.github.io/dataframe/sum.html)
 *
 ...
 * ```
 *
 * @param [expression] The [<code>RowExpression</code>][RowExpression] to compute the value to sum for each row.
 * @return The sum of the values [<code>expression</code>][expression] returns, as an [<code>Int</code>][Int].
 */
@JvmName("sumOfShort")
@OverloadResolutionByLambdaReturnType
 ...
 *
 * See also:
 * - [<code>`sum`</code>][Grouped.sum]`()` — the same, but for all suitable columns at once.
 * - [<code>`sum`</code>][Grouped.sum]` { columns }` — a single sum of all values in the selected columns, per group.
 * - [<code>`meanFor`</code>][Grouped.meanFor] — the sum of each selected column divided by its number of values,
 *   per group.
 * - [<code>`aggregate`</code>][Grouped.aggregate] — the general way to aggregate groups.
 * - [<code>The Sum Operation</code>][org.jetbrains.kotlinx.dataframe.api.SumDocs] — an overview of all `sum` modes.
 *
 ...
 *   If `false` (the default), a [<code>`NaN`</code>][org.jetbrains.kotlinx.dataframe.documentation.NaN] in the input is propagated to the result.
 *   This only has an effect on [<code>Double</code>][Double] and [<code>Float</code>][Float] values.
 * @param [columns] The [<code>ColumnsForAggregateSelector</code>][ColumnsForAggregateSelector] used to select the columns to compute the sum of.
...
/**
 * Aggregates this [<code>GroupBy</code>][GroupBy] by computing a single sum of all the values
 ...
 * This can include [<code>column groups</code>][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup] and nested columns.
 *
 * See [<code>Selecting Columns</code>][SumDocs.SumSelectingOptions].
 *
 * See also:
 * - [<code>`sumFor`</code>][Grouped.sumFor] — the sum of each selected column separately, per group.
 * - [<code>`sumOf`</code>][Grouped.sumOf] — the sum of the values a row expression returns
 ...
 *   If `false` (the default), a [<code>`NaN`</code>][org.jetbrains.kotlinx.dataframe.documentation.NaN] in the input is propagated to the result.
 *   This only has an effect on [<code>Double</code>][Double] and [<code>Float</code>][Float] values.
 * @param [columns] The [<code>ColumnsSelector</code>][ColumnsSelector] used to select the columns to compute the sum of.
 ...
 * This can include [<code>column groups</code>][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup] and nested columns.
 *
 * See [<code>Selecting Columns</code>][SumDocs.SumForSelectingOptions], or check out the
 * [<code>`Pivot` Grammar</code>][PivotDocs.Grammar].
 *
 * See also:
 * - [<code>`sum`</code>][Pivot.sum]`()` — the same, but for all suitable columns at once.
 * - [<code>`sum`</code>][Pivot.sum]` { columns }` — a single sum of all values in the selected columns, per group.
 * - [<code>Pivot aggregation</code>][PivotDocs.Aggregation] — all other ways to aggregate a [<code>Pivot</code>][Pivot].
 * - [<code>The Sum Operation</code>][org.jetbrains.kotlinx.dataframe.api.SumDocs] — an overview of all `sum` modes.
 *
...
/**
 * Aggregates this [<code>Pivot</code>][Pivot] by computing a single sum of all the values
 * in the selected columns, per group.
 *
 * Returns a single [<code>DataRow</code>][DataRow] with the [<code>pivot</code>][pivot] keys as (nested) columns, containing the sum of all
 ...
 * This can include [<code>column groups</code>][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup] and nested columns.
 *
 * See [<code>Selecting Columns</code>][SumDocs.SumSelectingOptions], or check out the
 * [<code>`Pivot` Grammar</code>][PivotDocs.Grammar].
 *
 * See also:
 * - [<code>`sum`</code>][Pivot.sum]`()` — the sum of each suitable column separately, per group.
 * - [<code>`sumFor`</code>][Pivot.sumFor] — the sum of each selected column separately, per group.
 * - [<code>Pivot aggregation</code>][PivotDocs.Aggregation] — all other ways to aggregate a [<code>Pivot</code>][Pivot].
 * - [<code>The Sum Operation</code>][org.jetbrains.kotlinx.dataframe.api.SumDocs] — an overview of all `sum` modes.
 *
 * For more information: [See "`pivot` statistics" on the documentation website.](https://kotlin.github.io/dataframe/summarystatistics.html#pivot-statistics),
 ...
 * each selected column separately, per group.
 *
 * Returns a [<code>DataFrame</code>][DataFrame] where each cell contains the sum of each selected column
 * of the group corresponding to that [<code>pivot</code>][pivot] key (column) and [<code>groupBy</code>][groupBy] key (row).
 *
 *
 *
 ...
 *
 * See also:
 * - [<code>`sum`</code>][PivotGroupBy.sum]`()` — the same, but for all suitable columns at once.
 * - [<code>`sum`</code>][PivotGroupBy.sum]` { columns }` — a single sum of all values in the selected columns,
 *   per group.
 * - [<code>PivotGroupBy aggregation</code>][PivotGroupByDocs.Aggregation] — all other ways to aggregate
 *   a [<code>PivotGroupBy</code>][PivotGroupBy].
...
/**
 * Aggregates this [<code>PivotGroupBy</code>][PivotGroupBy] by computing a single sum of all the values
 * in the selected columns, per group.
 *
 * Returns a [<code>DataFrame</code>][DataFrame] where each cell contains the sum of all the values in the selected columns
 * of the group corresponding to that [<code>pivot</code>][pivot] key (column) and [<code>groupBy</code>][groupBy] key (row).
 *
 *
 *
 ...
 *
 * See also:
 * - [<code>`sum`</code>][PivotGroupBy.sum]`()` — the sum of each suitable column separately, per group.
 * - [<code>`sumFor`</code>][PivotGroupBy.sumFor] — the sum of each selected column separately, per group.
 * - [<code>PivotGroupBy aggregation</code>][PivotGroupByDocs.Aggregation] — all other ways to aggregate
 *   a [<code>PivotGroupBy</code>][PivotGroupBy].
 * - [<code>The Sum Operation</code>][org.jetbrains.kotlinx.dataframe.api.SumDocs] — an overview of all `sum` modes.
 *
