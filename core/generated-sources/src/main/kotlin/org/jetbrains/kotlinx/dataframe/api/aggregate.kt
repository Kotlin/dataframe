 ...
 *
 * [AggregateDsl] allows to compute statistics on the columns within groups in [Pivot] and store the
 * results as a new column using
 ...
 *
 * The resulting [DataRow] has the same structure as the original [Pivot]; instead of the groups,
 * there are new columns of aggregated values created with
 ...
 * [DataRow] while preserving the original structure at higher levels.
 *
 ...
 *
 * [AggregateGroupedDsl][org.jetbrains.kotlinx.dataframe.aggregation.AggregateGroupedDsl] allows to
 * compute statistics on the columns within groups in
 * [GroupBy][org.jetbrains.kotlinx.dataframe.api.GroupBy] and store the results as a new column
 * using [into][org.jetbrains.kotlinx.dataframe.aggregation.AggregateDsl.into]. The given
 * [aggregating expression][body] is applied to each group independently.
 *
 * The resulting [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] has the same structure as
 * the original [GroupBy][org.jetbrains.kotlinx.dataframe.api.GroupBy]; instead of the groups, there
 * are new columns of aggregated values created with
 * [into][org.jetbrains.kotlinx.dataframe.aggregation.AggregateDsl.into].
 *
 * You can use any of
 * [DataFrame Aggregation Statistics][org.jetbrains.kotlinx.dataframe.aggregation.DataFrameAggregationStatistics]
 ...
 * group][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup] or
 * [frame column][org.jetbrains.kotlinx.dataframe.columns.FrameColumn] respectively in the resulting
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] while preserving the original structure at
 * higher levels.
 *
 * It allows [pivoting][org.jetbrains.kotlinx.dataframe.DataFrame.pivot] inside
 * [aggregate][org.jetbrains.kotlinx.dataframe.api.Grouped.aggregate] via corresponding methods:
 ...
 *
 * Pivoting inside [aggregate][org.jetbrains.kotlinx.dataframe.api.Grouped.aggregate] is useful for
 * counting cross-group matrix-like statistics.
 *
