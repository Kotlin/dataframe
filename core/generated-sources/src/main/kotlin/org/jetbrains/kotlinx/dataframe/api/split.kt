...
public fun <T> DataFrame<T>.split(vararg columns: String): Split<T, Any> = split {
    columns.toColumnSet()
}

@Deprecated(DEPRECATED_ACCESS_API)
@AccessApiOverload
public fun <T, C> DataFrame<T>.split(vararg columns: ColumnReference<C?>): Split<T, C> = split {
    columns.toColumnSet()
}

@Deprecated(DEPRECATED_ACCESS_API)
@AccessApiOverload
public fun <T, C> DataFrame<T>.split(vararg columns: KProperty<C?>): Split<T, C> = split {
    columns.toColumnSet()
}
...
public inline fun <T, C : Iterable<R>, reified R> Split<T, C>.default(
    value: R?
): SplitWithTransform<T, C, R> = by { it }.default(value)
...
public fun <T, C> Split<T, C>.by(
    ...
    it.toString().split(regex, limit = limit).let { if (trim) it.map { it.trim() } else it }
}
...
/**
 * Creates new String columns according to MatchResult
 * [capturing groups](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin.text/-match-result/group-values.html),
 * excluding the first group which is entire matched String. Example:
 * ```
 * dataFrameOf("str" to listOf("100 ml", "1 L"))
 *      .split { "str"<String>() }.match("(\d+)\s*(ml|l|L)").into("volume", "unit")
 * ```
 *
 * Created columns will be nullable if [regex] doesn't match some rows or there are nulls in
 * original column Check [Split.by] overload with regex parameter if you're looking to split String
 * value by [Regex] delimiter
 */
 ...
 *
 * Created columns will be nullable if [regex] doesn't match some rows or there are nulls in
 * original column Check [Split.by][org.jetbrains.kotlinx.dataframe.api.Split.by] overload with
 * regex parameter if you're looking to split String value by [Regex] delimiter
 */
...
@Deprecated(
    SPLIT_STR,
    ReplaceWith("""by(",").into(*names, extraNamesGenerator = extraNamesGenerator)"""),
)
@JvmName("intoTC")
...
public fun <T, C, R> SplitWithTransform<T, C, R>.inward(
    ...
    SplitWithTransform(df, columns, inward = true, tartypeOf, default, transform)
...
@Deprecated(
    SPLIT_STR,
    ReplaceWith("""by(",").inward(*names, extraNamesGenerator = extraNamesGenerator)"""),
)
@JvmName("inwardTC")
...
public inline fun <T, C : Iterable<R>, reified R> Split<T, C>.intoRows(
    dropEmpty: Boolean = true
): DataFrame<T> = by { it }.intoRows(dropEmpty)
...
internal inline fun <T, C, R> Convert<T, C?>.splitInplace(
    ...
    withRowCellImpl(getListType(type), Infer.None) {
        if (it == null) emptyList() else transform(it).asList()
    }
...
public fun <T, C, R> SplitWithTransform<T, C, R>.intoRows(dropEmpty: Boolean = true): DataFrame<T> {
    ...
    return df.convert { paths as ColumnSet<C?> }
        .splitInplace(tartypeOf, transform)
        .explode(dropEmpty) { paths }
}
...
public inline fun <T, C : Iterable<R>, reified R> Split<T, C>.inplace(): DataFrame<T> =
    by { it }.inplace()
