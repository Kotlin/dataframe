...
/**
 * Joins this [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with the [other][other]
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] using the selected key columns.
 *
 * Creates a new [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] by combining
 * [rows][org.jetbrains.kotlinx.dataframe.DataRow] from two input dataframes according to one or
 * more matching key columns.
 *
 * There are two categories of joins:
 * * **Merging joins** — merge matching rows from both
 *   [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s into a single row.
 * * **Non-merging joins** — select rows from the left
 *   [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] based on whether a match exists in the
 *   right one, without merging columns.
 *
 * The exact behavior depends on the specified [join type][type]:
 *
 * **Merging joins:**
 * * [JoinType.Inner][org.jetbrains.kotlinx.dataframe.api.JoinType.Inner] (default) — Includes only
 *   matching rows from both [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s; rows are
 *   merged.
 * * [JoinType.Left][org.jetbrains.kotlinx.dataframe.api.JoinType.Left] — Includes all rows from the
 *   left [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]; matching rows are merged,
 *   unmatched right-side values are filled with `null`.
 * * [JoinType.Right][org.jetbrains.kotlinx.dataframe.api.JoinType.Right] — Includes all rows from
 *   the right [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]; matching rows are merged,
 *   unmatched left-side values are filled with `null`.
 * * [JoinType.Full][org.jetbrains.kotlinx.dataframe.api.JoinType.Full] — Includes all rows from
 *   both [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s; matching rows are merged, all
 *   mismatches are filled with `null`.
 *
 * **Non-merging joins:**
 * * [JoinType.Filter][org.jetbrains.kotlinx.dataframe.api.JoinType.Filter] — Includes only rows
 *   from the left [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] that have a match in the
 *   right one; right-side columns are not merged.
 * * [JoinType.Exclude][org.jetbrains.kotlinx.dataframe.api.JoinType.Exclude] — Includes only rows
 *   from the left [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] that do *not* have a match
 *   in the right one; right-side columns are not merged.
 *
 * If no join columns are specified, all columns with matching names in both
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s are used.
 *
 * If both [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s contain columns with the same
 * name that are *not* part of the join keys, such columns are treated as distinct. Such a column
 * from the right [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] will be
 * [automatically renamed][`AutoRenamingColumnsInDataFrame`] in the resulting
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame].
 *
 * Each join type has a corresponding shortcut function:
 * [innerJoin][org.jetbrains.kotlinx.dataframe.api.innerJoin],
 * [leftJoin][org.jetbrains.kotlinx.dataframe.api.leftJoin],
 * [rightJoin][org.jetbrains.kotlinx.dataframe.api.rightJoin],
 * [fullJoin][org.jetbrains.kotlinx.dataframe.api.fullJoin],
 * [filterJoin][org.jetbrains.kotlinx.dataframe.api.filterJoin], and
 * [excludeJoin][org.jetbrains.kotlinx.dataframe.api.excludeJoin].
 *
 * See also [joinWith][org.jetbrains.kotlinx.dataframe.api.joinWith], which performs a join by
 * matching row values condition.
 *
 ...
 * [See `join` on the documentation website.](https://kotlin.github.io/dataframe/join.html).
 *
 * ### This `join` overload
 * Select join columns (including those that have different names in different
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s) using
 * [JoinDsl][org.jetbrains.kotlinx.dataframe.api.JoinDsl].
 *
 * [JoinDsl][org.jetbrains.kotlinx.dataframe.api.JoinDsl] allows you to define the columns used for
 * joining [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s and provides methods to match
 * columns with different names between the left and right side.
 *
 * Provides the left [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] both as the receiver
 * (`this`) and as the argument (`it`), allowing you to reference its columns directly. Use
 * [right][org.jetbrains.kotlinx.dataframe.api.JoinDsl.right] to access columns from the right
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame], and
 * [match][org.jetbrains.kotlinx.dataframe.api.JoinDsl.match] to explicitly pair columns with
 * different names.
 *
 * See also
 * [Columns selection via DSL][org.jetbrains.kotlinx.dataframe.documentation.SelectingColumns.ColumnsSelectionDsl].
 *
 * ### Examples
 *
 * ```kotlin
...
/**
 * Performs an [inner join][org.jetbrains.kotlinx.dataframe.api.JoinType.Inner] of this
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with the [other][other]
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] using the selected key columns. Includes
 * only matching rows from both [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s; rows are
 * merged.
 *
 * This is a shortcut for [join][org.jetbrains.kotlinx.dataframe.api.join] with
 * [JoinType.Inner][org.jetbrains.kotlinx.dataframe.api.JoinType.Inner].
 *
 * If no join columns are specified, all columns with matching names in both
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s are used.
 *
 ...
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame].
 *
 * See also general [join][org.jetbrains.kotlinx.dataframe.api.join], as well as other shortcuts
 * with each of join types: [leftJoin][org.jetbrains.kotlinx.dataframe.api.leftJoin],
 * [rightJoin][org.jetbrains.kotlinx.dataframe.api.rightJoin],
 * [fullJoin][org.jetbrains.kotlinx.dataframe.api.fullJoin],
 * [filterJoin][org.jetbrains.kotlinx.dataframe.api.filterJoin],
 * [excludeJoin][org.jetbrains.kotlinx.dataframe.api.excludeJoin].
 *
 * This can include [column groups][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup] and nested
 * columns.
...
/**
 * Performs a [left join][org.jetbrains.kotlinx.dataframe.api.JoinType.Left] of this
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with the [other][other]
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] using the selected key columns. Includes
 * all rows from the left [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]; matching rows are
 * merged, unmatched right-side values are filled with `null`.
 *
 * This is a shortcut for [join][org.jetbrains.kotlinx.dataframe.api.join] with
 * [JoinType.Left][org.jetbrains.kotlinx.dataframe.api.JoinType.Left].
 *
 * If no join columns are specified, all columns with matching names in both
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s are used.
 *
 ...
 *
 * See also general [join][org.jetbrains.kotlinx.dataframe.api.join], as well as other shortcuts
 * with each of join types: [innerJoin][org.jetbrains.kotlinx.dataframe.api.innerJoin],
 * [rightJoin][org.jetbrains.kotlinx.dataframe.api.rightJoin],
 * [fullJoin][org.jetbrains.kotlinx.dataframe.api.fullJoin],
 * [filterJoin][org.jetbrains.kotlinx.dataframe.api.filterJoin],
 * [excludeJoin][org.jetbrains.kotlinx.dataframe.api.excludeJoin].
...
/**
 * Performs a [right join][org.jetbrains.kotlinx.dataframe.api.JoinType.Right] of this
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with the [other][other]
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] using the selected key columns. Includes
 * all rows from the right [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]; matching rows are
 * merged, unmatched left-side values are filled with `null`.
 *
 * This is a shortcut for [join][org.jetbrains.kotlinx.dataframe.api.join] with
 * [JoinType.Right][org.jetbrains.kotlinx.dataframe.api.JoinType.Right].
 *
 * If no join columns are specified, all columns with matching names in both
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s are used.
 *
...
/**
 * Performs a [full join][org.jetbrains.kotlinx.dataframe.api.JoinType.Full] of this
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with the [other][other]
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] using the selected key columns. Includes
 * all rows from both [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s; matching rows are
 * merged, all mismatches are filled with `null`.
 *
 * This is a shortcut for [join][org.jetbrains.kotlinx.dataframe.api.join] with
 * [JoinType.Full][org.jetbrains.kotlinx.dataframe.api.JoinType.Full].
 *
 * If no join columns are specified, all columns with matching names in both
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s are used.
 *
...
/**
 * Performs a [filter join][org.jetbrains.kotlinx.dataframe.api.JoinType.Filter] of this
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with the [other][other]
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] using the selected key columns. Includes
 * only rows from the left [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] that have a match
 * in the right one; right-side columns are not merged.
 *
 * This is a shortcut for [join][org.jetbrains.kotlinx.dataframe.api.join] with
 * [JoinType.Filter][org.jetbrains.kotlinx.dataframe.api.JoinType.Filter].
 *
 * If no join columns are specified, all columns with matching names in both
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s are used.
 *
...
/**
 * Performs an [exclude join][org.jetbrains.kotlinx.dataframe.api.JoinType.Exclude] of this
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with the [other][other]
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] using the selected key columns. Includes
 * only rows from the left [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] that do *not* have
 * a match in the right one; right-side columns are not merged.
 *
 * This is a shortcut for [join][org.jetbrains.kotlinx.dataframe.api.join] with
 * [JoinType.Exclude][org.jetbrains.kotlinx.dataframe.api.JoinType.Exclude].
 *
 * If no join columns are specified, all columns with matching names in both
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s are used.
 *
...
public interface JoinDsl<out A, out B> : ColumnsSelectionDsl<A> {
    ...
    /**
     * Matches columns from the left and right
     * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame]s for
     * [joining][org.jetbrains.kotlinx.dataframe.api.join].
     *
     * The receiver column must belong to the left
     * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame], and the argument ([other]) column
     * must belong to the right [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame].
     *
     * @param [other] column from the right [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame].
     * @return [ColumnMatch][org.jetbrains.kotlinx.dataframe.api.ColumnMatch] representing the
     *   column pair used for joining.
