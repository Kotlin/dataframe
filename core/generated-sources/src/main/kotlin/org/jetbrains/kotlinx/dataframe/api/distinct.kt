 ...
 *
 * The [rows][org.jetbrains.kotlinx.dataframe.DataRow] in the resulting
 * [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] are in the same order as they were in the
 * original [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame].
 *
 * See also [distinctBy][org.jetbrains.kotlinx.dataframe.api.distinctBy] that removes duplicated
 * rows based on the specified columns and keeps all the columns in the resulting
 ...
 *
 * [See `distinctBy` on the documentation
 * website.](https://kotlin.github.io/dataframe/distinct.html#distinctby)
 *
 * @return A new [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] containing only distinct
 *   rows.
 */
 ...
 * website.](https://kotlin.github.io/dataframe/distinct.html#distinctby)
 *
 * @param [columns] The [ColumnsSelector] used to select columns that will be included in the
 *   resulting [DataFrame] and considered for evaluating distinct rows.
 * @return A new [DataFrame] containing only selected columns and distinct rows.
 */
@Refine
@Interpretable("Distinct0")
public fun <T, C> DataFrame<T>.distinct(columns: ColumnsSelector<T, C>): DataFrame<T> =
 ...
 * website.](https://kotlin.github.io/dataframe/distinct.html#distinctby)
 *
 * @param [columns] The names of the columns to select and to consider for evaluating distinct rows.
 * @return A new [DataFrame] containing only selected columns and distinct rows.
 */
public fun <T> DataFrame<T>.distinct(vararg columns: String): DataFrame<T> = distinct {
    columns.toColumnSet()
}

@Deprecated(DEPRECATED_ACCESS_API)
@AccessApiOverload
public fun <T> DataFrame<T>.distinct(vararg columns: AnyColumnReference): DataFrame<T> = distinct {
    columns.toColumnSet()
}

@Deprecated(DEPRECATED_ACCESS_API)
@AccessApiOverload
public fun <T> DataFrame<T>.distinctBy(vararg columns: KProperty<*>): DataFrame<T> = distinctBy {
    columns.toColumnSet()
}
 ...
 * original [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame].
 *
 * See also [distinct] that selects the specified columns (if the columns are not specified, selects
 * all columns) and keeps only distinct rows based on these selected columns.
 *
 * This can include [column groups][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup] and nested
 * columns.
 *
 ...
 *   rows.
 */
public fun <T> DataFrame<T>.distinctBy(vararg columns: String): DataFrame<T> = distinctBy {
    columns.toColumnSet()
}
 ...
 * website.](https://kotlin.github.io/dataframe/distinct.html#distinctby)
 *
 * @param [columns] The [ColumnsSelector] used to select columns that will be considered for
 *   evaluating distinct rows.
 * @return A new [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] containing only distinct
 *   rows.
 */
...
public interface DistinctColumnsSelectionDsl {
     ...
     *
     * This doesn't solve [DuplicateColumnNamesException] if you've selected two columns with the
     * same name. For this, you'll need to [rename][ColumnsSelectionDsl.named] one of the columns.
     *
     * See also [Grammar], [named][ColumnsSelectionDsl.named],
