 ...
 *
 * See also:
 * - [<code>add</code>][DataFrame.add] — computes a new column in the same way, and returns the [<code>DataFrame</code>][DataFrame]
 ...
 * A [<code>ValueColumn</code>][org.jetbrains.kotlinx.dataframe.columns.ValueColumn] can never have a non-nullable [<code>DataFrame</code>][org.jetbrains.kotlinx.dataframe.DataFrame] type,
 * so a call that would give it one fails with an [<code>IllegalArgumentException</code>][IllegalArgumentException].
 * That happens when the computed values are dataframes and [<code>Infer.Type</code>][org.jetbrains.kotlinx.dataframe.api.Infer.Type] derives a [<code>DataFrame</code>][org.jetbrains.kotlinx.dataframe.DataFrame] type for them,
 * and also under a nullable [<code>DataFrame</code>][org.jetbrains.kotlinx.dataframe.DataFrame] type when none of the computed values is `null`:
 * the default [<code>Infer.Nulls</code>][org.jetbrains.kotlinx.dataframe.api.Infer.Nulls] then drops the nullability and leaves exactly that forbidden type.
 * With at least one `null` among them the same call succeeds
 * and gives a [<code>ValueColumn</code>][org.jetbrains.kotlinx.dataframe.columns.ValueColumn] of the nullable [<code>DataFrame</code>][org.jetbrains.kotlinx.dataframe.DataFrame] type.
 *
 * See also:
 ...
 * With [<code>Infer.Type</code>][org.jetbrains.kotlinx.dataframe.api.Infer.Type] the [<code>type</code>][org.jetbrains.kotlinx.dataframe.DataColumn.type] of a [<code>ValueColumn</code>][org.jetbrains.kotlinx.dataframe.columns.ValueColumn] comes from the computed values instead,
