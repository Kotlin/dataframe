...
/**
 * Specifies how to handle columns in original dataframe that were not matched to any column in
 * destination dataframe schema.
 */
public enum class ExcessiveColumns {
    ...
    /**
     * Throw [ExcessiveColumnsException] if any excessive columns were found in the original
     * dataframe
     */
    Fail,
}
...
public interface ConvertSchemaDsl<in T> {
     ...
     *
     * Note: In most cases using `convert<Type>().with { }` is more convenient, however if you only
     * have [KType], this method can be used.
     */
    public fun <A, B> convert(from: KType, to: KType, converter: (A) -> B)

    /**
     * Advanced version of [convert]. If you want to define a common conversion for multiple types
     * (or any type), or you need extra information about the target, such as its schema, use this
     * method.
     *
     * The exact type conversion does have higher priority. After that, this flexible conversions
     * will be checked in order.
     *
     * @param condition a function that should return `true` if the conversion should be applied
     *   from the given `fromType` to the given `toSchema`.
...
/**
 * Defines how to fill specified columns in destination schema that were not found in original
 * dataframe. All [fill] operations for missing columns are executed after successful conversion of
 * matched columns, so converted values of matched columns can be safely used in [with] expression.
 *
...
public fun <T, C> ConvertToFill<T, C>.with(expr: RowExpression<T, C>) {
    (dsl as ConvertSchemaDslInternal<T>).fill(
        columns as ColumnsSelector<*, C>,
        expr as RowExpression<*, C>,
    )
}
...
public inline fun <reified C> ConvertSchemaDsl<*>.convert(): ConvertType<C> =
    ConvertType(this, typeOf<C>())
 ...
 * @throws [ColumnNotFoundException] if [DataFrame] doesn't contain columns that are required by
 *   destination schema.
 * @throws [ExcessiveColumnsException] if [DataFrame] contains columns that are not required by
 *   destination schema and [excessiveColumnsBehavior] is set to [ExcessiveColumns.Fail].
 * @throws [TypeConverterNotFoundException] if suitable type converter for some column was not
 *   found.
