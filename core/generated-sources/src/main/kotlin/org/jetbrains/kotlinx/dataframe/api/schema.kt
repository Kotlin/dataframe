 ...
 * ### Example
 *
 * The examples below use the same [<code>DataFrame</code>][org.jetbrains.kotlinx.dataframe.DataFrame] as the
 * [`schema` page on the documentation website](https://kotlin.github.io/dataframe/schema.html):
 ...
 * ```
 *
 * Written out, a [<code>column group</code>][ColumnGroup] is shown by indentation:
 *
 * ```text
 * name:
 *     firstName: String
 ...
 * ```
 *
 * A [<code>frame column</code>][FrameColumn] is shown by `*`, with the columns its dataframes share
 * indented under it:
 *
 * ```kotlin
 ...
 * the key columns, followed by a [<code>frame column</code>][FrameColumn] holding the groups.
 *
 * This is the schema of [<code>toDataFrame</code>][GroupBy.toDataFrame], so the group column carries its
 ...
 * The columns are the properties of [<code>T</code>][T], whether or not it carries the [<code>DataSchema</code>][DataSchema]
 * annotation. A type with no properties therefore gives a schema with no columns —
 * as does `DataFrame<*>`, where [<code>T</code>][T] is `Any?`.
 *
 * A [<code>DataFrame</code>][org.jetbrains.kotlinx.dataframe.DataFrame] has two schemas that do not have to agree:
 * the **runtime** one, read from the columns it holds, and the **compile-time** one,
 * read from the type argument `T` of `DataFrame<T>`.
 ...
 *   when [<code>T</code>][T] has a primary constructor, or a single constructor, its parameter order wins.
 ...
 *   reflection reports the properties of [<code>T</code>][T] in, which is neither their declaration order
