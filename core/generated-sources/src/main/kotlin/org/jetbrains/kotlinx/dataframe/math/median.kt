 ...
 * - (lower) middle else
 *
 * Based on quantile implementation; uses [QuantileEstimationMethod.R8] for primitive numbers, else
 * [QuantileEstimationMethod.R3].
 */
@PublishedApi
internal fun <T : Comparable<T>> Sequence<T>.medianOrNull(type: KType, skipNaN: Boolean): Any? {
    when {
        type.isMarkedNullable ->
            error(
                "Encountered nullable type ${renderType(type)} in median function. This should not occur."
            )
...
/** Primitive Number -> Double? T : Comparable<T> -> T? */
 ...
 * - index (lower) middle else NOTE: For primitive numbers the `seq.elementAt(seq.indexOfMedian())`
 *   might be different from `seq.medianOrNull()`
 *
 * Based on quantile implementation; uses [QuantileEstimationMethod.R3].
 */
internal fun <T : Comparable<T & Any>?> Sequence<T>.indexOfMedian(
    type: KType,
    skipNaN: Boolean,
): Int {
