 ...
 *
 * Every appended value must be compatible with the corresponding column. A [<code>ValueColumn</code>][ValueColumn] accepts `null` or a value
 * of its declared type. A [<code>ColumnGroup</code>][ColumnGroup] accepts `null`, a [<code>DataRow</code>][DataRow], or a [<code>List</code>][List]. Values from a [<code>DataRow</code>][DataRow] are matched
 * to the group's columns by name: a group column whose name is absent from the row receives `null`, while columns
 * present only in the [<code>DataRow</code>][DataRow] are ignored. Values from a [<code>List</code>][List] are read in the group's column order, one value per
 * column in the [<code>ColumnGroup</code>][ColumnGroup]. The list must contain at least as many values as the [<code>ColumnGroup</code>][ColumnGroup] has columns:
 * a shorter list throws [<code>IndexOutOfBoundsException</code>][IndexOutOfBoundsException], while additional values in a longer list are ignored.
 * A [<code>FrameColumn</code>][FrameColumn] accepts `null` or a [<code>DataFrame</code>][DataFrame].
 *
 ...
 *
 * Appending `null` widens the runtime type of each affected [<code>ValueColumn</code>][org.jetbrains.kotlinx.dataframe.columns.ValueColumn] to nullable, while the compile-time schema
 * of the returned [<code>DataFrame</code>][org.jetbrains.kotlinx.dataframe.DataFrame] is not refined. Use [<code>cast</code>][org.jetbrains.kotlinx.dataframe.api.cast] or [<code>convertTo</code>][org.jetbrains.kotlinx.dataframe.api.convertTo] with a nullable schema before accessing these
 ...
 * [<code>values</code>][values] is not a multiple of the number of columns, or a value has a type incompatible with the corresponding
 * column.
 * @throws [IndexOutOfBoundsException] if a [<code>List</code>][List] supplied for a [<code>ColumnGroup</code>][ColumnGroup] contains fewer values than the
 * [<code>ColumnGroup</code>][ColumnGroup] has columns.
 */
