 ...
 *
 * Returns the [DataFrame] with changed values in some cells (column types cannot be changed).
 *
 * ### Check out: [Grammar]
 *
...
public class Update<T, C>(
     ...
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `[ `__`.`__[**`where`**][Update.where]**` {
     * `**[`rowValueCondition`][SelectingRows.RowValueCondition.WithExample]**` } `**`]`
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `[
     ...
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; __`.`__[**`with`**][Update.with]**` {
     * `**[`rowExpression`][ExpressionsGivenRow.RowValueExpression.WithExample]**` }`**
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `| `__`.`__[**`notNull`**][Update.notNull]**` {
     * `**[`rowExpression`][ExpressionsGivenRow.RowValueExpression.WithExample]**` }`**
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `| `__`.`__[**`perCol`**][Update.perCol]**` {
     * `**[`colExpression`][ExpressionsGivenColumn.ColumnExpression.WithExample]**` }`**
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `| `__`.`__[**`perRowCol`**][Update.perRowCol]**` {
     * `**[`rowColExpression`][ExpressionsGivenRowAndColumn.RowColumnExpression.WithExample]**` }`**
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `| `__`.`__[**`withNull`**][Update.withNull]**`()`**
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `| `__`.`__[**`withZero`**][Update.withZero]**`()`**
     *
     * &nbsp;&nbsp;&nbsp;&nbsp; `| `__`.`__[**`asFrame`**][Update.asFrame]**` {
     * `**[`dataFrameExpression`][ExpressionsGivenDataFrame.DataFrameExpression.WithExample]**` }`**
     */
    ...
    public interface Columns {
        ...
        // Optional argument that can be set to redirect where the [Selecting Columns] link points
        // to
        @Suppress("ClassName")
     ...
     * #### For example:
     *
     * <code>`df`</code>`.`[update][org.jetbrains.kotlinx.dataframe.api.update]` { length
     * `[and][ColumnsSelectionDsl.and]` age }`
     *
     * <code>`df`</code>`.`[update][org.jetbrains.kotlinx.dataframe.api.update]` {
     ...
     * #### For example:
     *
     * <code>`df`</code>`.`[update][org.jetbrains.kotlinx.dataframe.api.update]`("length", "age")`
     */
    ...
    /**
     * @param [columns] The [KProperties][KProperty] corresponding to columns of this [DataFrame] to
     *   update.
     */
    internal typealias KPropertiesParam = Nothing

    /**
     * @param [columns] The [Strings][String] corresponding to the names of columns belonging to
     *   this [DataFrame] to update.
     */
 ...
 * ## The Update Operation
 *
 * Returns the [DataFrame][org.jetbrains.kotlinx.dataframe.DataFrame] with changed values in some
 * cells (column types cannot be changed).
 *
 ...
 * [Selecting Columns][org.jetbrains.kotlinx.dataframe.api.Update.UpdateSelectingOptions] for all
 ...
 * ## Optional
 * Combine
 * `df.`[update][org.jetbrains.kotlinx.dataframe.api.update]`(...).`[with][org.jetbrains.kotlinx.dataframe.api.Update.with]`
 ...
 */
public fun <T> DataFrame<T>.update(vararg columns: String): Update<T, Any?> = update {
    columns.toColumnSet()
}
...
@AccessApiOverload
public fun <T, C> DataFrame<T>.update(vararg columns: KProperty<C>): Update<T, C> = update {
    columns.toColumnSet()
}
...
@Deprecated(DEPRECATED_ACCESS_API)
@AccessApiOverload
public fun <T, C> DataFrame<T>.update(vararg columns: ColumnReference<C>): Update<T, C> = update {
    columns.toColumnSet()
}
...
/**
 * ## Where
 * Filter or find rows to operate on after
 * [selecting columns][org.jetbrains.kotlinx.dataframe.documentation.SelectingColumns] using a
 ...
 *
 * `df.`[update][update]` { `[cols][ColumnsSelectionDsl.cols]`(1..5) }.`[where][where]` {
 * `[index][org.jetbrains.kotlinx.dataframe.index]`() > 4 && city != "Paris" }`
 *
...
/**
 * ## At
 * Only update the columns at certain given
 * [row indices][org.jetbrains.kotlinx.dataframe.api.CommonUpdateAtFunctionDoc.RowIndicesParam]:
 *
 * Either a [Collection]<[Int]>, an [IntRange], or just `vararg` indices.
 *
 * For example:
 *
 * `df.`[update][org.jetbrains.kotlinx.dataframe.api.update]` { city
 * }.`[at][org.jetbrains.kotlinx.dataframe.api.at]`(5..10).`[with][org.jetbrains.kotlinx.dataframe.api.with]`
 * { "Paris" }`
 *
 * `df.`[update][org.jetbrains.kotlinx.dataframe.api.update]` { name
 * }.`[at][org.jetbrains.kotlinx.dataframe.api.at]`(1, 2, 3,
 * 4).`[with][org.jetbrains.kotlinx.dataframe.api.with]` { "Empty" }`
 *
 * ### This At Overload
 *
 * Provide a [Collection]<[Int]> of row indices to update.
 *
 * @param [rowIndices] The indices of the rows to update. Either a [Collection]<[Int]>, an
 *   [IntRange], or just `vararg` indices.
 */
@Interpretable("UpdateAt")
public fun <T, C> Update<T, C>.at(rowIndices: Collection<Int>): Update<T, C> = where {
    index in rowIndices
}
 ...
 *
 * @param [rowRange] The indices of the rows to update. Either a [Collection]<[Int]>, an [IntRange],
 *   or just `vararg` indices.
 */
@Interpretable("UpdateAt")
 ...
 *
 * &nbsp;&nbsp;&nbsp;&nbsp;`row.age /
 * col.`[mean][org.jetbrains.kotlinx.dataframe.DataColumn.mean]`(skipNA = true)`
 *
 * `}`
 *
 * ## See Also
 * - [Update with][org.jetbrains.kotlinx.dataframe.api.Update.with] to provide a new value for every
 *   selected cell giving its row and its previous value.
 * - [Update per col][org.jetbrains.kotlinx.dataframe.api.Update.perCol] to provide a new value for
 *   every selected cell giving its column.
 *
 ...
 *   to provide a new value for every selected cell giving its row and column.
 */
@Refine
@Interpretable("UpdatePerRowCol")
public inline fun <T, C> Update<T, C>.perRowCol(
    crossinline expression: RowColumnExpression<T, C, C>
): DataFrame<T> = updateImpl { row, column, _ -> expression(row, column) }
 ...
 * - [Update per col][org.jetbrains.kotlinx.dataframe.api.Update.perCol] to provide a new value for
 *   every selected cell giving its column.
 * - [Update per row col][org.jetbrains.kotlinx.dataframe.api.Update.perRowCol] to provide a new
 *   value for every selected cell giving its row and column.
 *
 * @param [expression] The
...
public inline fun <T, C, R : C?> Update<T, C>.with(
    crossinline expression: UpdateExpression<T, C, R>
): DataFrame<T> = updateImpl { row, _, value -> expression(row, value) }
 ...
 * [dataframe expression][org.jetbrains.kotlinx.dataframe.DataFrameExpression].
 *
 * For example:
 *
 * `df.`[update][update]` { name }.`[asFrame][asFrame]` {
 * `[select][org.jetbrains.kotlinx.dataframe.DataFrame.select]` { lastName } }`
 *
 * @param [expression] The
 ...
 */
public fun <T, C, R> Update<T, DataRow<C>>.asFrame(
    expression: DataFrameExpression<C, DataFrame<R>>
): DataFrame<T> = asFrameImpl(expression)
 ...
 *   [column expression][org.jetbrains.kotlinx.dataframe.ColumnExpression].
 * - Provide a new value for every selected cell per column using a [Map][Map]`<`[colName:
 *   String][String]`, value: C>` or [DataRow][org.jetbrains.kotlinx.dataframe.DataRow] as Map.
 *
 * ### See Also
 * - [Update with][org.jetbrains.kotlinx.dataframe.api.Update.with] to provide a new value for every
 *   selected cell giving its row and its previous value.
 ...
 *
 * `df.`[update][org.jetbrains.kotlinx.dataframe.api.update]` { name and age
 * }.`[where][org.jetbrains.kotlinx.dataframe.api.Update.where]` { ...
 * }.`[perCol][org.jetbrains.kotlinx.dataframe.api.perCol]`(defaults)`
 *
 * @param [values] The [Map]<[String], Value> to provide a new value for every selected cell. For
 *   each selected column, there must be a value in the map with the same name.
 * @throws [IllegalArgumentException] if a value for a selected cell's column is not defined in
 *   [values].
 */
@Refine
...
public fun <T, C> Update<T, C>.perCol(values: Map<String, C>): DataFrame<T> =
    updateWithValuePerColumnImpl {
        values[it.name()]
            ?: throw IllegalArgumentException("Update value for column ${it.name()} is not defined")
    }
...
public fun <T, C> Update<T, C>.perCol(values: AnyRow): DataFrame<T> =
    perCol(values.toMap() as Map<String, C>)
 ...
 * For example:
 *
 * `df.`[update][update]` { age }.`[perCol][perCol]` {
 * `[mean][org.jetbrains.kotlinx.dataframe.DataColumn.mean]`(skipNA = true) }`
 *
 * `df.`[update][update]` { age }.`[perCol][perCol]` {
 * `[count][org.jetbrains.kotlinx.dataframe.DataColumn.count]` { it > 10 } }`
 *
 * @param [valueSelector] The
 *   [Column Expression][org.jetbrains.kotlinx.dataframe.documentation.ExpressionsGivenColumn.ColumnExpression]
 *   to provide a new value for every selected cell giving its column.
 */
@Refine
...
internal infix fun <T, C> RowValueFilter<T, C>?.and(
    other: RowValueFilter<T, C>
): RowValueFilter<T, C> {
 ...
 * For example:
 *
 * `df.`[update][update]` { `[colsOf][colsOf]`<`[Int][Int]`?>()
 * }.`[notNull][notNull]`().`[perRowCol][Update.perRowCol]` { row, col ->`
 *
 ...
 * For example:
 *
 * `df.`[update][update]` { `[colsOf][colsOf]`<`[Number][Number]`?>()
 * }.`[notNull][notNull]`().`[perCol][Update.perCol]` { `[mean][mean]`() }`
 *
 * ### Optional
 * Provide an [expression] to update the rows with. This combines [with][Update.with] with
 * [notNull].
 *
 * For example:
 *
 * `df.`[update][update]` { city }.`[notNull][Update.notNull]` {
 * it.`[toUpperCase][String.toUpperCase]`() }`
 *
 * @param expression Optional
 *   [Row Expression][org.jetbrains.kotlinx.dataframe.documentation.ExpressionsGivenRow.RowExpression.WithExample]
 *   to update the rows with.
 */
@Refine
...
/**
 * ## With Null
 * Specific version of [with][org.jetbrains.kotlinx.dataframe.api.with] that simply sets the value
 * of each selected row to `null`.
 *
 * For example:
 *
 * `df.`[update][org.jetbrains.kotlinx.dataframe.api.update]` { id
 * }.`[where][org.jetbrains.kotlinx.dataframe.api.Update.where]` { it < 0
 * }.`[withNull][withNull]`()`
 */
@Refine
