 ...
 *
 * If [ParserOptions.useFastDoubleParser] is enabled, it will try to parse the input with the fast
 * double parser library, [FastDoubleParser](https://github.com/wrandelshofer/FastDoubleParser). If
 * not, or if it fails, it will use [NumberFormat] to parse the input.
 *
 * The [locale][locale] used by the double parser is defined like:
 *
 * [parserOptions][parserOptions]`?.`[locale][ParserOptions.locale]` ?:
 * `[Parsers.locale][Parsers.locale]` :? `[Locale.getDefault()][Locale.getDefault]
 *
 * [FastDoubleParser] has a fallback mechanism; In practice, this means it can recognize symbols and
 * notations of any locale recognized by Java as long as that symbol does not conflict with the
 * given locale.
 *
 * For example, if your locale uses ',' as decimal separator, it will NOT recognize ',' as thousands
 * separator, but it will recognize ' ', '٬', '_', ' ', etc. as such. The same holds for characters
 * like "e", "inf", "×10^", "NaN", etc.
 *
 * Public, so it can be used in other modules.
 *
 * @param parserOptions can be supplied to configure the parser. If `null`, the global parser
 *   options ([DataFrame.parser][DataFrame.Companion.parser]) will be used. We'll only use
 *   [ParserOptions.locale] and [ParserOptions.useFastDoubleParser].
 */
@Suppress("ktlint:standard:comment-wrapping")
public class FastDoubleParser(private val parserOptions: ParserOptions? = null) {

    private val supportedFastCharsets =
        setOf(Charsets.UTF_8, Charsets.ISO_8859_1, Charsets.US_ASCII)

    private val useFastDoubleParser =
        parserOptions?.useFastDoubleParser ?: Parsers.useFastDoubleParser
    private val locale = parserOptions?.locale ?: Parsers.locale

    private val parser =
        ConfigurableDoubleParser(/* symbols= */ setupNumberFormatSymbols(), /* ignoreCase= */ true)

    /**
     * Sets up the [NumberFormatSymbols] for the [ConfigurableDoubleParser] based on the [locale]
     * with fallbacks from all other locales.
     *
     * Fallback characters/strings are only added if they're not clashing with local
     * characters/strings.
     */
    private fun setupNumberFormatSymbols(): NumberFormatSymbols =
        numberFormatSymbolsCache.getOrPut(locale) {
            ...
            /**
             * Builds a set with the specified char from [this] and [fallbackChars] will be added to
             * the set too, when they're safe to add.
             */
            fun Char.withFallback(fallbackChars: CharArray): Set<Char> = buildSet {
            ...
            /**
             * Builds a set with the specified string from [this] and [fallbackStrings] will be
             * added to the set too, when they're safe to add.
             */
            fun String.withFallback(fallbackStrings: Array<String>): Set<String> = buildSet {
            ...
            NumberFormatSymbols.fromDecimalFormatSymbols(localDecimalFormatSymbols)
                ...
                .withMinusSign(localDecimalFormatSymbols.minusSign.withFallback(MINUS_SIGNS))
                .withInfinity(localDecimalFormatSymbols.infinity.withFallback(INFINITIES))
                .withNaN(localDecimalFormatSymbols.naN.withFallback(NANS))
        }
    ...
    private fun String.parseToDoubleOrNullFallback(): Double? =
        when (lowercase()) {
            in INFINITIES,
            in PLUS_INFINITIES -> Double.POSITIVE_INFINITY
            ...
            if (it == null) {
                logger.trace {
                    "Could not parse '$this' as Double with NumberFormat with locale '$locale'."
                }
            }
        }
     ...
     *
     * It uses the [fast double parser][ConfigurableDoubleParser] if
     * [ParserOptions.useFastDoubleParser] is enabled, else, or if that fails, it uses
    ...
    /**
     * Here we store all possible decimal format symbols of all locales on the system. These will be
     * used as fallbacks for the selected locale. They are only added by [withFallback] if they
     * don't interfere with symbols already in the provided [locale] (so ',' is not added as
     * grouping separator if '.' is already the locale's decimal separator).
     */
    internal companion object {
        ...
        val INFINITIES by lazy {
            allDecimalFormatSymbols
                .mapNotNullTo(mutableSetOf()) { it.infinity }
        ...
        val NANS by lazy {
            allDecimalFormatSymbols
                .mapNotNullTo(mutableSetOf()) { it.naN }
        ...
        val GROUPING_SEPARATORS by lazy {
            allDecimalFormatSymbols
                .mapNotNullTo(mutableSetOf()) { it.groupingSeparator }
        ...
        val EXPONENTS by lazy {
            allDecimalFormatSymbols
                .mapNotNullTo(mutableSetOf()) { it.exponentSeparator }
                .toTypedArray()
        }
