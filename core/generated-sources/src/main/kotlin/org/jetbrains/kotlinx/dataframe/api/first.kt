...
public inline fun <T> DataFrame<T>.first(crossinline predicate: RowFilter<T>): DataRow<T> =
    rows().first {
        predicate(it, it)
    }

public inline fun <T> DataFrame<T>.firstOrNull(crossinline predicate: RowFilter<T>): DataRow<T>? =
    rows().firstOrNull {
        predicate(it, it)
    }
