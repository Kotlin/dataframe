 ...
 *
 * None of the neighbouring operations require the [<code>DataColumn</code>][DataColumn] to contain exactly one value:
 * [<code>first</code>][DataColumn.first] and [<code>last</code>][DataColumn.last] return the first or the last value,
 * [<code>firstOrNull</code>][DataColumn.firstOrNull] returns the first value,
 * or `null` if the [<code>DataColumn</code>][DataColumn] is empty,
 * and [<code>take</code>][DataColumn.take] and [<code>takeLast</code>][DataColumn.takeLast]
 * return several values as a new [<code>DataColumn</code>][DataColumn].
 * There is no `singleOrNull` for a [<code>DataColumn</code>][DataColumn].
 *
 ...
 * that return the first or the last row and do not require the [<code>DataFrame</code>][DataFrame]
 ...
 * that return the first or the last row, or `null` if the [<code>DataFrame</code>][DataFrame] is empty,
 * and do not require the [<code>DataFrame</code>][DataFrame] to contain exactly one row.
 *
 * @return A [<code>DataRow</code>][DataRow] containing the single row in this [<code>DataFrame</code>][DataFrame],
 ...
 * See also [<code>singleOrNull</code>][DataFrame.singleOrNull], that returns `null` instead of throwing,
 * and [<code>first</code>][DataFrame.first], [<code>last</code>][DataFrame.last],
 * that return the first or the last matching row and do not require the [<code>DataFrame</code>][DataFrame]
 ...
 * and do not require the [<code>DataFrame</code>][DataFrame] to contain exactly one matching row.
 *
 * @param [predicate] A [<code>row filter</code>][RowFilter] used to get the single row
 * that satisfies a condition specified in this filter.
 *
...
public interface SingleColumnsSelectionDsl {
     ...
     *
     * NOTE: For [<code>column groups</code>][ColumnGroup], `single` is named `singleCol` instead to avoid confusion.
     *
     ...
     * This function operates solely on columns at the top-level.
     *
     * NOTE: For [<code>column groups</code>][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup], `single` is named `singleCol` instead to avoid confusion.
     *
     * For more information: [See First (Col), Last (Col), Single (Col) on the documentation website.](https://kotlin.github.io/dataframe/columnselectors.html#first-col-last-col-single-col)
     *
