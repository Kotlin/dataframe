 ...
 * See also [lastOrNull], [first], [take], [takeLast].
 *
 * @param [predicate] A lambda expression used to get the last value that satisfies a condition
...
/**
 * Returns the last value in this [DataColumn] that matches the given [predicate]. Returns `null` if
 * the [DataColumn] contains no elements matching the [predicate] (including the case when the
 * [DataColumn] is empty).
 *
 * ### Example
 *
 ...
 *   specified in this expression. This predicate takes a value from the [DataColumn] as an input
 *   and returns `true` if the value satisfies the condition or `false` otherwise.
 * @return The last value in this [DataColumn] that matches the given [predicate], or `null` if the
 *   [DataColumn] contains no elements matching the [predicate].
 */
public inline fun <T> DataColumn<T>.lastOrNull(predicate: (T) -> Boolean): T? =
...
/**
 * Returns the last [row][DataRow] in this [DataFrame] that satisfies the given [predicate]. Returns
 * `null` if the [DataFrame] contains no rows matching the [predicate] (including the case when the
 * [DataFrame] is empty).
 *
 ...
 * ```
 *
 * See also [last][DataFrame.last], [first][DataFrame.first], [take][DataFrame.take],
 * [takeLast][DataFrame.takeLast], [takeWhile][DataFrame.takeWhile].
 *
 * @param [predicate] A [row filter][RowFilter] used to get the last value that satisfies a
 *   condition specified in this filter.
 * @return A [DataRow] containing the last row that matches the given [predicate], or `null` if the
 ...
 * ```
 *
 * See also [lastOrNull][DataFrame.lastOrNull], [first][DataFrame.first], [take][DataFrame.take],
 * [takeLast][DataFrame.takeLast], [takeWhile][DataFrame.takeWhile].
 *
 * @param [predicate] A [row filter][RowFilter] used to get the last value that satisfies a
 *   condition specified in this filter.
...
public inline fun <T> DataFrame<T>.last(predicate: RowFilter<T>): DataRow<T> =
    rowsReversed().first { predicate(it, it) }

/**
 * Returns the last [row][DataRow] in this [DataFrame]. If the [DataFrame] does not contain any
 * rows, returns `null`.
 *
 * See also [last][DataFrame.last], [first][DataFrame.first], [take][DataFrame.take],
 * [takeLast][DataFrame.takeLast].
 *
 * @return A [DataRow] containing the last row in this [DataFrame], or `null` if the [DataFrame] is
 *   empty.
 */
...
/**
 * [Reduces][GroupByDocs.Reducing] the groups of this [GroupBy] by taking the last [row][DataRow]
 * from each group, and returns a [ReducedGroupBy] containing these rows (one [row][DataRow] per
 * group, each [row][DataRow] is the last [row][DataRow] in its group).
 *
 ...
 *
 * @return A [ReducedGroupBy] containing the last [row][DataRow] (or a [row][DataRow] with `null`
...
/**
 * [Reduces][GroupByDocs.Reducing] the groups of this [GroupBy] by taking from each group the last
 * [row][DataRow] satisfying the given [predicate], and returns a [ReducedGroupBy] containing these
 * rows (one [row][DataRow] per group, each [row][DataRow] is the last [row][DataRow] in its group
 ...
 * @param [predicate] A [row filter][RowFilter] used to get the last value that satisfies a
 *   condition specified in this filter.
 * @return A [ReducedGroupBy] containing the last [row][DataRow] matching the [predicate] (or a
 *   [row][DataRow] with `null` values, except the grouping key) from each group.
 */
@Interpretable("GroupByReducePredicate")
public fun <T, G> GroupBy<T, G>.last(predicate: RowFilter<G>): ReducedGroupBy<T, G> = reduce {
...
/**
 * [Reduces][PivotDocs.Reducing] this [Pivot] by taking the last [row][DataRow] from each group, and
 * returns a [ReducedPivot] that contains the last [row][DataRow] from the corresponding group in
 * each column.
 *
 * For more information about [Pivot] with examples:
 ...
 *
 * @return A [ReducedPivot] containing in each column the last [row][DataRow] from the corresponding
 *   group.
 */
 ...
 * satisfying the given [predicate], and returns a [ReducedPivot] that contains the last
 * [row][DataRow], matching the [predicate], from the corresponding group in each column.
 *
 * For more information about [Pivot] with examples:
 * [See `pivot` on the documentation website.](https://kotlin.github.io/dataframe/pivot.html)
 *
 ...
 * @param [predicate] A [row filter][RowFilter] used to get the last value that satisfies a
 *   condition specified in this filter.
 * @return A [ReducedPivot] containing in each column the last [row][DataRow] that satisfies the
 *   [predicate], from the corresponding group (or a [row][DataRow] with `null` values)
 */
public fun <T> Pivot<T>.last(predicate: RowFilter<T>): ReducedPivot<T> = reduce {
    lastOrNull(predicate)
}
...
/**
 * [Reduces][PivotGroupByDocs.Reducing] this [PivotGroupBy] by taking the last [row][DataRow] from
 * each combined [pivot] + [groupBy] group, and returns a [ReducedPivotGroupBy] that contains the
 * last row from each corresponding group. If any combined [pivot] + [groupBy] group in
 ...
 *
 * For more information about [PivotGroupBy] with examples:
 ...
 *   key either the last [row][DataRow] of the corresponding [DataFrame] formed by this pivot–group
 ...
 * [pivot] + [groupBy] group the last [row][DataRow] satisfying the given [predicate]. Returns a
 * [ReducedPivotGroupBy] that contains the last [row][DataRow], matching the [predicate], from each
 ...
 *
 * [See "`pivot` + `groupBy`" on the documentation
 * website.](https://kotlin.github.io/dataframe/pivot.html#pivot-groupby)
 *
 * [See `pivot` on the documentation website.](https://kotlin.github.io/dataframe/pivot.html)
 *
 ...
 *   condition specified in this filter.
 * @return A [ReducedPivotGroupBy] containing in each combination of a [groupBy] key and a [pivot]
 *   key either the last matching the [predicate] [row][DataRow] of the corresponding [DataFrame]
 ...
 */
public fun <T> PivotGroupBy<T>.last(predicate: RowFilter<T>): ReducedPivotGroupBy<T> = reduce {
    lastOrNull(predicate)
}
...
public interface LastColumnsSelectionDsl {
     ...
     *
     * &nbsp;&nbsp;&nbsp;&nbsp;__`.`__[**`last`**][org.jetbrains.kotlinx.dataframe.api.ColumnsSelectionDsl.last]`
     * [ `**`{ `**[`condition`][org.jetbrains.kotlinx.dataframe.documentation.DslGrammarTemplateColumnsSelectionDsl.DslGrammarTemplate.ConditionDef]**`
     * }`**` ]`
     *
     * &nbsp;&nbsp;&nbsp;&nbsp;
     ...
     *
     * &nbsp;&nbsp;&nbsp;&nbsp;__`.`__[**`lastCol`**][org.jetbrains.kotlinx.dataframe.api.LastColumnsSelectionDsl.lastCol]`
     * [ `**`{ `**[`condition`][org.jetbrains.kotlinx.dataframe.documentation.DslGrammarTemplateColumnsSelectionDsl.DslGrammarTemplate.ConditionDef]**`
     * }`**` ]`
     */
    ...
    /**
     * ## Last (Col)
     * Returns the last column from [this] that adheres to the optional given [condition]. If no
     ...
     * #### Examples:
     *
     * `df.`[select][DataFrame.select]` { `[last][ColumnsSelectionDsl.last]` {
     * it.`[name][ColumnReference.name]`().`[startsWith][String.startsWith]`("order") } }`
     *
     * `df.`[select][DataFrame.select]` { "myColumnGroup".`[lastCol][String.lastCol]` {
     * it.`[name][ColumnReference.name]`().`[startsWith][String.startsWith]`("order") } }`
     *
     ...
     * This function operates solely on columns at the top-level.
     *
     * NOTE: For [column groups][org.jetbrains.kotlinx.dataframe.columns.ColumnGroup], `last` is
     * named `lastCol` instead to avoid confusion.
     *
     ...
     * `[last][org.jetbrains.kotlinx.dataframe.api.ColumnsSelectionDsl.last]` {
     * it.`[name][org.jetbrains.kotlinx.dataframe.columns.ColumnReference.name]`().`[startsWith][String.startsWith]`("order")
     * } }`
     *
     * `df.`[select][org.jetbrains.kotlinx.dataframe.DataFrame.select]` {
     * "myColumnGroup".`[lastCol][kotlin.String.lastCol]` {
     * it.`[name][org.jetbrains.kotlinx.dataframe.columns.ColumnReference.name]`().`[startsWith][String.startsWith]`("order")
     * } }`
     *
     * #### Examples for this overload:
     ...
     * `[colsOf][SingleColumn.colsOf]`<`[String][String]`>().`[last][ColumnSet.last]` {
     * it.`[name][ColumnReference.name]`().`[startsWith][String.startsWith]`("year") } }`
     *
     * `df.`[select][DataFrame.select]` {
     * `[colsOf][SingleColumn.colsOf]`<`[Int][Int]`>().`[last][ColumnSet.last]`() }`
     *
     * @param [condition] The optional [ColumnFilter][org.jetbrains.kotlinx.dataframe.ColumnFilter]
     *   condition that the column must adhere to.
     * @return A [SingleColumn][org.jetbrains.kotlinx.dataframe.columns.SingleColumn] containing the
     *   last column that adheres to the given [condition].
     * @throws [NoSuchElementException] if no column adheres to the given [condition].
     * @see [ColumnsSelectionDsl.first]
     */
     ...
     */
    @Interpretable("Last2")
    public fun SingleColumn<DataRow<*>>.lastCol(
        condition: ColumnFilter<*> = { true }
    ): SingleColumn<*> = this.ensureIsColumnGroup().asColumnSet().last(condition)
     ...
     * #### Examples for this overload:
     *
     * `df.`[select][DataFrame.select]` { Type::myColumnGroup.`[lastCol][SingleColumn.lastCol]` {
     * it.`[name][ColumnReference.name]`().`[startsWith][String.startsWith]`("year") } }`
     *
     * `df.`[select][DataFrame.select]` {
     * DataSchemaType::myColumnGroup.`[lastCol][KProperty.lastCol]`() }`
     *
     * @param [condition] The optional [ColumnFilter][org.jetbrains.kotlinx.dataframe.ColumnFilter]
     *   condition that the column must adhere to.
     * @return A [SingleColumn][org.jetbrains.kotlinx.dataframe.columns.SingleColumn] containing the
     ...
     * #### Examples for this overload:
     *
     * `df.`[select][DataFrame.select]` { "pathTo"["myColumnGroup"].`[lastCol][ColumnPath.lastCol]`
     * { it.`[name][ColumnReference.name]`().`[startsWith][String.startsWith]`("year") } }`
     *
     * @param [condition] The optional [ColumnFilter][org.jetbrains.kotlinx.dataframe.ColumnFilter]
     *   condition that the column must adhere to.
