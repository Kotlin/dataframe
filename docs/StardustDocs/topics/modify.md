...
The syntax of most column operations assumes that they are applied to columns, so they don't include the word `column` in their naming.    

On the other hand, the Kotlin DataFrame library follows `Koltin Collections` naming for row-wise operations
as [`DataFrame`](DataFrame.md)
can be interpreted as a [`Collection`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/) of rows. 
The slight naming difference with `Kotlin Collection` is that all operations are named in the imperative way: `sortBy`,
`shuffle` etc. 
...
* [implode](implode.md) — merge column values into lists grouped by other columns
