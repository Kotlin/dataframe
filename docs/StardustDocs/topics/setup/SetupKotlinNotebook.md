...
```
%use dataframe(%dataFrameNotebookVersion%)
```
...
> For version `%dataFrameVersion%`, in notebooks use version `%dataFrameNotebookVersion%` instead.
...
> %use dataframe, kandy
> // or
> %use dataframe(%dataFrameNotebookVersion%), kandy(%kandyVersion%)
> ```
> {style="warning"}
