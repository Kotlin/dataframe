...
/**
 * Implementations of this class are auto-generated from 'examples/projects' into
 * 'build/generated/testBuildingExamples'.
 *
 * See ':build-logic/src/main/kotlin/dfbuild/buildExampleProjects/generateTestCase.kt'.
 *
 * Each folder needs to have its own test class so the tests can run in parallel.
 *
 * This class, as well as the generated tests, are automatically registered as the
 * 'testBuildingExamples' [SourceSet][org.gradle.api.tasks.SourceSet] by the
 * `dfbuild.buildExampleProjects` convention plugin.
 */
@Suppress("unused")
abstract class TestBuildingExampleProjects {
    /**
     * Allows receiving certain Gradle properties. These are passed down to the tests from the
     * `dfbuild.buildExampleProjects` convention plugin.
     */
    protected fun getGradleProperty(name: String): String? =
        System.getProperty("gradle.properties.$name")

    protected fun buildExampleProject(folder: File, isDev: Boolean) {
        val name =
            folder.name.toCamelCaseByDelimiters().replaceFirstChar { it.uppercase() } +
                (if (isDev) "Dev" else "")

        val buildSystem =
            folder.detectBuildSystem()
                ?: error(
        ...
        when (buildSystem) {
            ...
            BuildSystem.MAVEN -> buildMavenProject(name = name, folder = folder)
        }
    }

    protected fun buildGradleProject(name: String, folder: File, isAndroid: Boolean) {
        // Needs the android.sdk.dir property to be set or -Pandroid.sdk.dir=... added as Gradle
        // argument
        ...
        GradleConnector.newConnector().forProjectDirectory(folder).connect().use {
            it.newBuild()
