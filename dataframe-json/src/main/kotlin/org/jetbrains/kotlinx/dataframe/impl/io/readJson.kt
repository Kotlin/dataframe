...
internal fun readJsonImpl(
    ...
    val df: AnyFrame =
        when (typeClashTactic) {
            ARRAY_AND_VALUE_COLUMNS -> {
                when (parsed) {
                    is JsonArray ->
                        fromJsonListArrayAndValueColumns(
                            records = parsed,
...
/**
 * Json to DataFrame converter that creates [Any] columns. A.k.a. [TypeClashTactic.ANY_COLUMNS].
 *
 ...
 * @param header Optional list of column names. If given, [records] will be read like an object with
...
internal fun fromJsonListAnyColumns(
    ...
    val columns: List<AnyCol> =
        when {
                        ...
                        is JsonArray -> {
                            val parsed =
                                fromJsonListAnyColumns(
                                    ...
                                    jsonPath =
                                        jsonPath
                                            .replaceLastWildcardWithIndex(i)
            ...
            // Create one column of type FrameColumn, or List<> from all the records if they are all
            // arrays
                            ...
                            DataColumn.createValueColumn(
                                ...
                                type =
                                    List::class.createType(
                                        listOf(KTypeProjection.invariant(elementType))
                                    ),
                            )
                        }
            ...
            colType == AnyColType.OBJECTS && isKeyValue -> {
                // collect the value types to make sure Value columns with lists and other values
                // aren't all turned into lists
                ...
                val dataFrames = records.map { record ->
                    when (record) {
                        is JsonObject -> {
                            ...
                            dataFrameOf(
                                columnOf(*map.keys.toTypedArray())
                                    .named(NameValueProperty<*>::name.name),
                                createColumnGuessingType(
                                        values = map.values,
                        ...
                        is JsonNull,
                        null -> DataFrame.emptyOf<AnyNameValueProperty>()
                ...
                val valueColumnSchema =
                    when {
                        // in these cases we can safely combine the columns to get a single column
                        // schema
                        valueColumns.all { it is ColumnGroup<*> } ||
                            valueColumns.all { it is FrameColumn<*> } ->
                            valueColumns.concat().toDataFrame().schema().columns.values.single()

                        // to avoid listification, we create the value columns schema ourselves
                        // (https://github.com/Kotlin/dataframe/issues/184)
                            ...
                            schema =
                                lazy {
                                    DataFrameSchemaImpl(
                                        columns =
                                            mapOf(
                                                NameValueProperty<*>::name.name to
                                                    ColumnSchema.Value(typeOf<String>()),
                                                NameValueProperty<*>::value.name to
                                                    valueColumnSchema,
                                            )
                                    )
                                },
            ...
            // Create multiple columns from all the records if they are all objects, merging the
            // objects in essence
            colType == AnyColType.OBJECTS && !isKeyValue -> {
                nameGenerator.names.map { colName ->
                    ...
                    when {
                        ...
                        parsed.isSingleUnnamedColumn() ->
                            ...
                            DataColumn.createColumnGroup(colName, parsed.unwrapUnnamedColumns())
                                as AnyCol
                    }
                }
            }
    ...
    return when {
        ...
        columns.size == 1 &&
            hasArray &&
            header.isNotEmpty() &&
            columns[0].typeClass == List::class ->
            columns[0].cast<List<*>>().splitInto(*header.toTypedArray())
...
/**
 * Json to DataFrame converter that creates allows creates `value` and `array` accessors instead of
 * [Any] columns. A.k.a. [TypeClashTactic.ARRAY_AND_VALUE_COLUMNS].
 *
 * @param records List of json elements to be converted to a [DataFrame].
 * @param unifyNumbers Whether to [unify the numbers that are read][UnifyingNumbers].
 * @param keyValuePaths List of [JsonPath]s where instead of a [ColumnGroup], a
...
internal fun fromJsonListArrayAndValueColumns(
    ...
    // list element type can be JsonObject, JsonArray or primitive
    // So first, we gather all properties of objects to merge including "array" and "value" if
    // needed
    // so the resulting type of a property with instances 123, ["abc"], and { "a": 1, "b": 2 } will
    // be
    ...
    records.forEach { record ->
        when (record) {
            is JsonObject -> record.entries.forEach { nameGenerator.addIfAbsent(it.key) }
    ...
    val valueColumn =
        if (hasPrimitive || records.isEmpty()) {
    ...
    // only properties that consist of just objects (or are empty) can be merged to key/value
    // FrameColumns
                    ...
                    UnnamedColumn(
                        DataColumn.createFrameColumn(
                            ...
                            schema =
                                lazy {
                                    dataFrames
                                        .mapNotNull { it.takeIf { it.rowsCount() > 0 }?.schema() }
                                        .intersectSchemas()
                                },
                        )
                    )
                        ...
                        colName == valueColumn && (hasPrimitive || records.isEmpty()) -> {
                            ...
                            val res =
                                if (nanIndices.isNotEmpty()) {
                                    fun <C> DataColumn<C>.updateNaNs(nanValue: C): DataColumn<C> {
                                        ...
                                        return mapIndexed(column.type) { i, v ->
                                            if (i == nextNanIndex) {
                                                j++
                                                nextNanIndex =
                                                    if (j < nanIndices.size) nanIndices[j] else -1
                                                nanValue
                                            } else {
                                                v
                                            }
                                    ...
                                    when (column.typeClass) {
                                        Double::class ->
                                            column.cast<Double?>().updateNaNs(Double.NaN)
                                        Float::class -> column.cast<Float?>().updateNaNs(Float.NaN)
                                        String::class -> column.cast<String?>().updateNaNs("NaN")
                                        else -> column
                                    }
                            ...
                            val res =
                                when {
                                    parsed.isSingleUnnamedColumn() -> {
                                        ...
                                        parsed.unwrapUnnamedColumns().chunked(startIndices, colName)
                                }
                            UnnamedColumn(res)
                        }
...
// unnamed column means this column is not created from field of a record [{"value": 1}, {"value":
// 2}],
