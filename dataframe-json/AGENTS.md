...
`binary-compatibility-validator` guards the API (see root `AGENTS.md` for the `apiDump`/`apiCheck` flow).
Module-specific: the `@Deprecated(level = HIDDEN)` `readJson(stream, ...)` overloads exist purely for binary
