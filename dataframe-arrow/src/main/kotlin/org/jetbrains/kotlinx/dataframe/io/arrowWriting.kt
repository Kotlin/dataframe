...
/**
 * Create [ArrowWriter] for [this] DataFrame with explicit [targetSchema]. If DataFrame does not
 * match with [targetSchema], behaviour is specified by [mode], mismatches would be sent to
 * [mismatchSubscriber]
 */
...
public fun AnyFrame.writeArrowIPC(channel: WritableByteChannel) {
    this.arrowWriter().use { writer -> writer.writeArrowIPC(channel) }
}
...
public fun AnyFrame.writeArrowIPC(stream: OutputStream) {
    this.arrowWriter().use { writer -> writer.writeArrowIPC(stream) }
}
 ...
 * Save data to
 * [Arrow interprocess streaming format](https://arrow.apache.org/docs/java/ipc.html#writing-and-reading-streaming-format),
 * write to new or existing file on the given [path]. If file exists, it can be recreated or
 * expanded.
 */
public fun AnyFrame.writeArrowIPC(path: Path, append: Boolean = true) {
    this.arrowWriter().use { writer -> writer.writeArrowIPC(path, append) }
}
...
public fun AnyFrame.saveArrowIPCToByteArray(): ByteArray =
    this.arrowWriter().use { writer -> writer.saveArrowIPCToByteArray() }
...
public fun AnyFrame.writeArrowFeather(channel: WritableByteChannel) {
    this.arrowWriter().use { writer -> writer.writeArrowFeather(channel) }
}
...
public fun AnyFrame.writeArrowFeather(stream: OutputStream) {
    this.arrowWriter().use { writer -> writer.writeArrowFeather(stream) }
}
 ...
 * Save data to
 * [Arrow random access format](https://arrow.apache.org/docs/java/ipc.html#writing-and-reading-random-access-files),
 * write to new or existing file on the given [path]. If file exists, it would be recreated.
 */
public fun AnyFrame.writeArrowFeather(path: Path) {
    this.arrowWriter().use { writer -> writer.writeArrowFeather(path) }
}
...
public fun AnyFrame.saveArrowFeatherToByteArray(): ByteArray =
    this.arrowWriter().use { writer -> writer.saveArrowFeatherToByteArray() }
