...
/**
 * Create Arrow [Field] (note: this is part of [Schema], does not contain data itself) that has the
 * same name, type and nullable as [this]
 */
public fun AnyCol.toArrowField(
...
internal fun ColumnSchema.toArrowField(
    name: String,
    mismatchSubscriber: (ConvertingMismatch) -> Unit,
): Field =
    when (this) {
        ...
        is ColumnSchema.Group -> {
            val childFields =
                schema.columns.map { (childName, childSchema) ->
        ...
        is ColumnSchema.Frame -> {
            ...
            val childStructField =
                Field("item", FieldType(true, ArrowType.Struct(), null), childFields)
            Field(name, FieldType(nullable, ArrowType.List(), null), listOf(childStructField))
        }
    }

internal fun KType.toArrowField(
    name: String,
    mismatchSubscriber: (ConvertingMismatch) -> Unit,
): Field {
    val nullable = isMarkedNullable
    return when {
        this == nullableNothingType ->
            Field(name, FieldType(true, ArrowType.Null(), null), emptyList())
...
/**
 * Create Arrow [Schema] matching [this] actual data. Columns with not supported types will be
