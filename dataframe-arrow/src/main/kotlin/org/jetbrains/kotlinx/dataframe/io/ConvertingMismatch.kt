...
/**
 * Detailed message about any mismatch when saving to Arrow format with user-defined schema that
 * does not match with actual data. Can be sent to callback, written to log or encapsulated to
 * exception
 */
public sealed class ConvertingMismatch(
    ...
    public sealed class WideningMismatch(column: String) : ConvertingMismatch(column, null, null) {
        public data class AddedColumn(override val column: String) : WideningMismatch(column) {
            override fun toString(): String =
                "Added column \"$column\" not described in target schema"
        }

        public data class RejectedColumn(override val column: String) : WideningMismatch(column) {
            override fun toString(): String =
                "Column \"$column\" is not described in target schema and was ignored"
        }
    }

    public sealed class NarrowingMismatch(column: String) : ConvertingMismatch(column, null, null) {
        public data class NotPresentedColumnIgnored(override val column: String) :
            NarrowingMismatch(column) {
            override fun toString(): String =
                "Not nullable column \"$column\" is not presented in actual data, saving as is"
        }

        public data class NotPresentedColumnError(override val column: String) :
            NarrowingMismatch(column) {
            override fun toString(): String =
    ...
    public sealed class TypeConversionNotFound(
        ...
        public data class ConversionNotFoundIgnored(
            ...
            override fun toString(): String =
                "${cause.message} for column \"$column\", saving as is"
        }
    ...
    public data class SavedAsString(override val column: String, val type: Class<*>) :
        ...
        override fun toString(): String =
            "Column \"$column\" has type ${type.canonicalName}, will be saved as String\""
    }

    public sealed class NullableMismatch(column: String, row: Int?) :
