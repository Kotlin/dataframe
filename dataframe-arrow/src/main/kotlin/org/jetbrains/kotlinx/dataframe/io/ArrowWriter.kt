...
public interface ArrowWriter : AutoCloseable {
    ...
    /**
     * If [restrictWidening] is true, [dataFrame] columns not described in [targetSchema] would not
     * be saved (otherwise, would be saved as is). If [restrictNarrowing] is true, [targetSchema]
     * fields that are not nullable and do not exist in [dataFrame] will produce exception
     * (otherwise, would not be saved). If [strictType] is true, [dataFrame] columns described in
     * [targetSchema] with non-compatible type will produce exception (otherwise, would be saved as
     * is). If [strictNullable] is true, [targetSchema] fields that are not nullable and contain
     * nulls in [dataFrame] will produce exception (otherwise, would be saved as is with nullable =
     * true).
     */
    ...
    /**
     * Create Arrow [VectorSchemaRoot] with [dataFrame] content cast to [targetSchema] according to
     * the [mode].
     */
     ...
     * write to opened [channel].
     */
    public fun writeArrowIPC(channel: WritableByteChannel) {
        allocateVectorSchemaRoot().use { vectorSchemaRoot ->
            ArrowStreamWriter(vectorSchemaRoot, null, channel).use { writer -> writer.writeBatch() }
        }
    }
     ...
     * Save data to
     * [Arrow interprocess streaming format](https://arrow.apache.org/docs/java/ipc.html#writing-and-reading-streaming-format),
     * write to new or existing [file]. If file exists, it can be recreated or expanded.
     */
     ...
     * Save data to
     * [Arrow interprocess streaming format](https://arrow.apache.org/docs/java/ipc.html#writing-and-reading-streaming-format),
     * write to a new or existing file on the given [path]. If file on the given [path] exists, it
     * can be recreated or expanded.
     */
    ...
    public fun writeArrowFeather(channel: WritableByteChannel) {
        allocateVectorSchemaRoot().use { vectorSchemaRoot ->
            ArrowFileWriter(vectorSchemaRoot, null, channel).use { writer -> writer.writeBatch() }
        }
    }
     ...
     * Save data to
     * [Arrow random access format](https://arrow.apache.org/docs/java/ipc.html#writing-and-reading-random-access-files),
     * write to new or existing [file]. If [file] exists, it would be recreated.
     */
