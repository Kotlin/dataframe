...
internal class ArrowKtTest {
    ...
    fun testResource(resourcePath: String): URL =
        ArrowKtTest::class.java.classLoader.getResource(resourcePath)!!
    ...
    fun testReadingFromFile() {
        ...
        val c by columnOf("c1" to columnOf("inner"), "c2" to columnOf(4.0), "c3" to columnOf(50.0))
    ...
    fun testReadingAllTypesAsEstimated() {
        ...
        assertEstimations(
            exampleFrame =
                DataFrame.readArrowIPC(testArrowIPC("test.arrow"), NullabilityOptions.Infer),
            expectedNullable = false,
            hasNulls = false,
        )
        ...
        assertEstimations(
            exampleFrame =
                DataFrame.readArrowIPC(testArrowIPC("test.arrow"), NullabilityOptions.Checking),
            expectedNullable = true,
            hasNulls = false,
        )
        ...
        assertEstimations(
            exampleFrame =
                DataFrame.readArrowIPC(testArrowIPC("test.arrow"), NullabilityOptions.Widening),
            expectedNullable = true,
            hasNulls = false,
        )
    }
    ...
    fun testWritingBySchema() {
        val testFile = File.createTempFile("cities", "arrow")
        citiesExampleFrame.arrowWriter(Schema.fromJSON(citiesExampleSchema)).use {
            it.writeArrowFeather(testFile)
        }
    ...
    fun testWidening() {
        ...
        shouldThrow<IllegalArgumentException> {
            DataFrame.readArrowFeather(testRestrictWidening)["page_in_wiki"]
        }

        val testAllowWidening =
            citiesExampleFrame
                .arrowWriter(
                    targetSchema = Schema.fromJSON(citiesExampleSchema),
                    mode =
    ...
    fun testNarrowing() {
        ...
        frameWithoutRequiredField
            .arrowWriter(
                targetSchema = Schema.fromJSON(citiesExampleSchema),
                mode = ArrowWriter.Mode.STRICT,
            )
            .use { shouldThrow<ConvertingException> { it.saveArrowFeatherToByteArray() } }

        val warnings = ArrayList<ConvertingMismatch>()
        val testAllowNarrowing =
            frameWithoutRequiredField
                .arrowWriter(
                    targetSchema = Schema.fromJSON(citiesExampleSchema),
                    mode =
        ...
        warnings.shouldContain(
            ConvertingMismatch.NarrowingMismatch.NotPresentedColumnIgnored("settled")
        )
        shouldThrow<IllegalArgumentException> {
            DataFrame.readArrowFeather(testAllowNarrowing)["settled"]
        }
    }
    ...
    fun testStrictType() {
        ...
        frameWithIncompatibleField
            .arrowWriter(Schema.fromJSON(citiesExampleSchema), ArrowWriter.Mode.STRICT)
            .use { shouldThrow<ConvertingException> { it.saveArrowFeatherToByteArray() } }
    ...
    fun testStrictNullable() {
        ...
        DataFrame.readArrowFeather(testLoyalNullable)["settled"].type() shouldBe
    ...
    fun testParsing() {
        ...
        val frameString =
            dataFrameOf("columnDot", "columnComma")(columnStringDot, columnStringComma)
        ...
        try {
            ...
            DataFrame.readArrowFeather(serializedAsUs) shouldBe
                dataFrameOf("columnDot", "columnComma")(columnDoubleFraction, columnDoubleRound)
            ...
            DataFrame.readArrowFeather(serializedAsRu) shouldBe
                dataFrameOf("columnDot", "columnComma")(columnDoubleFraction, columnDoubleFraction)
        } finally {
    ...
    fun testBigMixedColumn() {
                ...
                targetSchema =
                    Schema(
                        listOf(
                            Field(
                                "bigMixedColumn",
                                FieldType.nullable(ArrowType.Int(64, true)),
                                emptyList(),
                            )
                        )
                    ),
        ...
        assert(
            warnings
                .filterIsInstance<ConvertingMismatch.TypeConversionFail.ConversionFailIgnored>()
                .size == 1
        )
        assert(warnings.filterIsInstance<ConvertingMismatch.SavedAsString>().size == 1)

        DataFrame.readArrowFeather(data)["bigMixedColumn"] shouldBe
            dataFrame["bigMixedColumn"].map { it.toString() }
    }
    ...
    private fun writeArrowTimestamp(
        ...
        RootAllocator().use { allocator ->
            ...
            val schemaTimeStamp =
                Schema(listOf(timeStampNano, timeStampMicro, timeStampMilli, timeStampSec))
            VectorSchemaRoot.create(schemaTimeStamp, allocator).use { vectorSchemaRoot ->
                val timeStampMilliVector =
                    vectorSchemaRoot.getVector("ts_milli") as TimeStampMilliVector
                val timeStampNanoVector =
                    vectorSchemaRoot.getVector("ts_nano") as TimeStampNanoVector
                val timeStampMicroVector =
                    vectorSchemaRoot.getVector("ts_micro") as TimeStampMicroVector
    ...
    fun testArrowReaderExtension() {
        ...
        val featherChannel =
            ByteArrayReadableSeekableByteChannel(expected.saveArrowFeatherToByteArray())
    ...
    fun testDuckDBArrowIntegration() {
        ...
        conn.use {
            ...
            Assert.assertTrue(
                dbArrowReader.javaClass.name.equals("org.apache.arrow.c.ArrowArrayStreamReader")
            )
    ...
    fun testColumnGroupRoundtrip() {
        val original =
            dataFrameOf(
                ...
                "inner" to
                    columnOf("nested1" to columnOf("a", "b", "c"), "nested2" to columnOf(1, 2, 3)),
            )
    ...
    fun testNestedColumnGroupRoundtrip() {
        val deeplyNested by columnOf("level2" to columnOf("level3" to columnOf(1, 2, 3)))
    ...
    fun testColumnGroupWithNulls() {
        val group by columnOf("a" to columnOf("x", null, "z"), "b" to columnOf(1, 2, null))
    ...
    fun testReadParquetWithListColumns() {
        ...
        df.columnNames() shouldBe
            listOf("id", "numbers", "strings", "nullable_list", "list_with_nulls")
        ...
        df["numbers"].values().toList() shouldBe
            listOf(listOf(1L, 2L, 3L), listOf(4L, 5L), listOf(6L))
        df["strings"].values().toList() shouldBe
            listOf(listOf("a", "b"), listOf("c"), listOf("d", "e", "f"))
        df["nullable_list"].values().toList() shouldBe listOf(listOf(1L, 2L), null, listOf(3L))
        df["list_with_nulls"].values().toList() shouldBe
            listOf(listOf(1L, null, 3L), listOf(4L, 5L), listOf(null))
    }
    ...
    fun `read parquet with two batches and nulls in second batch`() {
        ...
        // batch 1 — no nulls
        orders[0]["item"].values().toList() shouldBe listOf("Laptop", "Mouse")
        // we're deliberately skipping our usual nullability narrowing for ListVector<StructVector>
        // FrameColumns
    ...
    fun `read parquet with lists of all primitive types`() {
        ...
        // signed ints
        df["list_int8"].values().toList() shouldBe
            listOf(listOf(1.toByte(), (-1).toByte()), emptyList<Byte>())
        df["list_int16"].values().toList() shouldBe
            listOf(listOf(1000.toShort(), (-1000).toShort()), listOf(42.toShort()))
        df["list_int32"].values().toList() shouldBe
            listOf(listOf(100000, -100000), emptyList<Int>())
        df["list_int64"].values().toList() shouldBe
            listOf(listOf(10000000000L, -10000000000L), listOf(0L))

        // unsigned ints (widened types)
        df["list_uint8"].values().toList() shouldBe
            listOf(listOf(0.toShort(), 255.toShort()), listOf(128.toShort()))
        ...
        df["list_uint64"].values().toList() shouldBe
            listOf(
        ...
        // decimal
        df["list_decimal"].values().toList() shouldBe
            listOf(listOf(BigDecimal("123.45"), BigDecimal("-67.89")), listOf(BigDecimal("0.00")))

        // string
        df["list_string"].values().toList() shouldBe
            listOf(listOf("hello", "world"), emptyList<String>())
        df["list_large_string"].values().toList() shouldBe
            listOf(listOf("foo", "bar"), listOf("baz"))

        // bool
        df["list_bool"].values().toList() shouldBe listOf(listOf(true, false), listOf(true))

        // date
        df["list_date_day"].values().toList() shouldBe
            listOf(listOf(LocalDate(2024, 1, 15), LocalDate(2025, 6, 30)), emptyList<LocalDate>())
        df["list_date_ms"].values().toList() shouldBe
            listOf(
        ...
        // time
        df["list_time"].values().toList() shouldBe
            listOf(listOf(LocalTime(9, 30, 0), LocalTime(14, 15, 30)), emptyList<LocalTime>())

        // timestamp
        df["list_timestamp"].values().toList() shouldBe
            listOf(
        ...
        df["list_duration"].values().toList() shouldBe
            listOf(listOf(1.hours, 30.minutes), emptyList<Duration>())

        // null vs empty list
        df["nullable_list"].values().toList() shouldBe listOf(listOf(10, 20), null)
        df["nullable_elements"].values().toList() shouldBe
