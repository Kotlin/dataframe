...
class DataFrameUnfold : AbstractSchemaModificationInterpreter() {
    ...
    override fun Arguments.interpret(): PluginDataFrameSchema {
        return receiver.asDataFrame().replace { columns }.with {
            val d = it.asSimpleColumn() as? SimpleDataColumn
            if (d != null) {
                if (!d.type.type.canBeUnfolded(session)) {
                    it
                } else {
                    SimpleColumnGroup(it.name(), toDataFrame(maxDepth, d.type.type, TraverseConfiguration()).columns()).asDataColumn()
                }
            } else {
                it
            }
