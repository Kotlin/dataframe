...
/**
 * Runs the example notebooks in the dev folder (`/examples/notebooks/dev`), only when debug mode is
 * on: `kotlin.dataframe.debug=true`
 */
class SampleNotebooksTests : DataFrameJupyterTest() {
    ...
    @Test fun puzzles() = exampleTest("puzzles", "40 puzzles")

    @Test
    fun github() =
        exampleTest(
            dir = "github",
            cellClause = CellClause.stopAfter { cell -> "personal access token" in cell.source },
            cleanup = { File("jetbrains.json").delete() },
        )
    ...
    fun titanic() =
        exampleTest(
            ...
            replacer = CodeReplacer.byMap(testFile("titanic", "titanic.csv")),
        )
    ...
    fun wine() =
        exampleTest(
            ...
            replacer = CodeReplacer.byMap(testFile("wine", "winequality-red.csv")),
        )
    ...
    fun netflix() {
        ...
        try {
            // Set explicit locale as of test data contains locale-dependent values (date for
            // parsing)
    ...
    fun movies() =
        exampleTest(
            dir = "movies",
            replacer = CodeReplacer.byMap(testFile("movies", "movies.csv")),
            // There is no tags data in repository
            cellClause = CellClause.stopAfter { cell -> "tags.csv" in cell.source },
        )
    ...
    @Test fun quickstart() = exampleTest(dir = "quickstart")
    ...
    private fun doTest(
        ...
        try {
            ...
            if (
                e.causesSequence().filterIsInstance<IllegalStateException>().any {
                    it.message?.contains("null DefinitelyNotNullType for") == true
                }
            ) {
    ...
    companion object {
        ...
        fun testFile(folder: String, fileName: String) =
