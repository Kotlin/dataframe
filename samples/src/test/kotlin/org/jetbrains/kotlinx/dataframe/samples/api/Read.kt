...
class Read : DataFrameSampleHelper("read", "api") {
    ...
    @Test
    fun csvLocale() {
        val df = dataFrameOf("numbers" to columnOf("12,123", "41,111")).saveDfHtmlSample()
    }
    ...
    fun readJsonRow() {
        ...
        row.columnTypes() shouldBe
            listOf(typeOf<String>(), typeOf<Int>(), typeOf<Float>(), typeOf<Boolean>())
    }
    ...
    fun readJsonFromUrl() {
        // SampleStart
        DataFrame.readJson(
            "https://raw.githubusercontent.com/Kotlin/dataframe/refs/heads/master/data/participants.json"
        )
        // SampleEnd
    }
    ...
    fun readJsonTypeClashTactic() {
        ...
        // SampleStart
        val df =
            DataFrame.readJsonStr(text, typeClashTactic = JSON.TypeClashTactic.ANY_COLUMNS)
                // SampleEnd
                .saveDfHtmlSample()
    }
    ...
    fun readNumbersWithSpecificLocale() {
        ...
        // SampleStart
        val df =
            DataFrame.readCsv(file, parserOptions = ParserOptions(locale = Locale.UK))
                // SampleEnd
                .saveDfHtmlSample()
    }
    ...
    fun readNumbersWithColType() {
        ...
        // SampleStart
        val df =
            DataFrame.readCsv(file, colTypes = mapOf("numbers" to ColType.String))
                // SampleEnd
                .saveDfHtmlSample()
    }
    ...
    fun dateTimeSample() {
        val df =
            dataFrameOf("date" to columnOf("13/Jan/23 11:49 AM", "14/Mar/23 5:35 PM"))
                .saveDfHtmlSample()
    }
        ...
        val df =
            DataFrame.readCsv(
                    ...
                    parserOptions =
                        ParserOptions(
                            dateTime =
                                DateTimeParserOptions.Kotlin.withFormat(
                                    LocalDate.Format {
    ...
    fun readDatesWithDefaultType() {
        val file = testCsv("dates")
        // SampleStart
        val df = DataFrame.readCsv(file, colTypes = mapOf(ColType.DEFAULT to ColType.String))
        // SampleEnd
    }
