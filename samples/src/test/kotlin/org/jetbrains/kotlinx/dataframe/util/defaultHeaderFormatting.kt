...
internal fun <T> DataFrame<T>.defaultHeaderFormatting(
    headers: ColumnsSelector<T, *>
): FormattedFrame<T> {
    ...
    return columns.foldIndexed(start) { idx, acc, header ->
        acc.formatHeader { header }.with { textColor(baseColorSet[idx]) and monospace }
    }
}

@Suppress("INVISIBLE_REFERENCE")
internal fun <T> FormattedFrame<T>.defaultHeaderFormatting(
    headers: ColumnsSelector<T, *>
): FormattedFrame<T> {
