...
class Modify : DataFrameSampleHelper("operations", "modify") {
    ...
    fun formatExample_strings() {
        ...
        df.format()
            .with { bold and textColor(black) and background(white) }
            .format("name")
            .with { underline }
            .format { "name"["lastName"] }
            .with { italic }
            .format("isHappy")
            .with { background(if (it as Boolean) green else red) }
            .format("weight")
            .notNull()
            .with { linearBg(it as Int, 50 to blue, 90 to red) }
            .format("age")
            .perRowCol { row, col ->
                col as DataColumn<Int>
                textColor(linear(value = col[row], from = ageMin to blue, to = ageMax to green))
            }
            // SampleEnd
            .saveDfHtmlSample()
    }
    ...
    fun formatExample_properties() {
        ...
        df.format()
            ...
            .format { weight }
            .notNull()
            .linearBg(50 to FormattingDsl.blue, 90 to FormattingDsl.red)
            .format { age }
            .perRowCol { row, col ->
                textColor(linear(value = col[row], from = ageMin to blue, to = ageMax to green))
            }
    ...
    fun columnArithmetics_concatenation() {
        val weather = dataFrameOf("day" to columnOf(1, 2, 3), "temperature" to columnOf(10, 20, 30))
    ...
    fun columnArithmetics_times() {
        val routes =
            dataFrameOf("route" to columnOf("A", "B", "C"), "distanceKm" to columnOf(1.2, 3.5, 0.8))
        val products =
            dataFrameOf(
