...
internal object DelimParams {
    ...
    /**
     * @param path The file path to read. Use [charset\] to specify the encoding. Can also be
     *   compressed as `.gz` or `.zip`, see [Compression][Compression].
     */
    typealias PATH_READ = Nothing

    /**
     * @param file The file to read. Use [charset\] to specify the encoding. Can also be compressed
     *   as `.gz` or `.zip`, see [Compression][Compression].
     */
    typealias FILE_READ = Nothing

    /**
     * @param url The URL from which to fetch the data. Use [charset\] to specify the encoding. Can
     *   also be compressed as `.gz` or `.zip`, see [Compression][Compression].
     */
     ...
     *
     *   If non-empty, the data will be read with \[header\] as the column titles (use \[skipLines\]
     *   if there's a header in the data). If empty (default), the header will be read from the
     *   data.
     */
     ...
     *
     *   Fixed-width columns can occur, for instance, in multi-space delimited data, where the
     *   columns are separated by multiple spaces instead of a single delimiter, so columns are
     *   visually aligned. Column widths are determined by the header in the data (if present), or
     *   manually by setting \[fixedColumnWidths\].
     */
     ...
     *
     *   Requires \[hasFixedWidthColumns\]. If empty, the column widths will be determined by the
     *   header in the data (if present), else, this manually sets the column widths. The number of
    ...
    /**
     * @param colTypes The expected [ColType] per column name. Default: empty map, a.k.a. infer
     *   every column type.
     *
     *   If supplied for a certain column name (inferred from data or given by \[header\]), the
     *   parser will parse the column with the specified name as the specified type, else it will
     *   infer the type.
     *
     *   e.g. `colTypes = `[mapOf][mapOf]`("colName" `[to][to]`
     *   `[ColType][ColType]`.`[Int][ColType.Int]`)`. You can also set
     *   [ColType][ColType]`.`[DEFAULT][ColType.DEFAULT]` `[to][to]` `[ColType][ColType]`.X` to set
     *   a _default_ column type, like [ColType.String].
     */
    val COL_TYPES: Map<String, ColType> = emptyMap()

    /**
     * @param skipLines The number of lines to skip before reading the header and data. Default:
     *   `0`.
     *
     *   Useful for files with metadata, or comments at the beginning, or to give a custom
     *   \[header\].
     */
     ...
     *     - [nullStrings][ParserOptions.nullStrings], which, if `null`, will take the global
     *       setting + {@include [DefaultNullStringsContentLink]}.
     *     - [skipTypes][ParserOptions.skipTypes], which will always add
     *       [typesDeephavenAlreadyParses] to the given types or the global setting.
     */
     ...
     *
     *   If `false`, empty lines will be interpreted as having _empty_ values if
     *   \[allowMissingColumns\].
     */
    ...
    /**
     * @param ignoreSurroundingSpaces Whether to ignore leading and trailing blanks around
     *   non-quoted fields. Default: `true`.
     */
    ...
    /**
     * @param quoteMode The [QuoteMode] to use when writing CSV / TSV files. Default:
     *   [QuoteMode.MINIMAL].
     */
    val QUOTE_MODE: QuoteMode = QuoteMode.MINIMAL

    /**
     * @param escapeChar The escape character to use when writing CSV / TSV files with
     *   [QuoteMode.NONE]. Default: `null`. This will double-quote the value.
     */
    val ESCAPE_CHAR: Char? = null

    /**
     * @param commentChar The character that indicates a comment line in a CSV / TSV file. Default:
     *   `'#'`.
     */
    const val COMMENT_CHAR: Char = '#'

    /**
     * @param recordSeparator The character that separates records in a CSV / TSV file. Default:
     *   `'\\n'`, a Unix-newline.
     */
     ...
     *
     *   Before instantiating the [CSVFormat], the [CSVFormat.Builder] will be passed to this
     *   lambda. This will allow you to configure/overwrite any CSV / TSV writing options.
     */
